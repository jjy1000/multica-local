...
> **Current release: 0.5.8 (in progress).** 0.5.6 拆解 + 0.5.7 trust 入口恢复已 ship;0.5.8
> 在 `server/pkg/taskfailure/classify.go` 增加了对 Anthropic SDK `EmptyResponseError` /
> `APIConnectionError`(`API Error: API returned an empty or malformed response (HTTP 200)`)
> 的识别,归类为 `ReasonAgentProviderServerError` 让 task queue 走 retry 路径。同时
> 把 fork 缺失的 `server/pkg/redact` 包源码恢复(handler/service 仍 import 它)。0.5.7
> (commit `d92201c`): trust 纠错入口恢复 + 0.5.6 拆解遗留死代码清理 — 前端 self-opt
> 信任面已 product-level 直通(no flag gate),`experiments/` 下 2 个死 manifest 目录删除,
> `installableSources` 死 entry 清除。审计基线(2026-08-02): 6 个剩余实验 flag 主干全通、`claude_science_lab` 锁 294 科研 skill 为设计内(用户自建 40 个未锁)。
