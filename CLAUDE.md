...
>
> **Current release: 0.5.103** (2026-09-07, shipped `3c22b916b`; `/Applications/Multica.app` = 0.5.103, cold-start verified; **board count/body parity + Pythia engine auto-start** — `0b29633ec` fixed the true root cause behind 0.5.102's duplicate-cards incident: the board's per-category fan-out sent `status_category` which the client never serialized AND the server `ListIssues` never parsed, so all 7 buckets carried the same global page + workspace-wide total (every column badged 92/425). Server now parses `status_category`/`statuses`/`assignee_filters`/`project_ids`/`label_ids`/etc. (fragments mirrored from `ListGroupedIssues`), client serializes them, and `useLoadMoreByStatus` gained `workspaceFilter` so badge read/load-more key/request share one filter-scoped cache row. `d5a90373e` fixed the Pythia 预演 "引擎不可用" fallback: engine subprocess only spawned via lab-panel visit (issue dispatch never mounts one → `source=synthetic`, probabilities 105%/112% from `0.42+0.07r`); now auto-starts at boot + on labs-toggle event, synthetic probability clamped ≤0.97, IssueLabsSection link gates on `suffix && labEnabled` (the `hides_deliverable` gate showed "实验室未启用" for a flag that was ON), and the section header carries a running-task stop button. `03965d94b` pins repocache tests against a global `core.hooksPath` override. Notes: [`.omc/0.5.103-ship-2026-09-07.md`](.omc/0.5.103-ship-2026-09-07.md). Prior: 0.5.102 (board-dedupe hotfix — the 3-layer dedupe in `addIssueToBuckets`/`flattenIssueBuckets`/`buildColumns` stays as defense in depth; keep its regression tests), 0.5.101 (upstream port MUL-7050 + MUL-7016 B.4; MUL-7008 deferred), 0.5.100 (3 MULs + ship-chain verify fatal), 0.5.97 (lab result rendering overhaul). Full per-release history: `.omc/<ver>-ship-<date>.md`.
>
