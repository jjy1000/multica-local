...
>
> **Current release: 0.5.48** (2026-08-21, head `0db8b9da2` on `epic/0.5.13-integration`, 1 functional commit on top of 0.5.47: integrate `cjk-emphasis` remark plugin into fork's `readonly-content.tsx` (CJK markdown `'**水温适度。**水的温度'` edge-case now renders correctly). 3 of 4 SKIPs from 0.5.44 cleared (MUL-6472 + CJK LANDED; MUL-6310 / MUL-6417 still pending). `pnpm typecheck` 6/6, `go test` 32/34 (1 pre-existing flaky `TestDashboardPerAgentRollupsUseExactWindow` + 1 pre-existing mythos panic). Historical release notes (0.5.12 → 0.5.41) archived at [`.omc/_legacy/release-notes-archive.md`](.omc/_legacy/release-notes-archive.md); 0.5.43-0.5.48 notes at `.omc/release-notes-0.5.{43,44,45,46,47,48}.md`; load-bearing contracts from those releases live in [Known Stability Surfaces](#known-stability-surfaces), [Active Contracts](#active-contracts-03457---polling-fallback-lab-leader-rewrite-by-issue-route-order), [Fork-Applicable HIGH Vuln Contracts](#fork-applicable-high-vuln-contracts-8-pending-0518-sec-first).
