...
> Lab environment batch (plan `.omc/plans/lab-environment-0.5.18-plan.md` 阶段 0–3, 13 atomic commits). 阶段 0 reliability/security P1: Pythia proxy allowlist +`/agent/events`+`/scorecard/resolve` (DT-P1-2), IPC `ensure-up` gated on safety-net blacklist (DT-P1-1), `user_` flag-key prefix deduped via `experimental.UserPluginPrefix` (BE-P1-2), `lab_managed` stamped on single-fetch `GetAgent`/`GetSquad` (SEC-P1-7, was list-only). 阶段 1 (G1 core): user-plugin `runtime_kind: subprocess` now executes `manifest.runtime.command`+`args` (argv, no shell) in the sandbox env — was 501 "reserved upgrade slot". 阶段 2 (G1 verify): `multica lab delegate` normalization + 4 poll-path tests, `multica-lab-builder` SKILL updated (6 built-ins + subprocess contract + self-test checklist), `scripts/lab-plugin-smoke.sh`. 阶段 3 (G2): `code_canvas` replaced the /health-only stub with a real stdlib-only `/render` service + render view + i18n. `pnpm typecheck` 6/6; `go test` green (2 pre-existing/env failures unrelated). Zero migrations. Next: 阶段 4 (LabOutputPanel + iframe auth proxy — spec) + 阶段 5 (6 HIGH vulns; triage `.omc/audit/2026-08-05-vuln-scan/triage.md`).
>
