...
> **Current release: 0.5.40 (committed + shipped 2026-08-19, installed at /Applications/Multica.app, server PID 88073).**
> 4 atomic commits on epic/0.5.13-integration (19115a4b6…0d1cf2582). Headlines: **CVE-2026-9277 shell-quote dep bump** + **MUL-6330 exact-prefix skill picker** + **MUL-6362 redact secrets from provider command logs** (with NEW fork-local `launch.go`/`launch_test.go` + 13 runtime edits + Config.LaunchPrefix field + structural regression guard). Verification: `pnpm typecheck` 6/6 + `go test ./pkg/agent` 11.7s + vitest 26/26 (21 existing + 5 new ranking) + ship chain PASS, cold start ~4s. Pre-existing mythos panic confirmed pre-existing via worktree on 0.5.39 HEAD; unrelated to this batch. Release note `.omc/release-notes-0.5.40.md`, ship log `.omc/0.5.40-ship-2026-08-19.md`.
>
> Key contracts (the load-bearing reasons this release is non-trivial):
> - **MUL-6362 launch.go NEW file** (`server/pkg/agent/launch.go`, 158 LOC): fork had no `launch.go` (upstream patch modified an existing one), so fork's port is a net-new file. Holds the redaction helpers (`Config.logAgentCommand`, `Config.logAgentCommandWithPrompt`, `redactAgentCommandArgs`, `safeAgentCommandFlagName`, `isASCIIAlpha`). Trust model: `agentCommandLogArgs` records adapter's invocation argv + optional trusted positionals; trust is only honored if `LaunchPrefix + invocationArgs` match the final exec.Cmd argv suffix — mismatch fails closed. `Config.LaunchPrefix []string` is a NEW field in fork (tolerates nil for built-in providers; future custom-runtime-profile fixed-args can set it).
> - **MUL-6362 structural regression guard** (`server/pkg/agent/launch_test.go::TestOnlyLaunchGoLogsAgentCommandArgs`): AST-walks every .go file in `pkg/agent` and fails on any `Debug/Info/Warn/Error/Log/LogAttrs` call carrying `args`/`argv`/`agent command` literal or `args`/`cmdArgs`/`argv`/`cmd.Args` identifier. Catches both direct reverts and renamed-equivalent reverts. Cheap (~50 LOC test, <100ms). This is the kind of guard that prevented the 0.5.36 MUL-6243 port wholesale-adoption trap.
> - **MUL-6362 13 runtime files surgically ported** (per-file diff 2/1 or 2/2 for codex which also got the comment fix): antigravity/claude/codebuddy/copilot/cursor/openclaw/qoder (no trust), hermes/kimi/kiro (`"acp"` trust), opencode (`logAgentCommandWithPrompt` + `"run"` trust), codex (`"app-server"` trust). 7 upstream runtimes (mcode/grok/qwen/qwenpaw/reasonix/traecli/deveco) skipped because fork has no ProviderLogo implementation outside the 13-case enum.
> - **MUL-6330 exact-prefix ranking** (`e42b2883e`): slash-command suggestion ranks matches as exact name > name prefix > name substring > description, preserving configured order within tier. 5 new tests pin tier ordering + 20-item cap interaction. Fork file paths identical to upstream; per-file diff 1.0x.
> - **CVE-2026-9277 dep bump** (`19115a4b6`): `shell-quote` 1.8.3 → 1.9.0, transitive via `react-devtools-core@6.1.5` (dev-only). Forced via `pnpm.overrides`. Lock + package.json only.
> - **MUL-6264 + MUL-6394 SKIP-NO-SURFACE this round (added to deferred)**: MUL-6264 chat agent names — fork has no `packages/views/chat/components/chat-thread-list.tsx` (fork chat = `chat-window.tsx` + `chat-fab.tsx` + `use-chat-context-items.ts`). MUL-6394 comment create sibling caches — fork has no `onIssueAuxiliaryRevision`; no `bucketedListEntries`/`flatListEntries`/`tableRowEntries`/`issueArrayEntries` helpers (fork's cache invalidation = `cache-helpers.ts` with `addIssueToBuckets`/`findIssueLocation`/`patchIssueInBuckets`). Both revert to upstream if/when fork migrates the surface.
>
> Deferred: MUL-6286 (actor properties — port WITH MUL-6305 fail-closed migration), MUL-5651 (port MUL-4257/4302/4304/4351 first), MUL-6321 OpenClaw slow hosts, MUL-6063 delegated task failures, MUL-5991 (jcode), 0c69f1f95 (Hermes resume-auth), MUL-6350 plugin rebuild 1/4 (watch series — 3/4 hook-engine branch also assessed this round, SKIP-DEAD-CASE for whole series: ~30k LOC wholesale, fork's user-plugin model is local subprocess + sandbox-isolated, zero overlap with upstream's 3rd-party iframe + HMAC-outbound, 4/4 unmerged), upstream new-commit scan (last: `aa6d6dadc` baseline; 23 commits scanned through `659bb41abd` this round — net 3 ported + 2 SKIP-NO-SURFACE added to deferred: MUL-6264 chat-thread-list missing in fork, MUL-6394 onIssueAuxiliaryRevision pattern missing in fork; 6 MUL-6243 incremental commits + MUL-6343 semantic activity timestamps + MUL-6396 perf realtime fanout flood still deferred for next-round deep-eval), **MUL-6323** (worktree-gate blame redirect — agent died 429 mid-port leaving 23 files +1819 LOC / 2 wholesale files / 6 typecheck errors; reverted to HEAD. Retry = fresh session + quota + small-chunk pattern: Go batch → TS batch → main-thread commit), mythos child-done notification (service-layer gap, see 0.5.38), plus 7 SKIPs:
