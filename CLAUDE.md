...
Before editing any subsystem with a known-regression or regression-suspect surface, read the matching memory file in `~/.claude/projects/-Users-jiangjianyan-jjy-multica-exploration-dev/memory/` (re-pointed 2026-09-02: the pre-rename `multica-main` slug directory no longer exists — the live store, 31 files, sits under the current `multica-exploration-dev` slug). The full index is in `MEMORY.md` next to the files (one line per memory, descriptive title only).
