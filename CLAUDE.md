...
>
> **Current release: 0.5.102** (2026-09-07, shipped `f8026081f`; `/Applications/Multica.app` = 0.5.102, cold-start ~6s; **board-dedupe hotfix** — `d6f918c6c` patched a client-side cache → render bug where creating one issue showed N identical cards in the same column (5× in 待办 / 4× in 进行中 was a real example). Fix is 3-layer defensive dedupe in `addIssueToBuckets` (`packages/core/issues/cache-helpers.ts:41`) + `flattenIssueBuckets` (`packages/core/issues/queries.ts:149`) + `buildColumns` (`packages/views/issues/utils/drag-utils.ts:45`) + 3 regression tests; +107 LOC. Notes: [`.omc/0.5.102-ship-2026-09-07.md`](.omc/0.5.102-ship-2026-09-07.md). Prior: 0.5.101 (2 of 3 PRs upstream port — MUL-7050 + MUL-7016 B.4; PR-3 MUL-7008 still deferred), 0.5.100 (3 MULs ported + ship-chain verify fatal landed), 0.5.97 (lab result rendering overhaul). Full per-release history: `.omc/<ver>-ship-<date>.md`.
>
