...
> **Current release: 0.5.23 (committed 2026-08-16, awaiting packaging).**
> MUL-3963 + MUL-4525 full port on top of 0.5.22 baseline (`5d3cacd31` plan doc) — 9 atomic commits + 1 i18n Phase 4. 1. `feat(agents)` `86a51e417` — MUL-3963 invocation-permission schema (migration 245: `agent.invocation_permissions` JSONB + `agent_template_invocation_permissions` projection table; sqlc queries). 2. `feat(agents)` `1543dda98` — MUL-3963 invocation-permission core helpers + wire (`packInvocationPermissions` / `unpackInvocationPermissions` + template-engine gate). 3. `fix(handlers)` `5dd927517` — MUL-3963 call-site wiring; view gate keys on `permission_mode`. 4. `feat(comments)` `98ab672d4` — surface blocked @mention `trigger_outcomes` (MUL-4525 §2; upstream 51325eeab). 5. `fix(comments)` `f0d7a1b91` — one outcome per explicit mention (MUL-4525 §2 round-2; upstream 7b03baa37). 6. `fix(comments)` `7ae5f6f2f` — honest role + status in `trigger_outcomes` fan-out. 7. `fix(comments)` `a0e8af0eb` — fail-closed active-task check, never fake deferred. 8. `fix(comments)` `de1adfc5c` — honest merge outcome (refused merge is blocked, not fake coalesced). Plus: MUL-4525 §2 merge-with-originator-re-stamp (`b749ee8ef` thread originator + `c72a7453a` close test gap + `1ec401a9e` un-skip `TestConsecutiveCommentsDifferentOriginatorsFullEnqueuePath` + `d8d73a9f5` clean up t.Skip comment), MUL-4010 (`7a28ea3bb` `permission_mode`/`invocation_targets` through template create), MUL-4063 (`49de9c47e` wake private-leader squad parent leader on child-done), MUL-4305 (`50dbf7d9b` keep originator on agent-created issues), and Phase 4 i18n (`84be8440f` register `swarm` locale keys in `resources-types.ts` + `locales/index.ts` for all 4 locales en/zh-Hans/ja/ko). **Closes test skip**: `TestConsecutiveCommentsDifferentOriginatorsFullEnqueuePath` (the 0.5.21-deliberately-skipped gate) now passes. `pnpm typecheck --force` 6/6 (33.7s, cache bypassed); `go test -count=1 ./internal/handler/ ./internal/experimental/ ./internal/service/ ./cmd/multica/` all green (handler 12.9s + experimental 0.7s + service 1.3s + cmd/multica 1.3s). Zero migrations on top of 0.5.22 (migration 245 already shipped with 0.5.22 baseline). Awaiting packaging per `make ship-mac` (user said "先无需打包" — don't ship until asked). Next: 0.5.23 packaging (optional), or audit + commit the 10 uncommitted working-tree files (`packages/core/{api,permissions,types}` + `packages/views/agents/*` + new `access-picker.tsx` — MUL-3963/MUL-4010 agent-detail-instrumentation work that did not land in 0.5.23).
>
> **0.5.22 (committed 2026-08-16, awaiting packaging).**
...
> Verification: `pnpm typecheck` 6/6; `go test -count=1 ./internal/handler/` `./internal/experimental/` `./internal/service/` all green (14.6s handler + 0.4s experimental + 1.2s service); gofmt clean on all edited files. Zero migrations. Awaiting packaging per `make ship-mac` (user said "先无需打包" — don't ship until asked).
> Next: 0.5.22 packaging (optional), or move to next ship candidate (semantica × Multica Phase 2 follow-up, or 0.5.23).
>
