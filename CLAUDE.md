...
>
> **Current release: 0.5.80** (2026-08-27, on `epic/0.5.72-followups`; `/Applications/Multica.app` = 0.5.80, official cold-start verify PASS ~4s). Chain recap: **0.5.77** — Batch 5 REVIEW-tier cherry-picks (MUL-6639 skill file metadata + CLI stall detection; MUL-6658 per-backend process-tree ownership; 22 atomic sub-commits) — [`.omc/release-notes-0.5.77.md`](.omc/release-notes-0.5.77.md). **0.5.78** — labs hardening: plugin visibility purge on delete/reseed, signed-URL TTL at HTTP boundary, `ASSIGNEE_MUTEX_LABS = {mythos_swarm, swarm_topology}`, UTC-anchored dashboard seeds, semantica wheel builder. **0.5.79** — MUL-6703 split-port "Import from local" skill archive upload (server multipart `finishSkillImport` + core pack-archive + dialog + locales ×4). **Fork deviation kept (edit-time trap)**: the skills API has NO zod envelope layer — client calls use the raw-JSON idiom via `this.fetch`/`fetchRaw`; upstream skill ports must adapt, never import the envelope. **0.5.80** — labs fullscreen-takeover fix: in-tab navigation into `/experimental/*` unmounted `WorkspaceRouteLayout`, whose cleanup cleared the workspace singleton → `{slug && <AppSidebar/>}` dropped the left rail + toolbar and the lab rendered fullscreen. New one-shot release-suppression handshake (`platform/workspace-singleton-release-guard.ts`) armed by every navigation path into `/experimental/*` (both adapters' push/replace/openInNewTab, pinned-new-tab helper, `multica:navigate` handler) and consumed in the layout's unmount cleanup; `ExperimentalViewShell`'s fixed Back pill removed (it was this bug's workaround). Full detail: [`.omc/release-notes-0.5.80.md`](.omc/release-notes-0.5.80.md), ship log: [`.omc/0.5.80-ship-2026-08-27.md`](.omc/0.5.80-ship-2026-08-27.md). Historical chain details for 0.5.77–0.5.79 stay in their release notes / previous header text in git history. Authoritative upstream triage: [`.omc/upstream-integration-triage-2026-08-26.md`](.omc/upstream-integration-triage-2026-08-26.md) (through incremental #3; upstream head `76aada3a9`). **Open decision gate: upstream inbox architecture (MUL-6632, 19 files ≈ 4400 LOC)** — adopt vs fork-local filtering; until decided, inbox-family upstream commits stay SKIP-DIVERGENCE. Ship log (0.5.79): [`.omc/0.5.79-ship-2026-08-27.md`](.omc/0.5.79-ship-2026-08-27.md).
