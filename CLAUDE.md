...
> **Current release: 0.5.17 (shipped 2026-08-12).**
...
- **F-007 (HIGH 0.9, FA-3) — Self-opt auto-apply replaces ENTIRE instructions.** File: `server/internal/service/agent_self_optimization/runner.go:594`. **Note**: CLAUDE.md + 0.5.2 ship log claim "auto-apply only ever adds". Audit may have caught a real regression or a documentation drift. **Plan**: Read `runner.go:594` first to verify; if audit is correct, fix to load current instructions + append edit. If audit is wrong, document the discrepancy in the triage file. **Status**: pending verify-then-fix (0.5.18+).
