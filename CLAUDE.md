...
>
> **Current release: 0.5.47** (2026-08-21, head `ba3d96918` on `epic/0.5.13-integration`, 5 functional commits on top of 0.5.46: 8 autopilot_quota migrations (261-268) + sqlc regen + `clientErrorMessage` helper + MUL-6472 dispatch leak (first user-facing cherry-pick since 0.5.44) + `pkg/pluginruntime` stub. Unblocks 3 of 4 SKIPs from 0.5.44: MUL-6472 LANDED ✅; MUL-6310 / CJK / MUL-6417 still need more. `pnpm typecheck` 6/6, `go test` 32/34 (1 pre-existing flaky `TestDashboardPerAgentRollupsUseExactWindow` + 1 pre-existing mythos panic). Historical release notes (0.5.12 → 0.5.41) archived at [`.omc/_legacy/release-notes-archive.md`](.omc/_legacy/release-notes-archive.md); 0.5.43-0.5.47 notes at `.omc/release-notes-0.5.{43,44,45,46,47}.md`; load-bearing contracts from those releases live in [Known Stability Surfaces](#known-stability-surfaces), [Active Contracts](#active-contracts-03457---polling-fallback-lab-leader-rewrite-by-issue-route-order), [Fork-Applicable HIGH Vuln Contracts](#fork-applicable-high-vuln-contracts-8-pending-0518-sec-first).
...
> **Read the "Current release" header at the very top of this file first** — that paragraph captures the load-bearing contracts of whatever release is live (today: 0.5.46). It changes per ship. Memory files below are the deeper lessons.
