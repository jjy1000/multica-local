...
>
> **Current release: 0.5.50** (2026-08-21, head `2c67c2207` on `epic/0.5.13-integration`, 3 commits on top of 0.5.49: 2 sqlc queries (SetAgentTaskBranchName + SetAgentTaskErrorIfEmpty, MUL-6310 prerequisite) + 3 agent_task_queue keyset indexes (MUL-5999 workspace teardown). Schema-only — no binary behavior change. **Ship skipped** (per 0.5.46/0.5.49 no-op pattern). 0.5.48 stays installed. MUL-6310 caller integration still blocked on task.go + protocol + TaskService methods. `pnpm typecheck` 6/6, `go build` clean. Historical release notes (0.5.12 → 0.5.41) archived at [`.omc/_legacy/release-notes-archive.md`](.omc/_legacy/release-notes-archive.md); 0.5.43-0.5.50 notes at `.omc/release-notes-0.5.{43,44,45,46,47,48,49,50}.md`; 0.5.49 retrospective at `.omc/0.5.49-retrospective.md`; load-bearing contracts from those releases live in [Known Stability Surfaces](#known-stability-surfaces), [Active Contracts](#active-contracts-03457---polling-fallback-lab-leader-rewrite-by-issue-route-order), [Fork-Applicable HIGH Vuln Contracts](#fork-applicable-high-vuln-contracts-8-pending-0518-sec-first).
