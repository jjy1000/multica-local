...
>
> **Current release: 0.5.75** (2026-08-27, `epic/0.5.72-followups`, 4 upstream cherry-picks on top of 0.5.74 lab-integration polish). Headline: **Upstream integration batch** — surgically ported 4 of the 5 cherry-picks the prior session's isolated-worktree batch landed. (1) **PR B2 #7557** `fix(cli): remove obsolete autopilot priority flag` — `server/cmd/multica/cmd_autopilot.go` drops the `--priority` flag from the autopilot subcommand, migrates the `cmd_autopilot_test.go` table to use `tag` for priority-bearing autopilot references; (2) **PR B2 #7418** `feat(server): MULTICA_TASK_QUEUED_TTL` — env-driven TTL for queued agent tasks (`defaultTaskQueuedTTL = 2h`, env override) + `TestQueuedTTLFromEnv` (4 subtests) + `TestDefaultTaskQueuedTTL` (pin 2h) + `apps/desktop/src/main/server-manager.ts::serializeEnvFile` adds `MULTICA_TASK_QUEUED_TTL=5m` (fork overrides upstream's 2h with 5m — single-user desktop fails fast on stale queued tasks); (3) **PR B3 #6634** `MUL-5928: bound board description previews` — `packages/views/issues/components/description-preview.tsx` clamps description preview rendering to bounded chars/lines; (4) **PR B3 #7385** `fix(github): support single-character issue prefixes` — `server/internal/handler/github.go` allows single-char prefixes in GH integration. **Skipped**: PR B3 #7018 `fix(chat): keep following rapid live output` reverted due to incomplete cherry-pick (the upstream test file referenced fork-absent props `transformContent`/`onQuickAction`/`quickActionsPendingMessageId`/`quickActionsDisabled`/`message_kind: 'onboarding_*'`/`quick_actions` — per CLAUDE.md §Known Stability Surfaces "Cherry-pick completeness check" lesson; the source-side 5-line followOutput fix is recoverable later with a custom test file). **Pre-existing failure locked**: `TestDashboardPerAgentRollupsUseExactWindow` fails on `eb9d23704` baseline (not introduced by this batch) — same class as `TestQuickCreateIssueParentTrustBoundary` (CLAUDE.md §Known Stability Surfaces). Verified: `pnpm typecheck` 6/6 PASS; `go test -count=1 -timeout 600s ./internal/... ./pkg/agent/...` passes all packages except the 2 pre-existing failures. Ship log: [`.omc/0.5.75-ship-2026-08-27.md`](.omc/0.5.75-ship-2026-08-27.md). Cross-session memory: `~/.claude/projects/-Users-jiangjianyan-jjy-multica-exploration-dev/memory/0.5.75-upstream-integration-paused-2026-08-26.md`.
