...
>
> **Current release: 0.5.76** (2026-08-27, `epic/0.5.72-followups`, 2 upstream cherry-picks on top of 0.5.75). Headline: **Upstream integration batch #2** — 5 parallel-batch agent workers recovered 2 surgical cherry-picks from `Batch B` + `Batch C2` of the upstream triage plan: (1) **MUL-6658** `fix(cli): tell a rejected task token to stop, not to sign in again` — `server/internal/cli/{client,errors}.go` adds `serverMessagePrefixes` map + `KindAuthRequired && httpErr.TaskScoped` short-circuit so the CLI does not prompt the user to re-authenticate when their task token is actually rejected; removes `TestSetHeaders_AdvertisesStableAttachmentURLs` from cherry-pick (asserts MUL-5372 `X-Client-Capabilities: stable_attachment_urls` header not in fork — per CLAUDE.md §Known Stability Surfaces "Cherry-pick completeness check" lesson). 11 new test cases pin behavior; (2) **#6081** `fix(autopilots): keep configuration panel visible for wide runbooks` — `packages/views/autopilots/components/autopilot-dialog.tsx:604` adds `min-w-0` to Runbook container so wide runbooks don't squeeze the configuration panel. **Skipped (per triage plan, ALREADY_PRESENT in fork history)**: Batch A 4 CVEs (`builder-util-runtime 9.7.0`, `brace-expansion`, `CVE-2026-9277 shell-quote`, `JWT secret fail-fast`) — fork commits `e57d0b57c`/`f5de3a927`/`d03025093`/`7a14de758` already shipped equivalent fixes pre-0.5.75. **Skipped (fork-divergence, structural)** Batch C1 4/4 + Batch C2 4/5 — chat (`build-steps.ts` rewritten to `build-timeline.ts` by fork `42dbf60dc8`; chat-message-list props from reverted #7018) + inbox (different file layout; fork uses `inbox-display.ts` not `inbox-list.tsx`) + i18n (no-value filtering feature never ported; `filter-chips-bar.tsx` upstream-only) + workspace members tab (fork deleted per CLAUDE.md §Localized Fork). **Skipped (CLAUDE.md §Localized Fork violations)** Batch D all docs/changelog — v0.4.35 entry explicitly advertises Cloud/billing/seat-management features. Verified: `pnpm typecheck` 6/6 PASS; new `TestFormatErrorRejectedTaskTokenDoesNotSuggestAnotherCredential` + 6 `TestExtractServerMessage` subtests PASS; Go test all green except 2 pre-existing failures (locked). Ship log: [`.omc/0.5.76-ship-2026-08-27.md`](.omc/0.5.76-ship-2026-08-27.md).
