...
>
> **Current release: 0.5.93** (2026-08-31, shipped from `epic/0.5.72-followups`; `/Applications/Multica.app` = 0.5.93, cold-start PASS ~6s; ship log [`.omc/0.5.93-ship-2026-08-31.md`](.omc/0.5.93-ship-2026-08-31.md); release notes [`.omc/release-notes-0.5.93.md`](.omc/release-notes-0.5.93.md)). **0.5.93 promotes MCP 管理 from a settings tab to a first-class configure nav page** (commit `d6601de08` + bump `b1ec61ff2`, same-day user feedback after 0.5.92): sidebar 配置 group now reads 运行时 / Skills / **MCP** / 设置 (`NAV_PAGE_REGISTRY` + `paths.mcp` + web `[workspaceSlug]/mcp` + desktop `path: "mcp"` routes); `mcp-sync-tab.tsx` git-mv'd to `packages/views/mcp/` (strings stay in settings.json `mcp_sync.*` — keys shipped in 0.5.92), settings `?tab=mcp-sync` registration REMOVED (move, not copy). Verified: typecheck 6/6; installed-asar checks — desktop route ×1, nav registry ×2, locale labels ×16, settings tab ×0; shipped with the app RUNNING (no Postgres abort, cf. 0.5.92 lesson). Previous release **0.5.92** (2026-08-31): MCP sync mirror + MCP call usage metric (detail paragraph below; ship log [`.omc/0.5.92-ship-2026-08-31.md`](.omc/0.5.92-ship-2026-08-31.md)). The causal graph loop stays closed both ways — write ~90% since 0.5.83, read ~70% since 0.5.85. The 0.5.84 contracts (#8 trust ladder + #9 FLAG_ROUTE_SUFFIX + never-nag + TouchCausalNode callsite) all still hold.
>
...
**No red-baseline rule (since 2026-09-01)**: `pnpm typecheck`, `pnpm lint`, and `pnpm test` are green at HEAD, and `check.sh` runs all three plus Go + E2E. Do NOT reintroduce a "N failures = pre-existing baseline" note; that pattern hid an accessible-name defect, a page-white-screening crash, and a security-allowlist bypass for two weeks. If a suite must be parked (an undecided upstream contract such as the MUL-6632 inbox family), skip it explicitly with a comment naming the gate and what still passed — never by leaving it red. Current parked count: 33 (`inbox-page.test.tsx`, 2 marker tests in `description-preview.test.ts`).
