...
> **Current release: 0.5.36 (committed 2026-08-18, shipped + installed at /Applications/Multica.app).**
> **MUL-6243 fully closed out end-to-end.** Backend (0.5.33) + TS/CLI (0.5.34) + board-by-category foundation (`16c8d6955`) + picker/board/filters/settings UI (`7767b3f65` + 2 surgical fixups). Custom per-workspace issue statuses are now usable from every surface: catalog-driven status picker grouped by category, board columns as CATEGORIES (custom status never adds a column), status filter lists the workspace catalog, settings page (`issue-statuses-tab.tsx`, 678 LOC) with drag reorder + archive-retires behavior.
> **Process lesson (biggest of this cycle)**: agent "take-theirs" conflict resolutions can silently import upstream's ENTIRE file (4.4x LOC inflation: agent worktree 22767 insertions vs upstream's 5186). Verify each file's diff size against `git show <upstream-commit> --stat` per-file; files 5-50x larger = wholesale adoption → revert to the pre-cherry-pick commit (NOT `HEAD` — HEAD already contains the bad commit) + re-apply the semantic change surgically. When agents die mid-flight (API 429 quota), the main thread's tool calls continue fine — assess the worktree state, revert if trapped, finish surgically.
> **4 cherry-picks also landed**: MUL-6300 (`1d2ba1d3f`) daemon reply-turn status arc, MUL-6334 (`906b07c5d` + `7006b90ea` gpt-5.6-sol pricing) bill cache reads in cost charts, MUL-6272 (`878ed6942` + `5eb1f471c`) command palette Pages from nav registry.
> **Skipped this cycle**: MUL-6305 (gated on MUL-6286 actor properties — fork never shipped migration 341), MUL-5651 (gated on MUL-4257/4302/4304/4351 batch primitives — fork-missing sqlc methods + AgentTaskQueue columns). Both tracked for 0.5.37+.
> **Verification**: `pnpm typecheck` 0 errors + `go build ./...` clean + full `go test ./internal/... ./pkg/agent/...` sweep passes except the documented pre-existing `TestTickSupervision_CompletionByFinalIssueStatus` panic (byte-identical at 0.5.34 baseline). Pre-existing (verified at 16c8d6955 in isolated worktree): 2 issue-detail scroll-to-comment tests. Ship chain 7/7 PASS, cold start 4s, Server PID 37263. Release note `.omc/release-notes-0.5.36.md`. Memory `0.5.36-mul6243-ui-closeout-2026-08-18.md`.
> Deferred: MUL-6286 (actor properties — port WITH MUL-6305 fail-closed migration), MUL-5651 (port MUL-4257/4302/4304/4351 first), MUL-6321 OpenClaw slow hosts, MUL-6063 delegated task failures, MUL-5991 (jcode), 0c69f1f95 (Hermes resume-auth), MUL-6350 plugin rebuild 1/4 (watch series), upstream new-commit scan (last: `9d6c0c81e` + 46 commits through v0.4.29 — scout 2026-08-19 surfaced 2 SKIPs below). Plus two port-attempt SKIPs documented 2026-08-19 (client-log audit follow-up):
