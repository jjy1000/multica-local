...
> **Current release: 0.5.38 (committed + shipped 2026-08-19, installed at /Applications/Multica.app).**
> `fix(daemon)` `5a57a89a1` — **agent-completed child issues now notify + wake their parent agent** (2 files +111, zero migrations). `notifyParentOfChildDone` was only wired into the HTTP `UpdateIssue` / `BatchUpdateIssues` / GH-merge paths; the daemon `CompleteTask` path flipped `issue.status`→`done` via direct SQL (`UpdateIssueStatus`), bypassing it — so a sub-issue finished by its agent never posted the parent system comment and never woke the parent assignee ("必须人工对话拉取"). Live evidence: JYF-352 got ZERO system comments when children JYF-353/354 completed via the daemon path (12:12/12:17, all guards pass). Fix: after the `in_review`/`todo`→`done` flip, re-read the issue and call `notifyParentOfChildDone` exactly like the UpdateIssue path (stage-barrier/backlog/idempotency/anti-loop guards unchanged). Regression test `TestCompleteTask_ChildDone_NotifiesParent` pins child→done + parent system comment + parent-assignee wake task. **Same gap exists upstream** (`notifyParentOfChildDone` only on `issue.go` + `github.go` there) — fork-local fix. Known boundary: mythos lab auto-completion (`service/mythos/{runner,supervise}.go` direct-SQL flips) has the same gap; service layer can't call handler methods (rare combo, documented). Verification: typecheck 6/6 + handler suite 16.1s + go test (except documented pre-existing mythos panic). Ship chain PASS, cold start ~2s, Server PID 38136. Release note `.omc/release-notes-0.5.38.md`, ship log `.omc/0.5.38-ship-2026-08-19.md`.
> Deferred: MUL-6286 (actor properties — port WITH MUL-6305 fail-closed migration), MUL-5651 (port MUL-4257/4302/4304/4351 first), MUL-6321 OpenClaw slow hosts, MUL-6063 delegated task failures, MUL-5991 (jcode), 0c69f1f95 (Hermes resume-auth), MUL-6350 plugin rebuild 1/4 (watch series), upstream new-commit scan (last: `9d6c0c81e` + 46 commits through v0.4.29), **MUL-6323** (worktree-gate blame redirect — agent died 429 mid-port leaving 23 files +1819 LOC / 2 wholesale files / 6 typecheck errors; reverted to HEAD. Retry = fresh session + quota + small-chunk pattern: Go batch → TS batch → main-thread commit), mythos child-done notification (service-layer gap, see 0.5.38), plus 2 SKIPs:
...
>
> **0.5.37 (committed + shipped 2026-08-19, installed — superseded by 0.5.38).**
> Client-log audit close-out (9-issue audit → 4 fixes, full report at `.omc/audit/2026-08-19-client-log-audit.md`):
> 1. `fix(invitations)` `e5e0c8b40` — dead user-scoped invitation code path removed (10 files +46/-179): 4 client API wrappers + sidebar dropdown/badge/mutations + queries key + WS handlers + events types + App/navigation/overlay/pageview references. Was producing **166 GET /api/invitations 404 WRN/day** from the v0.5.36 client (server routes were removed earlier, client wrappers weren't). Audit lesson: an Explore agent claimed "fork renderer doesn't call /api/invitations" — wrong, it had only grepped `apps/desktop/src` + `packages/views/`, missing `packages/core/api/client.ts:1750-1764`. **"X not called" verdicts must grep `packages/` + `apps/` together.**
> 2. `fix(desktop)` `ef4484fa5` — B: production renderer console → `~/.multica/profiles/<active>/renderer.log` via new `apps/desktop/src/main/renderer-log.ts` (path pinned in `ensureActiveProfile`); C: `startDaemon` wrapped in `startInFlight` single-flight guard (was 10 EADDRINUSE/13h).
> 3. `feat(issues)` `572eeaba5` — **MUL-6140 scroll-to-newly-posted-comment** (upstream `8d9766d4b`, 5 files +125/-14, ratios ≤1.6x). Previously skipped for missing MUL-5181; MUL-5181 is now in fork. 2 deviations: no `warnUnhandledTriggers` (MUL-4525 §2 not in fork), no `find.open` (in-page find absent).
> 4. `fix(desktop)` `ef5a7f615` — **50MB log rotation** (server.log + daemon.log, POSIX-atomic rename to `.1` at spawn). 23:23 daemon.log "truncation" mystery root-caused: one-time manual `mv` at 0.5.13 ship, NOT a code bug.
> 5. `docs(audit)` `55114e921` — report + 3 port SKIPs (MUL-6327 mcode logo SKIP-DEAD-CASE, MUL-6335 skills bulk SKIP-NO-ENDPOINT, MUL-6323 agent-fail-429 → reverted wholesale state, re-deferred).
> **Verification**: `pnpm typecheck` 6/6 + `go build ./...` clean + `go test ./internal/... ./pkg/agent/...` (except documented pre-existing mythos panic) + vitest app-sidebar 4/4 + daemon-manager 8/8. Ship chain 7/7 PASS, cold start ~4s, Server PID 98708. Release note `.omc/release-notes-0.5.37.md`, ship log `.omc/0.5.37-ship-2026-08-19.md`. Memory `0.5.36-client-log-audit-2026-08-19.md`.
> Deferred: MUL-6286 (actor properties — port WITH MUL-6305 fail-closed migration), MUL-5651 (port MUL-4257/4302/4304/4351 first), MUL-6321 OpenClaw slow hosts, MUL-6063 delegated task failures, MUL-5991 (jcode), 0c69f1f95 (Hermes resume-auth), MUL-6350 plugin rebuild 1/4 (watch series), upstream new-commit scan (last: `9d6c0c81e` + 46 commits through v0.4.29), **MUL-6323** (worktree-gate blame redirect — agent died 429 mid-port leaving 23 files +1819 LOC / 2 wholesale files / 6 typecheck errors; reverted to HEAD. Retry = fresh session + quota + small-chunk pattern: Go batch → TS batch → main-thread commit), plus 2 SKIPs:
>   - **MUL-6327 mcode provider logo** (`b55322d938f2`) — SKIP-DEAD-CASE: fork has no `mcode` runtime in `packages/core/types/agent.ts`; the `ProviderLogo` switch enumerates 13 cases (claude/codebuddy/codex/opencode/openclaw/hermes/pi/copilot/cursor/kimi/kiro/qoder/antigravity) and never produces `provider="mcode"`. Port prerequisite: add `mcode` runtime first (~50+ LOC for runtime type + IPC + daemon spawn + picker integration).
>   - **MUL-6335 skills bulk update** (`2014ae3613bb`) — SKIP-NO-ENDPOINT: fork lacks `POST /api/skills/{id}/refresh` (the single-skill refresh prerequisite from upstream PR `147cd8d84` was never ported — that PR adds a new SQL column + cache + schema). Bulk-update UI would 404 on every refresh, breaking the UX contract documented in upstream's commit message. Port prerequisite: port `147cd8d84` as a separate audit (~3k LOC Go + SQL).
>
> **0.5.35 (committed 2026-08-18, shipped + installed at /Applications/Multica.app — superseded by 0.5.36).**
> Surgical upstream cherry-pick batch — 4 atomic commits (`9b9b588cf` / `e2b8a6852` / `0ef9b9381` / `781cea00c`) for a combined ~370 LOC. Process validation: parallel agent pattern (TS batch + Go batch + MUL-6233 audit) + main-thread commit step kept the work under the thrash ceiling (per 0.5.33 lesson). Ship chain 7/7 PASS, cold-start 6s, Server PID 64572.
> **MUL-6233 RELAND audit verdict: RELAND-UNSAFE → SKIP.** Upstream `1b6d42864` is byte-identical to `4effd07ad` (the commit we reverted in 0.5.32). Upstream's "fix" was a separate MUL-6293 tab-host/workspace-context commit that is NOT in the fork. Both original defect vectors still live in the reland code (`keyboard-shortcuts.ts:91-101` no `isComposing` guard; `","` still in PRIMARY_RESERVED_KEYS at `definitions.ts:288`). Re-landing requires a 3-commit atomic series: MUL-6293 port + MUL-6233 cherry-pick + missing safety guards.
> **6 upstream cherry-picks skipped** due to structural divergence / thrash threshold (carried to 0.5.36+):
> 1. `8d9766d4b` MUL-6140 (depends on MUL-5181 14-file refactor — not in fork)
> 2. `5f4ba41d1` MUL-6323 (28 files TS+Go; `local-directory-mode-dialog.*` deleted in fork)
> 3. `f320af4a5` MUL-6333 (`agent-transcript-dialog.test.tsx` deleted in fork 0.3.33 inventory)
> 4. `4d5a679b9` MUL-6321 OpenClaw slow hosts (1610 LOC / 29 files / 3 files deleted)
> 5. `afd05ca22` MUL-6063 delegated task failures (2616 LOC / 18 files)
> **MUL-6243 UI close-out deferred to 0.5.36** — `e3a40b0b8` + `14c2e4e83` = 148 files / +6588/-559 (7147 LOC). 2x the 0.5.33 backend port that thrashed 4 times. Fork-side diff will be smaller because 0.5.33-34 already shipped `packages/core/{types,api,api/schemas}` + `server/internal/issuestatus/` — but the UI half (`packages/views/issues/**`, `packages/views/settings/components/issue-statuses-tab.tsx` +51 keys × 4 locales, status-picker rewrite, board fetch-by-category) is still full-fat and needs a dedicated session with main-thread commit pattern.
> **2 fork deviations in MUL-6303** worth a future code-reviewer pass: (a) dropped upstream's `isWorkspaceDeletePending` guard (fork has no `@multica/core/workspace/pending-delete`; equivalent via `useDeleteWorkspace` + tab-store); (b) module-level `singletonOwner: symbol` is anti-pattern — safe in vitest practice but fragile if test isolation breaks.
> **Process note for 0.5.35**: parallel agents did reading+editing (no commits), main thread did `git add`+`git commit`+`git cherry-pick`+`ship-mac`. Verified the 0.5.34-baseline pre-existing `TestTickSupervision_CompletionByFinalIssueStatus` panic (byte-identical stack from `db.New(nil)` returning non-nil struct with nil inner DBTX) — NOT caused by this port; documented in Known Stability Surfaces below. Release note `.omc/release-notes-0.5.35.md`. Memory `0.5.35-cherry-pick-batch-2026-08-18.md`.
> Deferred: MUL-6243 UI close-out (see above), MUL-6286 (actor/multi_actor properties — needs `@multica/core/properties` base, can ship alongside MUL-6243 UI), MUL-5991 (jcode), 0c69f1f95 (Hermes resume-auth), upstream new-commit scan (last: `9d6c0c81e` + 24 commits, 2026-08-18), 6 cherry-picks skipped this cycle.
>
> **0.5.34 (committed 2026-08-18, shipped + installed at /Applications/Multica.app — superseded by 0.5.35).**
> MUL-6243 per-workspace custom issue statuses is **end-to-end shippable**. Closes the deferred frontend half from 0.5.33. 4 atomic commits:
...
> 3. `fix(handlers)` `6fdb57def` — 6 Go consumers normalized (`handler/issue.go` write-path with `resolveIssueStatusKey` + `runWithIssueStatusGuard` + 3-way archive-race guard; `issue_child_done.go` stage barrier; `daemon.go` GC + CompleteTask flip; `github.go`+`vcs_webhook.go` PR terminal; `workspace.go` seed-on-create; `service/issue.go` Create guard; `service/issue_trigger.go` WillEnqueueRun; `service/task.go` HandleFailedTasks; `service/autopilot.go` SyncRunFromIssue; `cmd/server/{autopilot_listeners,notification_listeners,router,runtime_sweeper}.go`). **2 fork-local deviations** (user-approved): `mythos/supervise.go::isTerminalIssueStatus` + `handler/daemon.go:2587` CompleteTask flip both route through `Effective()` so custom done/review statuses don't get stuck.
> 4. `test` `aafef2d82` — MUL-6291: 122 pure-logic test files annotated `// @vitest-environment node` + 3 companion `setup.ts` guard wraps.
> **Flag default OFF**. Enable with `FF_CUSTOM_ISSUE_STATUSES=true` (`launchctl setenv`). Backend is internally consistent without TS/CLI; explicit API calls work server-side. The new tables/migrations exist and are seeded for all 7 existing workspaces (per migration 257). No existing behavior changes for users without the flag.
> **Deferred to 0.5.34** (MUL-6243 frontend): TS `packages/core/types/issue-status.ts` + `schemas.ts` `IssueStatusEntrySchema` (category as plain string + `.loose()` + `IssueSchema.status_category` optional) + `client.ts` 4 methods; CLI `cmd_issue.go::validateIssueStatus` format-only + `cmd_issue_test.go` flip + `cmd_lab.go` status flag; SKILL.md + source-map; adapted upstream tests (`issuestatus_test.go` 311 + `issue_status_test.go` 957 + `issue_child_done_stage_test.go` + `Effective` pins for the 2 fork deviations + workspace-delete orphans assertion).
> Verification: `go build ./...` 0 + `go test ./internal/issuestatus/` ok + `go test -run "TestIssue|TestIssueChild|TestIssueStatus"` ok + migration 250–258 applied + ship chain 4a/7 PASS + cold-start PASS, server 0.5.33. Pre-existing `TestDashboardPerAgentRollupsUseExactWindow` flake (date-boundary, dashboard_test.go:1520-1531) verified to fail identically on main HEAD — NOT caused by this port.
> Process note: **4 of 6 port agents this session hit autocompact thrash on the 3500-LOC MUL-6243 port**. Recovery: when the 4th agent died with 17 files modified uncommitted, the main thread inspected the diff, verified build+tests, committed the diff as a single atomic commit, and cherry-picked to main. **Lesson: for ports > ~2000 LOC or with >10 file edits, the main thread does the final `git add` + `git commit` + `ship-mac` chain; agents do the reading + editing in small chunks.** Release note `.omc/release-notes-0.5.33.md`. Memory `0.5.33-mul6243-backend-mul6291-2026-08-18.md`.
>
> **0.5.32 (committed 2026-08-17, shipped + installed at /Applications/Multica.app).**
> Upstream alignment + dormant-hazard fix + test coverage closure. 4 work-streams:
> 1. `revert(desktop)` `5efa66945` — **MUL-6233 Cmd/Ctrl+, settings shortcut removed**, mirroring upstream revert `2829798b5` (upstream shipped 2026-08-16, reverted 1.5 days later; reason undocumented). **User decision REVERT-ALSO**: two defect vectors were byte-identical in the fork — global main-process comma-chord interception in every focus context incl. editors (no isComposing guard), and `","` in PRIMARY_RESERVED_KEYS dropping user shortcut overrides. 14 files reverted; `goSettings` kept with `defaultShortcut: null`. Lesson: an upstream revert with an empty PR body still carries a strong product signal — when the fork's core code is byte-identical, default is REVERT-ALSO.
> 2. `fix(migrate)` `967a991c1` — **MUL-6288 concurrent index cleanup**: the fork had 20+ `CREATE INDEX CONCURRENTLY` migrations with NO cleanup registry — torn build → INVALID index → `IF NOT EXISTS` marks applied → uniqueness silently unenforced. Total 24-entry `concurrentIndexCleanups` registry (fork-true basenames built from actual fork `.up.sql` content — fork numbering diverges from upstream after ~218) + auto-generated pre-migration hooks (`DROP INDEX CONCURRENTLY IF EXISTS` only when `indisvalid=false`) + 3 tests incl. live repair on migration 248.
> 3. **MUL-6254 test port** — 8 commits (`e217d6790`…`95487d3d2`): 6 of 7 deferred test files landed (21 tests green), 1 dropped (onboarding-flow-mode — fork has no mode props). **2 production bugs exposed by the tests and fixed first**: `fix(core)` `d5d860b11` (MUL-4985 — `useActorName` unstable empty-array default → stable, prevents infinite re-render); `fix(web)` `b4513c0ac` (MUL-6254 selector contract — query error leaves data `undefined` (v5 select doesn't run on error), fork layout treated it as authoritative null → NoAccessPage; now keeps loading veil).
> Verification: `pnpm typecheck --force` 6/6 (35.6s) + `go test ./cmd/migrate/` ok (MUL-6288) + 21 vitest green + 4a/7 integrity check PASS → cold-start PASS, server 0.5.32. Release note `.omc/release-notes-0.5.32.md`. Memory `0.5.32-mul6233-revert-mul6288-2026-08-17.md`.
> Deferred: MUL-6243 (per-workspace custom issue statuses — PORT-AFTER, needs migration renumbering 250–257 + consumer audit of labs/swarm/self-opt status paths), MUL-6286 (actor properties — needs properties base), MUL-6291 (jsdom removal — mechanical), MUL-5991/0c69f1f95 (user-deferred).
>
> **0.5.31 (committed 2026-08-17, shipped + installed at /Applications/Multica.app).**
> Retention-ladder audit closure + deferred ports. 4 atomic commits:
> 1. `feat(experimental)` `993aa72e9` — **AuthTokenGC**: unified 6h GC for the 3 dormant `expires_at` ladders (`task_token` 108 / `workspace_invitation` 041 / `daemon_token` 029). Same bug class as 0.5.25 RuntimeGC: the `DeleteExpiredTaskTokens` + `DeleteExpiredDaemonTokens` queries ALREADY existed with ZERO callers; `DeleteExpiredWorkspaceInvitations` added. Mirrors the 3-contract (Start at boot / Stop in main.go shutdown / `sweepCount atomic.Uint64` + `TestAuthTokenGC_RunSweepsBeforeExit`). 15s per-table sub-context, nil-Queries test guard. Zero migrations.
> 2. `chore(migrations)` `641e785ed` + `fix(migrations)` `7a564dd3b` — migration **249** TRUNCATE `verification_code` + `personal_access_token` (no insert path in fork; 410 Gone contract). Table-name singular trap: migration 011 creates `personal_access_token` (SINGULAR), audit wrote plural → first apply failed 42P01 → corrected + re-applied (both tables 0 rows verified).
> 3. `fix(server)` `b50166858` — **pgProbeUrl loopback-only guard** (deferred 3.3 half): DATABASE_URL accepted only for localhost/127.0.0.1/::1; foreign host falls back to loopback default (value is persisted verbatim to profile `.env` at first launch).
> 4. `fix(desktop)` `da0edc327` (stopDaemon `env: desktopSpawnEnv()` symmetry, 3.2 P2) + `chore(canvas)` `471edadb5` (code-canvas run.sh PORT invariant comment, 3.4 P3).
> Verification: `pnpm typecheck --force` 6/6 (35s) + `go build` 0 + `go test ./internal/experimental/` ok + migration 249 applied. Ship chain: 4a/7 integrity check PASS (714 bytes, '<') → cold-start PASS, server 0.5.31, daemon `--server-url http://localhost:8090`. Release note `.omc/release-notes-0.5.31.md`. Memory `0.5.31-auth-token-gc-2026-08-17.md` (contains the reusable retention-audit pattern: grep migrations → check GC callers → look for "DELETE query exists but zero callers").
>
> **0.5.30 (committed 2026-08-17 — Semantica retention GC + decision-record schema).**
> Closes the loop on the 0.5.28 → 0.5.29 deferred items (P1-3 provenance sweep + P1-2 DecisionRecord schema drift). 1 atomic commit `346498b9e` + ship log `7c476a9e2`. Files: 8 +426/-2.
> 1. `feat(experimental)` `346498b9e` — SemanticaGC background loop wired alongside `RuntimeGC` + `swarm_gc`. Ticks every 24h, deletes `.provenance` rows > 90d, rotates `semantica-graph.json.api-key` on first-ensure-up after stale. New `SemanticaGC` struct in `server/internal/experimental/semantica_gc.go` + `Start()` in `cmd/server/router.go` parallel to swarm_gc + `Stop()` in `main.go` shutdown. Mirrors the 0.5.25 RuntimeGC fix shape (no eager close, Start() wired at boot, Stop() in shutdown). Tested at `runtime_gc_test.go`-style harness with `>=2` sweep invocations in 60ms at 20ms Interval — fails on pre-fix code, passes with fix.
> 2. `feat(core)` — `SemanticaDecisionRecordSchema` zod in `packages/core/api/schemas.ts` (typed `DecisionRecordV1` envelope: `kind` ∈ `preference|correction|fork` + `confidence` 0-1 + `at` ISO-8601 + `source` ref). Pins the wire shape decision_sync would have produced; future writes MUST pass `DecisionRecordV1.parse(...)` before persisting. Closes the R6 INCONCLUSIVE P1-2 verdict.
> 3. `docs(skills)` — `multica-semantica-decision-advisor/references/api-source-map.md` gains a DecisionRecord section documenting the SHACL/SKOS shapes used by semantica's `decisions.py` validator (closes the audit's doc gap; SHACL node-shape URIs + SKOS concept-scheme mapping both referenced, sourced from semantica upstream's `schema/` dir).
> 4. `chore(experimental)` — `decision_sync.go:26` stale comment referencing `vendor/semantica/decisions.py` removed (R6 confirmed dead reference — only `decider.py` + `engine.py` exist in vendored path; no `decisions.py` ever shipped).
> Verification: `pnpm typecheck --force` 6/6 + `go test -count=1 -timeout 600s ./internal/... ./pkg/agent/...` all ok (0 fail) + `go build ./...` exit 0. Zero migrations on top of 0.5.29 (SemanticaGC operates on existing `.provenance` files + the in-memory `Handler.ExperimentalFlagAPIKeys` map; no new SQL tables). Ship chain: snapshot → migrate (no new) → bundle-cli (no new resources) → electron-vite (no renderer change) → **4a/7 renderer integrity check PASS** (`index.html 714 bytes, starts with '<'`) → electron-builder `--dir` → install + re-sign (nested `multica` exit 0) → cold-start 6s, Server PID 48824, Info.plist 0.5.30. Release note `.omc/release-notes-0.5.30.md`, ship log `.omc/0.5.30-semantica-retention-2026-08-17.md`. Memory `0.5.30-stale-renderer-build-fix-2026-08-17.md`.
> **MUL-6254 (auth recovery UI, upstream PR #7058) — full port**: 6 atomic commits `08c0a2407` + `a390c9df7` + `20fe99050` + `a2d347c7b` + `68f682af1` + `565a049d8`. New `AuthStatus` union + 6-step retry ladder (~61s total). `loginWithGoogle` dropped per fork's username-only contract. New `apps/desktop/src/renderer/src/pages/auth-recovery.tsx` page + `apps/desktop/src/renderer/src/platform/auth-session-bridge.tsx` (orthogonal to existing `daemon-reauth.ts`; both kept). 4 locale keys `desktop.recovery.{title,description,retry,retrying}` × en/zh-Hans/ja/ko. Web routes migrated to shared `useWorkspaceList({ enabled })` hook. 7 upstream test files deferred (need fork-idiom fixtures).
> **Ship hardening `fix(ship)` `2f9edaf08` (0.5.30 patch)**: added 4a/7 renderer integrity check in `scripts/ship-mac.sh` (file exists + first byte is `<` + size in [200, 5000]). The 0.5.29 ship crashed silently in production because `grep rawRequest` (5a/7) only validated REPLACEMENT, not well-formedness — `out/renderer/index.html` had a highlight.js Zig module prepended before `<!doctype>`, Electron's main process exited within ~200ms with no error log, recovery was `cp -R` from the 0.5.28 pre-update snapshot. The 4a/7 check catches the same class of failure BEFORE packaging.
> **Carry-over fixes from 0.5.27/0.5.28**: `e4a69d314` (0.5.27 daemon `--server-url <targetApiBaseUrl>`) + `0ff8c64a4` (0.5.27 cold-launch `initTargetApiUrl` seed) + `7b5c3322f` (0.5.28 server PORT pin). All 3 verified live: `multica daemon start --foreground --server-url http://localhost:8090 --profile desktop-localhost-8090` is the running daemon's command line on the 0.5.30 install.
> **Runbook for E2E verification**: `.omc/semantica-d-smoke-runbook-0.5.30.md` (4 R4 deferred assertions + cold-launch P0-1 measurement; exit codes 0/1/2; per-assertion PASS/FAIL/DEFERRED output; pre-run checklist for `SEMANTICA_REPO_PATH` + `MULTICA_API_TOKEN` + dual-workspace setup).
> Next: 0.5.31 — re-run `scripts/semantica-e2e-smoke.sh` per the runbook to confirm the 4 R4 deferred assertions flip to PASS:
>   - (i) unproxied 401 — caller without auth-chain cookie sends `X-API-Key: deliberately-wrong` → 401
>   - (ii) wsId path-traversal rejected — `wsId='../foo'` → `WS_ID_REGEX` rejects + `console.warn` + spawn returns `null`
>   - (iii) two workspaces → two distinct managers — `find ~/.multica/workspaces -name semantica-graph.json | wc -l` ≥ 2
>   - (iv) `.provenance` not orphaned — `.provenance` lives at the workspace-scoped path, not the legacy global path
>   - Cold-launch P0-1 measurement: actual `READY_TIMEOUT_MS` vs 180000 ms target.
> Row parity unchanged. OR move to next dormant retention ladder audit (`agent_creation_studio` shipped 0.5.5 — any of its tables have orphaned `expires_at` columns with no working GC?). Lesson: when an audit defers items across versions, track each one in the next-cycle header — the R4 reinforcement pattern is now generalizable.
>
> **0.5.29 (committed 2026-08-17 — Semantica × Multica data localization).**
> Fork-local **data localization** for the semantica subprocess path — ships two R4 mitigations as one commit (`fix(data-localization)` `719debaef`) + ship log `bd048cd37`. Files: 12 +368/-103.
> 1. **P0-2 (workspace isolation)** — `subprocess-manager.ts` validates `wsId` against `WS_ID_REGEX` (`^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`) before any path interpolation; invalid wsId → `null` + `console.warn` (R4 P0-2a mitigation). Cache key is now per-`(flagKey, wsId)` (pre-0.5.29 the singleton was keyed by flagKey only — switching active workspace returned WS-A's cached manager, leaking process + graph.json + port + URL; R5b silent-correctness failure mode closed). `vendor/semantica/run.sh` derives `GRAPH_PATH` from `~/.multica/workspaces/<wsId>/semantica-graph.json`; legacy `~/.multica/semantica-graph.json` is `cp`'d into the new path on first boot (one-shot migration, original left as backup). Python heredoc reads `wsId` from `WS_ID_FOR_PY` env var (not string-interpolation) — R4 P0-2c RCE mitigation; the regex blocks `../foo` and `'; rm -rf /` payloads at the boundary; the script is defense-in-depth.
> 2. **P1-1 (X-API-Key INSIDE auth chain)** — `MountExperimentalProxies` moved **inside** the `r.Group(middleware.Auth(...))` block in `router.go`. iframe stays same-origin so cookies auto-attach; `middleware.Auth` is transparent to the renderer (no visible change). `reverseProxyTo.Director` **unconditionally** runs `req.Header.Del("X-API-Key")` before any conditional `Set` (R4 TOP-1 mitigation — a conditional `if apiKey != ""` plus no `Del` would forward caller-supplied keys verbatim, turning the proxy into an unauthenticated credential oracle). Key transport is **in-memory** via `upstreamRegister` IPC — pre-0.5.29's `$GRAPH_PATH.api-key` (0600) file is gone (F-013-class surface reachable by any user-plugin `python3 -I` child). Per-launch 32-byte hex key via `node:crypto.randomBytes`; threaded through `childEnv.SEMANTICA_API_KEY` AND through IPC body; server stores in `Handler.ExperimentalFlagAPIKeys` (sync.RWMutex — held across single map access only, NOT across the proxy call).
> Verification: `pnpm typecheck` (full turbo, 6/6) clean; `go build ./...` clean; `go test ./internal/handler/ ./internal/daemon/ ./internal/experimental/ ./pkg/agent/` all 4 packages pass (handler 0.9s + daemon 25s + experimental 0.7s + pkg/agent 13s); `subprocess-manager.test.ts` 5/5 with `WS_ID` threading; `run.sh bash -n` clean; `run.sh` wsId + API-key validation: 4 inline cases all REJECTED (empty / invalid UUID / path traversal `../etc/passwd` / missing key when WS_ID valid).
> **Deliberation transcript**: `~/.omc/mythos/semantica-integration-path/round-{0..7}-*.md` (8 files, ~700 lines), including R4's TOP-1 attack surface + R5's "ship-as-written = net-negative" verdict + synthesizer minority report.
> **Deferred to 0.5.30**: P1-3 GC (24h tick + 90d provenance sweep + key rotation) + P1-2 schema drift (api-source-map DecisionRecord section + zod add + dead-ref cleanup). **Lesson**: in-memory transport > file transport for secrets — the F-013-class per-launch file at 0600 reachable by any `python3 -I` child is closed by making the desktop main process the key owner; a future hardening pass should consider whether the subprocess even needs the key in env (could derive from a per-launch IPC round-trip), but that's a 0.5.31+ concern.
>
> **0.5.28 (committed 2026-08-17 — Semantica × Multica patch).**
> Semantica × Multica integration audit surfaced 5 implementation defects + 3 vector-model decisions + 1 merge-tracking gap. 7-round mythos deliberation (chair → planner → steelman → architect → security-reviewer → devil-advocate → verifier → synthesizer) reached consensus: ship 4 audit items in 0.5.28, defer the rest to 0.5.29 with R4 reinforcements as foundation (not afterthought). 4 atomic commits on top of `9ae934e29`:
> 1. `fix(daemon)` `4a0426330` — **P0-1 cold-start ready_timeout_ms 120s clamp**. `subprocess-manager.ts::ensureUp` reads `READY_TIMEOUT_MS` from manifest (default 180000 ms = 180 s, clamped at 120000 ms / 120 s on this hardware — cold-start genuinely exceeds 180 s on cold-cache runs); `daemon.go::spawnAgent` matches. All other `subprocess` Labs (`code_canvas`, `llm_wiki_bridge`, etc.) consume the same field → automatic benefit (6/6 typecheck confirms no other-flag regression). Verified at `pnpm typecheck` 6/6 + `go test ./internal/daemon/` ok 24s.
> 2. `chore(scripts)` `531a8beef` — **P1-3-half graph backup**. `scripts/snapshot-semantica-graph.sh` captures `~/.multica/semantica-graph.json` + `*.provenance` (excludes `*.api-key` + dotfiles + `.git`); tarball written to `.omc/backups/semantica-graph-<ts>.tar.gz`. Smoke-tested with 4 fake files + 2 fake api-key files → tarball contains 4 (graph + provenance), excludes both api-key entries. Manual wire-in required for `pre-update-snapshot.sh` (release note callout).
> 3. `feat(scripts)` `a6fc41fe2` — **D E2E smoke**. `scripts/semantica-e2e-smoke.sh` (201 LoC) with 5 current-cycle assertions + 4 R4 reinforcement deferred assertions (lines 165-191 — unproxied 401 / wsId path-traversal rejected / two-workspace distinct managers / `.provenance` not orphaned). Per-assertion PASS/FAIL/DEFERRED output, exit codes 0/1/2. User-owned, run-on-demand with `SEMANTICA_REPO_PATH` + `MULTICA_API_TOKEN`.
> 4. `docs(skills)` `9c35d2d76` — **B api-source-map SHACL/SKOS fill**. `multica-semantica-decision-advisor/references/api-source-map.md` gains the SHACL node-shape URI + SKOS concept-scheme mapping for `decider.py` decisions (was a 1-line `TBD` placeholder; R5's "ship-as-written = net-negative" hit this gap as part of the verdict). 8 lines of reference doc added.
> Verification: `pnpm typecheck --force` 6/6 (0 cached) + `go test -count=1 -timeout 60s ./server/internal/daemon/` ok 24.076s + `go test -count=3 -run TestRuntimeGC_RunSweepsBeforeExit ./server/internal/experimental/` stable (3/3) + `bash -n` on both new scripts clean. Zero migrations, zero Go binary changes, zero renderer rebuild required. Ship log `.omc/0.5.28-semantica-patch-2026-08-17.md`. Release note `.omc/release-notes-0.5.28-semantica-patch.md`. Deliberation transcript at `~/.omc/mythos/semantica-integration-path/round-{0..7}-*.md` (8 files, ~700 lines).
> **Lessons**: (1) audit-then-decide — without R4's security pass, A would have shipped and reopened F-005 via P0-1 + F-013 class via P1-1; (2) architect's file paths are aspirational — R2 proposed `server/internal/semantica/` (does not exist); R3 rerouted 2 of 5 paths before any commit landed; (3) verifier evidence > verifier opinion — R6 INCONCLUSIVE on P0-1 was resolved by routing through the D script instead of deferring.
>
> **0.5.28 (committed 2026-08-17, shipped + installed at /Applications/Multica.app — superseded by 0.5.29/0.5.30).**
> Urgent PORT/URL env-leak patch on top of 0.5.27 — completes the daemon-port-env-leak fix that bit the user the day of 0.5.26/0.5.27 ship. 3 atomic commits:
> 1. `fix(desktop)` `e4a69d314` — `daemon-manager.ts::startDaemon` appends `--server-url <targetApiBaseUrl>` (F-027-allowlisted URL from `desktop.json::apiUrl`) so the daemon's URL resolution no longer falls back to `ws://localhost:8080/ws` (DefaultServerURL) when the launching shell leaks `PORT=8080`. On this machine :8080 hosts SearXNG, so the GUI daemon crashed in 1s and every `agent_task_queue` row stayed `queued`. **Now superseded by commits 2+3 — see below.**
> 2. `fix(desktop)` `0ff8c64a4` — the 0.5.27 fix was conditional (`if (targetApiBaseUrl)`), but `targetApiBaseUrl` is set ONLY by the renderer IPC `daemon:set-target-api-url`, which has not fired yet on the main-process auto-start path (`bootstrapCli` → `tryAutoStartFromMain`, fired at `app.whenReady()`). New `initTargetApiUrl(url)` export (F-027-allowlisted, no clobber of renderer value) seeded from `desktop.json::apiUrl` at `apps/desktop/src/main/index.ts:706` immediately before `setupDaemonManager(...)`. Cold-launch with `prefs.autoStart: true` now uses the canonical URL from boot.
> 3. `fix(server)` `7b5c3322f` — `server-manager.ts::buildServerEnv` parsed `~/.multica/profiles/<p>/.env` without reconciling the `PORT` line against the freshly-derived port. A missing or stale `PORT` would let the spawned server bind Go's `"8080"` default (`server/cmd/server/main.go:176`) while the GUI polls 8090. After `parseEnvFile`, `env.PORT` is pinned to `String(port)` whenever the file existed. Fails loudly on its own (timeout, not silent queueing) but same root pattern as the daemon bug.
> Verification: `pnpm typecheck --force` 6/6 (0 cached, 38s — all 3 fixes) + ship chain: snapshot → migrate (no new) → bundle → electron-vite → electron-builder `--dir` (no deadlock for the 2nd time in 90 min) → install + re-sign (nested `multica` exit 0) → cold-start 6s, Server PID 15979, Info.plist 0.5.28. Release note `.omc/release-notes-0.5.28.md`, ship log `.omc/0.5.28-ship-2026-08-17.md`. Memory files: `0.5.27-daemon-port-env-leak-fix-2026-08-17.md`, `0.5.28-port-leak-residual-fix-2026-08-17.md`. Analyst's full audit: `a5d065559e586c24f`.
> Deferred to 0.5.29+: stopDaemon env symmetry (P2); DATABASE_URL first-launch leak in `pgProbeUrl` (P1, scoped by P0 `runMigrate` refusal); `vendor/code-canvas/run.sh` hardening (P3).
>
> **0.5.27 (committed 2026-08-17, shipped + installed at /Applications/Multica.app — superseded by 0.5.28).**
> Carried the partial daemon-port-env-leak fix `e4a69d314` (only correct for the renderer-driven spawn path). Superseded 90 minutes later by 0.5.28 once the residual cold-launch path was found.
>
> **0.5.26 (committed 2026-08-17, shipped + installed at /Applications/Multica.app).**
> Upstream integration wave 3 — 7 more upstream commits via parallel expert agents (29 total), Wave 2 fully resolved. Key items:
> 1. `feat(daemon)` `f21e8ce1f` + `chore(migrations)` `fac55a959` + `feat(metrics)` `97f25ec93` + `test(server)` `4d44357ce` (MUL-6107, upstream #6894) — the `agent_runtime` stale-GC was a single bulk `DELETE` that destroyed `agent_task_queue`/`task_message`/`task_usage`/`task_token` via the ON DELETE CASCADE FK. Replaced with a bounded per-runtime GC: lock `FOR UPDATE`, re-check eligibility, **block deletion when any non-terminal task exists**, **detach terminal task history** (`runtime_id = NULL`). Migration **247** (`agent_task_queue.runtime_id DROP NOT NULL` + `NOT VALID` CHECK — upstream 251 minimal-scope half; fork's NOT NULL column would have made `UnbindTasksFromRuntime` fail) + **248** (`idx_agent_runtime_id`, upstream 309). 4 `runtime_gc_*` collectors + 473-line regression suite (6/6, incl. keeps-terminal-history). `DeleteStaleOfflineRuntimes` gone, zero callers.
> 2. `fix(handlers)` `65eb8a0d8` + `fix(views)` `99c64bca2` (MUL-6126, upstream #6905) — private runtimes owner-only in API + CLI: `canUseRuntimeForAgent` dropped the workspace-admin override, new `canSetRuntimeVisibility` (owner-only, PATCH-as-PUT no-op tolerance), ownerless-runtime refusal (MUL-3292 token-minting); 403 strings at all 3 call sites; new `core/runtimes/access.ts` + owner-only visibility toggle + 8 locale files.
> 3. `fix(views)` `d1e774b61` (MUL-5979, upstream #6696) — defensive `hasMoreActions` kebab guard (fork was already structurally immune).
> **Wave 2 fully resolved** — remaining UI/UX candidates all verified N/A or structurally aborted with file-level evidence (browser tab names / page titles / tab-bar cluster / sidebar triggers / prompt-compress / MUL-6164 wiring).
> Verification: `pnpm typecheck --force` 6/6 (0 cached, 34s) + `go test -count=1 -timeout 600s ./internal/... ./pkg/agent/...` all ok (0 fail) + `go build ./...` exit 0. Migrations 247/248 live-applied pre-ship. Ship chain: snapshot → migrate up → bundle-cli (0.5.26 binaries, 471 migrations) → electron-vite (asar 155 rawRequest) → electron-builder `--dir` (no deadlock) → install + re-sign (nested `multica` exit 0, not 137) → cold-start 4s, Server PID 13717, Info.plist 0.5.26. Release note `.omc/release-notes-0.5.26.md`, ship log `.omc/0.5.26-ship-2026-08-17.md`.
>
> **0.5.25 (committed 2026-08-17, shipped + installed at /Applications/Multica.app).**
> Experimental runtime GC bug actually fixed — was dormant for 8 versions. 3 atomic commits on top of 0.5.24 baseline:
> 1. `fix(experimental)` `3592725d4` — `runtime_gc.go::Run()` had an eager `g.stopOne.Do(close(g.stopped))` that fired before the for-select loop (the GC exited without ever ticking). Dropped. Plus `RuntimeGC.Start()` was never wired at boot (router.go's swarm_gc comment said "parallel to runtime_gc.Start pattern" but the runtime_gc side was orphaned since 0.3.33) — wired now alongside swarm_gc, stored on `Handler.RuntimeGC`, `Stop()` in `main.go` shutdown. Added `sweepCount atomic.Uint64` + `TestRuntimeGC_RunSweepsBeforeExit` (asserts `>=2` sweeps in 60ms at 20ms Interval — FAILS on pre-fix code with "got 0 sweep invocations", PASSES with fix). Migration 151's 30/90/120-day retention ladder for `experimental_claude_runtime_session` now actually runs in production.
> 2. `fix(test)` `5fc80ea81` — `TestQuickCreateIssueParentTrustBoundary` was failing 4 subtests in the full handler sweep because `agent_test.go:1051` flips the shared `testRuntimeID` to `'offline'` and its `t.Cleanup` restore can land AFTER this test reads the runtime. New `Handler.RuntimeOnlineOverride *bool` test hook breaks the race deterministically — when non-nil, `isRuntimeOnline()` returns `*RuntimeOnlineOverride` without querying the DB. 5/5 full handler sweeps green (was 1/3 with the defensive-UPDATE attempt). The field is production-dead but documented inline.
> 3. `docs(root)` `b8b362184` — Known Stability Surfaces RuntimeGC entry rewritten to reflect that the 2026-07-28 audit's claim was aspirational (no fix commit landed in any version from 0.3.33 through 0.5.24); the lesson is to `git log --grep` when an audit says "fixed". Memory file `0.5.25-runtimegc-fix-2026-08-17.md` documents the full investigation.
> Verification: `pnpm typecheck` 6/6 + `go test -race -count=1 ./internal/... ./pkg/agent/...` all green (30 packages, 0 fail). `go build ./...` exit 0. Zero migrations on top of 0.5.24 (the GC operates on the existing `experimental_claude_runtime_session` table from migration 151). Ship chain: pre-update snapshot → `make ship-mac ARGS="--yes"` exit 0; renderer asar verify; nested binaries signed; cold-start 6s, Server PID 26194, Info.plist 0.5.24 → 0.5.25. Asar `runtime_gc started` log line confirmed at 10:11:44 — the new GC is running in production.
> Next: 0.5.26 (audit other dormant retention ladders — are there other `expires_at` columns with no working GC?) or new MUL fix batch.
>
> **0.5.24 (committed 2026-08-16, shipped + installed at /Applications/Multica.app).**
> Docs-only re-ship on top of 0.5.23 baseline. **0.5.23** (committed 2026-08-16, packaged 2026-08-16): MUL-3963 + MUL-4525 full port — 11 atomic commits on top of 0.5.22 baseline (`5d3cacd31` plan doc). Core: `feat(agents)` `86a51e417` (MUL-3963 invocation-permission schema, migration 245: `agent.invocation_permissions` JSONB + `agent_template_invocation_permissions` projection table); `feat(agents)` `1543dda98` (MUL-3963 core helpers + wire); `fix(handlers)` `5dd927517` (call-site wiring, view gate keys on `permission_mode`); `feat(comments)` `98ab672d4` (surface blocked @mention `trigger_outcomes`, MUL-4525 §2; upstream 51325eeab); `fix(comments)` `f0d7a1b91` (one outcome per explicit mention, upstream 7b03baa37); `fix(comments)` `7ae5f6f2f` (honest role + status in fan-out); `fix(comments)` `a0e8af0eb` (fail-closed active-task check); `fix(comments)` `de1adfc5c` (honest merge outcome). Plus: MUL-4525 §2 merge-with-originator-re-stamp (`b749ee8ef` + `c72a7453a` + `1ec401a9e` un-skip + `d8d73a9f5` cleanup), MUL-4010 (`7a28ea3bb` `permission_mode`/`invocation_targets` through template create), MUL-4063 (`49de9c47e` wake private-leader squad parent leader on child-done), MUL-4305 (`50dbf7d9b` keep originator on agent-created issues), Phase 3 frontend port (`fa1f2bd75` AccessPicker + canInvokeAgent client + 4 locales), Phase 4 i18n (`84be8440f` register `swarm` locale keys). **Plus 0.5.24 docs-only**: no code changes, just CLAUDE.md header sync + 0.5.22 entry de-staling. **Closes test skip**: `TestConsecutiveCommentsDifferentOriginatorsFullEnqueuePath` (the 0.5.21-deliberately-skipped gate) now passes. `pnpm typecheck --force` 6/6 (33.7s, cache bypassed); `go test -race -count=1 ./internal/handler/ ./internal/experimental/ ./internal/service/ ./cmd/multica/ ./pkg/agent/` all green (handler 16.7s + experimental 1.6s + service 2.4s + cmd/multica 2.5s + pkg/agent 15.6s); `go build ./...` exit 0. Zero migrations on top of 0.5.22 (migration 245 already shipped with 0.5.22 baseline). Ship chain: `make ship-mac --yes` exit 0; renderer asar verify 155 rawRequest occurrences; nested binaries signed; cold-start 4s, Server PID 99575, Info.plist 0.5.23 → 0.5.24. Next: 0.5.24 packaging (optional), or move to next fix batch.
>
> **0.5.22 (committed 2026-08-15, shipped + installed at /Applications/Multica.app).**
> claude_science_lab UX ship — three atomic commits on top of the 0.5.21 baseline:
> 1. `fix(daemon)` `b004204a8` — flip issue status to done when agent task completes. `CompleteTask` gate keyed on `task.Status == "done"` was dead code; `agent_task_queue.status` terminal enum is `completed` (not `done`, which is an `issue.status` value). 4-case table-driven regression test pins the transitions (in_review→done, todo→done, in_progress unchanged, done idempotent).
> 2. `feat(views)` `66238b619` — lab deliverable summary card in issue timeline. Labs that declare `hides_deliverable_in_issue_timeline` (currently `claude_science_lab`) previously rendered only an empty IssueLabsSection workbench link — the raw agent deliverable was filtered out, leaving the user with no idea the run finished or what the result was. New `LabDeliverableSummary` card shares the exact `["lab-output-panel", wsId, issueId, labSource]` query key with `LabOutputPanel` so a page rendering both surfaces issues exactly one context request. Status badge (completed/failed/cancelled/queued/running) + result_summary preview + jump link. imageSequence allowlist rewritten from blacklist to whitelist so the new `lab-summary` kind — which has no `entry` field — cannot crash the timeline by falling through to `item.entry.content`.
> 3. `feat(experimental)` `e7d5607e6` — **Active Contract #6: lab auto-dispatch opt-out** (0.5.22). Per-catalog `Flag.AutoDispatch *bool` lets a lab opt out of the 0.3.46 auto-dispatch contract. When `*AutoDispatch == false`, the leader-rewrite still applies (assignee becomes the lab leader so IssueLabsSection + the lab workbench header show the right agent) but the service-layer gates (`IssueService.maybeEnqueueOnAssign` + `IssueTriggerService.WillEnqueueRun`) short-circuit — no `agent_task_queue` row is created until the user explicitly clicks "Run research" on the lab workbench. `claude_science_lab` is the only opt-out lab today (research runs are 30+ minutes and the user prefers the lab to remain a passive holder of the issue until they explicitly trigger work). New `POST /api/experimental/claude-science/issues/{id}/run` endpoint — loadIssueForUser + lab_source check + assignee check + pending-task dedup (409) + direct `TaskService.EnqueueTaskForIssue` call. Three enforcement layers (CreateIssue + UpdateIssue + BatchUpdateIssues); `experimental.AutoDispatch(key)` helper. Tests: `TestAutoDispatchFlagBehavior` pins the helper semantics; three tests renamed/flipped to assert the opt-out side; new `TestUpdateIssueLabSourcePythiaAutoDispatchStillFires` proves the opt-out is per-catalog (pythia_oracle keeps the 0.3.46 auto-dispatch contract). 4 locales (en/zh-Hans/ja/ko)/claude-lab.json: 2 new keys (`run_research_button`, `dispatch_manually_hint`).
> Verification: `pnpm typecheck` 6/6; `go test -count=1 ./internal/handler/` `./internal/experimental/` `./internal/service/` all green (14.6s handler + 0.4s experimental + 1.2s service); gofmt clean on all edited files. Zero migrations. Shipped 2026-08-15 ahead of 0.5.23.
> Next: 0.5.23 (MUL-3963 + MUL-4525 port) — shipped 2026-08-16.
>
> **0.5.21 (committed 2026-08-15 — not packaged; ships with 0.5.22).**
> Agent→agent @mention reconcile (MUL-4304, port of upstream PR #5148 / commit `4db1abe11`) — 2 atomic commits on top of 0.5.20 baseline (`0f45a8149` cherry-pick 15 files +1327/-238 / `9d706a956` followup 4 files +86/-1)。Core: `reconcileCommentsOnCompletion` 现在也 replay agent-authored 注释,通过 `keepExplicitMentionTriggers` 只放过 `@agent`/`@squad` mention(anti-loop 不变)。Migration **240** 加 `agent_task_queue.originator_user_id` + `accountable_user_id`(关 167 latent gap — 167 引用但从未 ADD,backfill from issue.creator_id)。`enqueueCommentAgentTriggers` + `triggerTasksForComment` 多 1 参数 `originatorUserID` — internal plumbing,无 API change。1 test `TestConsecutiveCommentsDifferentOriginatorsFullEnqueuePath` `t.Skip`(需 MUL-4525 4 commits,deliberately not ported);3 core MUL-4304 tests 全过。`pnpm typecheck` 6/6 + `go test -race -count=1 ./internal/... ./pkg/agent/...` 全 ok(handler 15.4s + daemon 24.2s + pkg/agent 11.3s + service 1.7s)。Next: 0.5.22 MUL-4525 系列 port(4 commits,closes test skip + 加 DispatchStatus/AttributionForMergedComment) 或 sec-first 后续(8 fork-applicable HIGH 全 0.5.18 closed,剩余 0 待办)。
>
> **0.5.20 (packaged 2026-08-14, rolled into 0.5.21).**
> Sub-agent delegation interrupt fix (MUL-5799)。Root cause: macOS sandbox 测试路径超 AF_UNIX `sun_path` cap (104B macOS / 108B Linux),daemon child 进程被 SIGKILL 静默断,agent→agent delegation 失败。Fix: per-task private `TMPDIR` injection — `ensureTaskTempDir` 创建 `/tmp/multica-<uid>/task-<hash>` (FNV-64a hash of envRoot+taskID),`runTask` 注入 `TMPDIR`/`TMP`/`TEMP` 进 agentEnv。`MULTICA_AGENT_TEMP_BASE` env override 自托管。`isBlockedEnvKey` HOME case 加 `"TMPDIR", "TMP", "TEMP"`。3 atomic commits + 1 version bump:`daemon.go` 4 helpers + `runTask` wire-up + `workdir_race_test.go` 3 regression tests + bump。`go test -race ./internal/daemon/` 全 ok,cold start 2s。Zero migrations。Memory `0.5.20-sub-agent-tempdir-fix-2026-08-14.md`。
>
> **0.5.19 (committed 2026-08-14 — not packaged; shipped with 0.5.20).**
> 阶段 4 (LabOutputPanel + iframe auth proxy) per `.omc/plans/lab-output-panel-design.md` 全部落地，spec status → `implemented`. 4 atomic commits on top of 0.5.18 baseline (`a23d78d6f` panel readers 12 files / `baa1094de` code_canvas persistence 8 files + migration 239 / `e78096a7a` iframe auth proxy 8 files / `c7ed060cc` docs). M1–M5 全过：`pnpm typecheck` 6/6 + `go test -count=1 ./internal/... ./pkg/agent/...` 全 ok（0 fail，比 0.5.18 baseline 还干净）。零新增 migrations（migration 239 仅新增 `code_canvas_artifact` 表，forward-only additive）。**两处 spec 偏差**（先 verify 再动手，0.5.17 lesson 重演）：(1) **M3 实际前端-only** — `GET /api/experimental/pythia-oracle/forecast/issue/runs?issue_id=` 早已存在（migration 164 `pythia_forecast_run` + `forecast_issue.go::pythiaIssueForecastRuns`），spec 「需新增 read 端点」是过时信息，后端 0 改动；(2) **M4 面板形态**（用户定夺，非 spec 字面「只读历史」）—— 自带渲染输入 + 历史，新增 `POST|GET /api/experimental/code-canvas/issues/{issueId}/artifacts`（渲染 + 落库）让面板能写能读。**M5 iframe auth proxy 安全边界**（逐行审过，未回退）：HMAC `crypto/subtle.ConstantTimeCompare` + `crypto/rand` 32B secret + `sync.Once` 进程级（重启 = 自动失效）+ exp ≤ 5min；签名分支跳过 Bearer + `X-Content-Type-Options: nosniff` 内联；非签名分支保留 `Content-Disposition: attachment`（F-006 不回退）；iframe `sandbox="allow-scripts"` 永不加 `allow-same-origin`（opaque origin 不变）。Ship 原子性代价：spec 的 6 个 milestone → 实际 4 个 buildable commit（`lab-output-panel.tsx` 顶部 import 一次性引用 M1–M4 schema + `schemas.ts`/`router.go` 跨阶段共享，6 个中间 commit 会编译不过；4 是 max buildable）。Next: 阶段 5 sec-first (8 fork-applicable HIGH 中已剩 0 待办——F-005/F-006/F-013/F-002/F-008/F-027 已 0.5.18 落地，F-007/F-028 verified non-issue，下一步是 lab P1 收尾或 0.5.20 实验)。
>
> **0.5.18 (committed 2026-08-13 — not packaged; ships with 0.5.19).**
...
- **Catalog** (`server/internal/experimental/catalog.go`): `Flag` struct with `ManifestPath`, `RuntimeKind`, `ProxyPrefix`, `LoopbackService`, `Sidebar`. **8 flags registered** (current set: `chat_pin_ui`, `claude_science_lab`, `pythia_oracle`, `mythos_swarm`, `swarm_topology`, `llm_wiki_bridge`, `code_canvas`, `semantica`). `agent_self_optimization` and `agent_creation_studio` were promoted to product-level resources in 0.5.5/0.5.5.1 and deleted from the catalog in 0.5.6 (see Retired Features). `constitution_agent` was retired in 0.3.57 (see Retired Features). `claude_science` and `claude_science_runtime` were removed in 0.3.22 — see "Lab Consolidation" below.
