...
>
> **Current release: 0.5.92** (2026-08-31, shipped from `epic/0.5.72-followups`; `/Applications/Multica.app` = 0.5.92, cold-start PASS ~6s; ship log [`.omc/0.5.92-ship-2026-08-31.md`](.omc/0.5.92-ship-2026-08-31.md); release notes [`.omc/release-notes-0.5.92.md`](.omc/release-notes-0.5.92.md)). **Ships the MCP sync mirror + MCP call usage metric** (commits `3e4241501` WIP-align + `bffb023e8` feature 46 files + `e93d53c9b` bump; detail paragraph below): the in-server `mcpsync` worker mirrors `~/.claude.json::mcpServers` into `mcp_sync_server` (migration 285) with canonical-subtree-hash change detection, claim-time merges it beneath the agent's manual `mcp_config` for the claude provider (manual wins on collision; `--strict-mcp-config` makes the merged set the exact tool surface), the settings page gains a read-only MCP tab (`?tab=mcp-sync`, env/header values masked server-side, NO edit/delete route = "synced servers cannot be deleted from Multica"; source removals flip rows to `removed`), and the daemon counts `mcp__`-prefixed tool_use events into `agent_task_queue.mcp_calls` surfaced as the usage page's 5th KPI (`GET /api/dashboard/mcp-calls/daily`, completed_at-anchored, no hourly-rollup column). Verified: typecheck 6/6; FULL `go test -p 1 ./...` uncached with `DATABASE_URL`, 30 pkgs 0 FAIL (handler 24.7s = genuinely DB-backed); live smoke on the real config mirrored 15 servers with hash fast-path + manual-wins precedence; installed-app verification: worker self-started (`mcpsync: worker started` in server.log), mirror 15/15 synced, asar `mcp-sync` ×23 / `mcp_calls` ×25. **Ops lesson (one aborted ship)**: killing the app kills the bundled Postgres.app (:5432) — ship step-2 migrate needs it up, so SHIP WITH THE APP RUNNING; a gate run while the DB is down silently skips DB-backed packages (handler "ok" in ~1s = hollow) and must be re-run after the DB returns. Migration 285 is TAKEN by mcp_sync — the previously-planned sub-issue `hidden_at` migration is now 286+. The 0.5.91 dirty-tree warning is RESOLVED: the residue commit-aligned as `3e4241501` before this ship. Previous release **0.5.91** (2026-08-31): issue context-menu singleton refactor killing the app-wide freeze (detail below); ship log [`.omc/0.5.91-ship-2026-08-31.md`](.omc/0.5.91-ship-2026-08-31.md). The causal graph loop stays closed both ways — write ~90% since 0.5.83, read ~70% since 0.5.85. The 0.5.84 contracts (#8 trust ladder + #9 FLAG_ROUTE_SUFFIX + never-nag + TouchCausalNode callsite) all still hold.
>
