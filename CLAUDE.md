...
>
> **Current release: 0.5.91** (2026-08-31, shipped from `epic/0.5.72-followups`; `/Applications/Multica.app` = 0.5.91, cold-start PASS ~4s; ship log [`.omc/0.5.91-ship-2026-08-31.md`](.omc/0.5.91-ship-2026-08-31.md); release notes [`.omc/release-notes-0.5.91.md`](.omc/release-notes-0.5.91.md)). **Ships the issue context-menu singleton refactor that kills the app-wide freeze** (commits `4fc93130b` + `d43b1881d`; detail paragraph below): one Base UI ContextMenu root per surface (ListView/BoardView/SwimLaneView/GanttView) instead of one per row, cursor virtual anchor, per-item wrapper degrades to a cloneElement + throws without a provider — the "右键菜单点状态 → 行卸载连坐卸载打开中的 modal 菜单 → backdrop 泄漏吞掉全窗口点击 = 卡死" class is structurally gone. Verified: root typecheck 6/6, issue-actions-menu 8/8 (row-unmount regression + no-provider throw), swimlane 43/43, views = exact pre-existing baseline (42), Go gates N/A (zero server changes). Ship chain 7/7: snapshot `.bak` + pg_dump 32M retained, 277 migrations all-skip (zero new), asar fix-presence check (new tsx 6 717 B, `IssueContextMenuProvider` ×3). ⚠️ Shipped from a working tree that ALSO carried another session's uncommitted changes (core `networkMode: "always"` + WS-reconnect detail-invalidations predicate, status-picker/app-sidebar/experimental-locales WIP) — commit-align them ASAP. Previous release **0.5.90** (2026-08-30): OpenMythos enhancer-only outer loop — sole retired, coda-strategy delivery chain (persist + system comment @mention wake + claim briefing), `IssueOpenMythosIcon`, LabPicker enhancer-only; ship log [`.omc/0.5.90-ship-2026-08-30.md`](.omc/0.5.90-ship-2026-08-30.md). The causal graph loop stays closed both ways — write ~90% since 0.5.83 (Tier A native hooks + Tier B Semantica mirrors + nightly evolver + DB-anchored maintenance via mig 281), read ~70% since 0.5.85 (`handler/daemon.go::ClaimTaskByRuntime` injects compact causal subgraph context at claim time). Overall cooperation ≈ 70-75%. The 0.5.84 contracts (#8 trust ladder + #9 FLAG_ROUTE_SUFFIX + never-nag + TouchCausalNode callsite) all still hold.
>
