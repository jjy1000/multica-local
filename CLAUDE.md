...
> **Current release: 0.5.6 (installed at `/Applications/Multica.app`).** `agent_creation_studio` and `agent_self_optimization` are no longer Labs — they are product-level resources whose catalog literals, install handlers, view pages, and FE blacklists are all removed in 0.5.6. The 2 self-opt autopilots + `agent_creation_expert` + `智能体优化专家` leader agents are boot-provisioned and reachable through the AssigneePicker / `/autopilots` page; control is per-autopilot `enabled` (no per-flag opt-in). Migration 237 cleans up stale `experimental_resource_lock` and `experimental_resource_visibility` rows that referenced the removed flag keys. 0.5.5 promoted the install path to boot-provision; 0.5.5.1 decoupled the self-opt control surface; 0.5.5.2 hid the two flags from the Labs tab; 0.5.5.3 dropped the issue-detail "open panel" link and added a green banner to the self-opt view; 0.5.6 removes the now-empty UI plumbing (view, route, RecentLabsPanel, HIDDEN_LAB_KEYS, PRODUCT_LEVEL_LAB_KEYS, install handler, catalog literal) and the migration cleans up the dead rows. The leader agent names + autopilot rows + builtin_skill directories (`multica-creating-agents` / `multica-lab-builder`) survive — they are product-level resources, not catalog entries. **0 data loss**: workspace=1 / agent=97 / autopilot=16 / 4 self-opt autopilots all preserved. 0.5.4 fixed the LabPicker click-through so 智能体创建 on a single click dispatches via 0.3.46 P0#4. 0.5.3 generalized self-optimization from agents to four subjects (migrations 232-233). **Main working directory was renamed `multica-main` → `multica-exploration-dev` (2026-08-01).** Shipped via **manual asar repack** because `app-builder-bin@5.0.0-alpha.13` ENOENTs on a fork-0.3.63 stale `multica-main` path (CLAUDE.md "Ship chain fallback: manual asar repack (0.3.63+)"). Still deferred: `dashboard.go` cost wire-up (needs `foldRestrictedAgents` fix — see `.omc/plans/upstream-integration-0.5.0-proposal-rev2.md`), 93 fork-only files still on old text-size utilities, cloud physical-deletion PRs A/B/C/D on kept branches (`fork-hygiene-a/b/c/d`).
