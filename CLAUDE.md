...
>
> **Current release: 0.5.82** (2026-08-27, shipped from `epic/0.5.82-timesfm` merged into `epic/0.5.72-followups`; `/Applications/Multica.app` = 0.5.82, official cold-start verify PASS ~6s). **0.5.82** — WL2 of the labs evolution roadmap: local **TimesFM 2.5 forecasting lab** (torch-stack source vendor `apps/desktop/vendor/timesfm-src/`, zero upstream source edits; FastAPI `run_loopback.py` with Tier-0 seasonal-naive fallback, RAM preflight, `HF_HUB_OFFLINE=1`; weights `~/.multica/models/timesfm/model.safetensors` + wheelhouse NOT shipped in-repo — until seeded the engine answers `seasonal_naive` or is down). Load-bearing contracts: **flag key `timesfm` is a VERBATIM literal** duplicated across catalog/handler/mig CHECK/manager manifest (import-cycle law, pinned by `TestCatalogAutoDispatchContract` + `TestTimesfmFlagKeyLiteralsPinned` + migration static test); **route split** `/experimental/timesfm-lab` = view, bare `/experimental/timesfm` = REST proxy (semantica precedent — add BOTH sides to `FLAG_ROUTE_SUFFIX` + `routes.tsx` together, and new labs need the i18n namespace registered in `packages/views/i18n/resources-types.ts` + `locales/index.ts`); **engine-down honesty** — POST forecast 503s with NO synthetic envelope, GET /runs is a pure DB read that always works (ICP-2), and `provenance` (`model|seasonal_naive|mixed`) must never be stripped when reporting (skill + panel banner enforce); **ICP-1 records-only** — no manual trigger anywhere client-side (pinned by test); agent surface = `timesfm_oracle` leader + `multica-timesfm` builtin skill (inventory 20) teaching the REST contract; **migration 275** `timesfm_forecast_run` widened the `experimental_resource_lock.experimental_source` CHECK + `SourceTimesfm` or `Claim("timesfm")` hangs on 23514. **0.5.81** (prior, WL1): labs task-issue-first UX — `<IssueBreadcrumb/>` everywhere, ICP-3 deep-link round trip (`labRunHref()` out, `?run=` + `useDeepLinkRun` back; deep-link poll budget is **wall-clock 2s**, not frame-count); **gate-integrity lesson (load-bearing)**: final gates must be uncached `go test ./...` with `DATABASE_URL` exported — FULL-TURBO/scoped runs masked nine unregistered concurrent-index builds (262–272, backfilled `e7f0aa69c`). **Packaged-app ops facts**: the app bundles Postgres.app (`~/Library/Application Support/Multica/pg`, owns :5432 with its process tree); the cold-start verifier reads expected version from the PRIMARY checkout (correct post-merge-back; use `EXPECTED_VER=` pre-merge); OrbStack's `pocketbase` container maps host :8090 and races the packaged server (restart policy set to `no` 2026-08-27 — revert with `docker update --restart=unless-stopped pocketbase`). Ship log: [`.omc/0.5.82-ship-2026-08-27.md`](.omc/0.5.82-ship-2026-08-27.md); release notes [`.omc/release-notes-0.5.82.md`](.omc/release-notes-0.5.82.md). **WL3 (issue causal graph, 0.5.83) is next** — research-complete: [`.omc/wl3-causal-source-binding-report.md`](.omc/wl3-causal-source-binding-report.md), roadmap [`.omc/plans/0.5.81-0.5.83-labs-evolution-roadmap.md`](.omc/plans/0.5.81-0.5.83-labs-evolution-roadmap.md) (migrations start at **276**; WL2 took 275). 0.5.80's load-bearing contract stands: every navigation path into `/experimental/*` MUST arm the workspace-singleton release-suppression token. Authoritative upstream triage: [`.omc/upstream-integration-triage-2026-08-26.md`](.omc/upstream-integration-triage-2026-08-26.md) (through incremental #3; upstream head `76aada3a9`). **Open decision gate: upstream inbox architecture (MUL-6632, 19 files ≈ 4400 LOC)** — adopt vs fork-local filtering; until decided, inbox-family upstream commits stay SKIP-DIVERGENCE.
...
> **Read the "Current release" header at the very top of this file first** — that paragraph captures the load-bearing contracts of whatever release is live (today: 0.5.82). It changes per ship. Memory files below are the deeper lessons.
