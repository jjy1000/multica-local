...
>
> **Current release: 0.5.67** (2026-08-24, `epic/0.5.13-integration`, 7 atomic fixes on top of 0.5.60). Headline: **Mythos whack-a-mole + F-027 GAP closure batch** — labs audit (0.5.60 audit at [`.omc/audit/2026-08-23-labs-issue-integration-audit.md`](.omc/audit/2026-08-23-labs-issue-integration-audit.md) + 6 parallel auditors in [`.omc/release-notes-0.5.67.md`](.omc/release-notes-0.5.67.md)) closed every load-bearing mythos path: (1) **0.5.61 Bug-1** stale handler flag gate `experimental.DefaultFor("mythos_swarm")` deleted (was 404-ing every per-user enabled lab; same anti-pattern in `decision_sync.go` 198-200 also deleted); (2) **0.5.61 Bug-2** `upstream-registry.ts` `authToken()` reads JWT from `~/.multica/profiles/desktop-*/config.json` and attaches `Authorization: Bearer <token>` to POST/DELETE `/__experimental/upstream`; (3) **0.5.62 Bug-3** `runner.go:578,631` `CreatorType: "system"` → `"agent"` (was tripping 23514 on every fork); (4) **0.5.63 Bug-4** `IncrementIssueCounter` → `Number:` thread-through on both `CreateIssueParams` (was tripping 23505 on `uq_issue_workspace_number`); (5) **0.5.64 Bug-5** `mythos.Service.TaskService` wired, both runLoopIteration + runCoda call `EnqueueTaskForIssue` after each `CreateIssue` (was runner hangs forever — daemon never saw sub-issues); (6) **0.5.65 Bug-7** `mythosWaitTimeout` 60s → 5min (daemon takes 3-5min to first-claim, old timeout caused false-positive `completed`); (7) **0.5.66 Bug-8** preserve partial `waitFn` output when daemon completes 1-2s after timeout (was runner discarded captured body); (8) **0.5.67 Security F-027 GAP** extend `isAllowedTargetApiUrl` gate to upstream-registry IPC (0.5.61 Bug-2 attached Bearer without URL allowlist — JWT could leak to public host via `desktop.json` or `MULTICA_API_URL`). Pattern: each ship peeled back one layer of latent bugs masked by the 0.5.61 handler gate deletion. End-to-end verified: real mythos_run at 0.5.66 reached `status='completed'` with daemon-executed sub-issues (loop=262s, coda=140s) on issue `b5a53a35-82ce-4a78-9824-50c57ec333cd`. Operational fix also landed: **`scripts/backup.sh:246` `find -printf` GNU/BSD incompat** (6 ships 0.5.61-0.5.66 left no `.omc/backups/` entries); smoke-verified with 35 mock dirs — retention branch fires correctly, oldest moved to `_archive/`. **Audit-driven ship cadence caveat**: every gate deletion must peel back ALL downstream correctness checks atomically, not layer by layer — see [Known Stability Surfaces](#known-stability-surfaces) for the 4-bug mythos chain + the 1-shot integrated-fix recommendation. Audit: [`6-agent parallel audit (2026-08-24)`](.omc/audit/2026-08-23-labs-issue-integration-audit.md) — 14 architecture contracts ✅ intact, F-027 gap (now closed 0.5.67), 8 closed HIGH vuln contracts all intact, ship chain integrity restored. Memory file: [`0.5.61-0.5.67-mythos-whack-a-mole-2026-08-24.md`](~/.claude/projects/-Users-jiangjianyan-jjy-multica-exploration-dev/memory/0.5.61-0.5.67-mythos-whack-a-mole-2026-08-24.md) (cross-release memory of the bug chain + the integrated-fix recommendation). Release notes: [`.omc/release-notes-0.5.67.md`](.omc/release-notes-0.5.67.md) (consolidated batch), per-ship logs `.omc/0.5.6{1-7}-ship-2026-08-24.md`.
...
- **`vendor/openscience-bin/openscience` native binary missing — bundle-cli prints `claude_science flag will show 'service not bundled'` warning every build.** Source-of-truth dir `apps/desktop/vendor/openscience-bin/` does not exist in this fork (only `apps/desktop/vendor/openscience-src/agent-prompts/` text assets ship); `apps/desktop/scripts/bundle-cli.mjs` (line 300 + 408) reference a native binary that has to be drop-shipped externally. **Impact is limited** because `claude_science_lab` is `inline` RuntimeKind → traffic routes through the Multica runtime bridge (the same `MULTICA_REQUIRED=1` contract that pythia enforces in `oracle.py`); the missing native binary is only visible to users spawning the standalone `openscience` subprocess, bypassing Multica as the gateway. If you build or obtain the binary, drop it at `apps/desktop/vendor/openscience-bin/openscience` before the next bundle-cli run; otherwise the warning persists and `claude_science_lab` continues to work via the inline path. Mirrors the 0.5.43 code-canvas cp-block lesson — bumping a subprocess lab requires both the catalog/runtime side AND a `bundle-cli.mjs` cp block + git-tracked `resources/` copy; only the binary side is missing here.
...
> **Read the "Current release" header at the very top of this file first** — that paragraph captures the load-bearing contracts of whatever release is live (today: 0.5.67). It changes per ship. Memory files below are the deeper lessons.
