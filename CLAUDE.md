...
> **Current release: 0.5.39 (committed + shipped 2026-08-19, installed at /Applications/Multica.app).**
> 7 atomic commits on epic/0.5.13-integration (1629e1a26…e9839c18b). Headlines: **swarm_gc dormant-GC fix** + **4 upstream ports** (MUL-6390 pprof / MUL-6310 NUL gap / MUL-5879 Hermes / MUL-6387 IME) + **0.5.39 release prep** + **MUL-6324 chat resume**. Verification: `pnpm typecheck` 6/6 + `go test ./internal/... ./pkg/agent/...` 33 packages (only failure: documented pre-existing mythos panic, unrelated). Ship chain PASS, cold start ~2s, Server PID 56449. Release note `.omc/release-notes-0.5.39.md`, ship log `.omc/0.5.39-ship-2026-08-19.md`.
>
> Key contracts (the load-bearing reasons this release is non-trivial):
> - **swarm_gc dormant-GC fix** (`1629e1a26`): `Start()/Run()` take NO context (mirrors 0.5.25 runtime_gc pattern); sweep owns `context.Background` via 60s timeout + `Queries` nil guard + `sweepCount` counter. `TestSwarmGCRunSweepsUntilStopped` asserts ≥2 sweeps at 20ms interval. Contract #9 swarm archive+trash never ran since 0.5.21 — now it does. Same dormant-GC class caught twice → CLAUDE.md "verify the sweep branch is actually reachable with a test" lesson is now load-bearing for any future GC wiring.
> - **MUL-5879 BuiltinRuntime fence** (`57c84462b`): `Config.BuiltinRuntime bool` in `server/pkg/agent/agent.go`. Unset callers fail closed. `daemon/daemon.go` sets it via `customProfileLaunchForRuntime` detection. Future hermes work that depends on built-in specifics MUST use this fence — custom `protocol_family: hermes` profiles (deferred MUL-5991 jcode path) bypass the surfacing. Documented fork-local deviation.
> - **MUL-6324 migration renumber** (`e9839c18b`): upstream `349`→fork `259` (`chat_terminal_resume_index`); upstream `350`→fork `260` (`chat_retired_session_index`). SQL unchanged, prefix only. MUL-5305 block (`GetLatestChatTaskRolloutMissing` query + `PriorSessionResumeUnavailable` response field) intentionally omitted — fork lacks both. Live DB migrated; `pg_indexes` confirms both new indexes.
> - **MUL-6310 NUL scrub end-to-end** (`a05b4f75f` closes 0.5.35 gap): `sanitizeTaskCompleteRequest` / `sanitizeTaskFailRequest` wired before persistence; `ReportTaskMessages` deep-walks `Input` JSONB. End-to-end NUL coverage: util helper + skill sanitize + FailTask errMsg (all in `781cea00c`) + daemon HTTP scrub + regression tests (this commit).
> - **MUL-6350 3/4 hook-engine branch** (`upstream/mul-6350/plugins-3-hook-engine`): SKIP-DEAD-CASE for the whole series. ~30k LOC wholesale required; fork's user-plugin model is local subprocess + sandbox-isolated (zero overlap with upstream's 3rd-party iframe + HMAC-outbound); 4/4 unmerged. Existing "watch series" deferral in CLAUDE.md stays as-is.
>
> Deferred: MUL-6286 (actor properties — port WITH MUL-6305 fail-closed migration), MUL-5651 (port MUL-4257/4302/4304/4351 first), MUL-6321 OpenClaw slow hosts, MUL-6063 delegated task failures, MUL-5991 (jcode), 0c69f1f95 (Hermes resume-auth), MUL-6350 plugin rebuild 1/4 (watch series — 3/4 hook-engine branch also assessed this round, SKIP-DEAD-CASE for whole series: ~30k LOC wholesale, fork's user-plugin model is local subprocess + sandbox-isolated, zero overlap with upstream's 3rd-party iframe + HMAC-outbound, 4/4 unmerged), upstream new-commit scan (last: `9d6c0c81e` baseline; 57 commits scanned through `aa6d0dadc` MUL-6390 this round), **MUL-6323** (worktree-gate blame redirect — agent died 429 mid-port leaving 23 files +1819 LOC / 2 wholesale files / 6 typecheck errors; reverted to HEAD. Retry = fresh session + quota + small-chunk pattern: Go batch → TS batch → main-thread commit), mythos child-done notification (service-layer gap, see 0.5.38), plus 5 SKIPs:
