...
>
> **Current release: 0.5.60** (2026-08-23, `epic/0.5.13-integration`, 19 atomic commits on top of 0.5.59). Headline: **Labs × issue integration audit batch** — full-scope audit at [`.omc/audit/2026-08-23-labs-issue-integration-audit.md`](.omc/audit/2026-08-23-labs-issue-integration-audit.md) (6 parallel auditors + live DB ground truth) found the platform chain structurally sound (8/8 flags wired end-to-end) but leaking at three P0 points, all closed here: (1) `BatchUpdateIssues` was missing the 0.5.21 `swarm_topology` mutex case (batch PATCH could bypass the swarm 400; fixed + 3 test corners pinned); (2) the semantica explorer called `/decisions` via bare `fetch` — unreachable from packaged desktop, team-mode banner dead on the primary target (now `api.rawRequest`); (3) `experimental_resource_lock`/`experimental_resource_visibility` leaked orphan rows unboundedly (3401/3418 at audit; migration 274 one-shot cleanup + `SweepOrphanedExperimentalResources` on the SwarmGC tick). Plus: lab_mode create/update validation parity + boundary enum check, pythia stamp-key fallback fix, swarm_topology static IPC descriptor, mythos picker `lab_managed` filter, code_canvas `installable:true` alignment, chat_pin_ui hidden from the issue LabPicker (dead binding), and the semantica bundled-run.sh/migration-273 ship-integrity hole committed. **Post-release UX follow-ups** (shipped after the main batch, all lab-side, no migration): progress feedback during lab creation + Back affordance on every `/experimental/*` view (`df60a8d62`, `4689e6d2f`'s sibling 0.5.60 ship); "View in lab" click-to-jump on Pythia / Mythos / CodeCanvas result cards with `?issue=<id>` URL contract (`eda5b69bb`); ClaudeScience parity — attachments / predictions / code blocks each gain the same link (`4689e6d2f`); the four lab views that hold an `issueId` in state now seed it from `?issue=` so the user lands on the matching issue's data instead of the empty default (`1c2789609`); `CLAUDE.md` header / `release-notes-0.5.60.md` / `0.5.60-ship-2026-08-23.md` / audit report all synced to the post-release commits (`5c3ed7a2f`). **Post-ship root cause**: the 0.5.59 "推演成功但无数据" incident turned out to be Go defer-argument evaluation capturing the empty `collected` slice header — every successful run persisted zero rows; closed by deferring a closure + the end-to-end pin `TestIssueForecastStreamPersistsCollectedRounds` (do NOT revert the closure to a direct deferred call), and the first-ever semantica wheel was built + committed. Prior release 0.5.58 (semantica port-and-localize series closed, Phase 0 → P6) contracts live in the [Semantica integration](#semantica-integration-0552-0558) section. Release notes: [`.omc/release-notes-0.5.60.md`](.omc/release-notes-0.5.60.md), [`.omc/release-notes-0.5.59.md`](.omc/release-notes-0.5.59.md) (pythia persist + UX closure), [`.omc/release-notes-0.5.58.md`](.omc/release-notes-0.5.58.md). Verification: `pnpm typecheck` 6/6, `go build ./...` clean, new tests green with `-race`, migration 274 applied. Historical release notes (0.5.12 → 0.5.51) archived at [`.omc/_legacy/release-notes-archive.md`](.omc/_legacy/release-notes-archive.md); 0.5.43-0.5.51 notes at `.omc/release-notes-0.5.{43..51}.md`; 0.5.49 retrospective at `.omc/0.5.49-retrospective.md`; load-bearing contracts from those releases live in [Known Stability Surfaces](#known-stability-surfaces), [Active Contracts](#active-contracts-03457---polling-fallback-lab-leader-rewrite-by-issue-route-order), [Fork-Applicable HIGH Vuln Contracts](#fork-applicable-high-vuln-contracts-8-pending-0518-sec-first).
...
> **Read the "Current release" header at the very top of this file first** — that paragraph captures the load-bearing contracts of whatever release is live (today: 0.5.51). It changes per ship. Memory files below are the deeper lessons.
