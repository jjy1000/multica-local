...
>
> **Current release: 0.5.70** (2026-08-24, `epic/0.5.13-integration`, 4 atomic follow-up ships on top of 0.5.67). Headline: **Cleanup cycle — labs audit 100% closed** — three follow-up ships (0.5.68-0.5.70) closed every deferred item the 0.5.67 audit had flagged: (1) **0.5.68 Bug-9 sole-mode recovery watch**: `runner.go::runCoda` returns new `codaTimedOut bool`; when waitFn hits the 5min deadline in sole mode, `Run()` calls `scheduleSoleRecoveryWatch` that polls the coda sub-issue every 10s for up to 1h and overwrites `mythos_run.coda_conclusions` with the daemon's real synthesis; (2) **0.5.69 watch json.Marshal fix**: 0.5.68 watch used naive string concat which tripped SQLSTATE 22P02 on the first non-escaped markdown char (caught by verification agent's end-to-end run with real daemon output); fix is `json.Marshal([]string{latest})`; (3) **0.5.70 swarm_topology cancel 500 fix**: `req.Payload` defaulted to `json.RawMessage("{}")` when caller omits the field, avoiding the 23502 from migration 241's NOT NULL constraint; (4) **0.5.70 5 new migration static pins** + **2 test upgrades** (`TestRunnerEnqueuesSubIssues` substring; `TestPostDecisionSync_FiresWhenLoopbackURLSet` body+path+header). Bug-10 confirmed non-issue (server has no `WriteTimeout`; Go 1.20+ default = unlimited; `curl --max-time 540` was a test artifact). End-to-end verified: 14 regression tests + 4 vitest cases all green; Multica.app = 0.5.70 cold-start ✅. Release notes: [`.omc/release-notes-0.5.70.md`](.omc/release-notes-0.5.70.md) (cleanup cycle, 3 ships) + [`.omc/release-notes-0.5.67.md`](.omc/release-notes-0.5.67.md) (audit batch, 7 ships). Audit: [`.omc/audit/2026-08-23-labs-issue-integration-audit.md`](.omc/audit/2026-08-23-labs-issue-integration-audit.md) — 660+ lines cumulative, labs audit scope **100% closed**.
