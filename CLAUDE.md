...
>
> **Current release: 0.5.77** (2026-08-27, `epic/0.5.72-followups`, 2 large-batch upstream cherry-picks on top of 0.5.76). Headline: **Batch 5 REVIEW-tier cherry-picks** — 5 parallel-batch agent workers tackled 6 large commits from upstream triage `.omc/upstream-integration-triage-2026-08-26.md` Batch 5. Result: **22 atomic sub-commits** landed, 3 batches correctly STOPPED at pre-flight. (1) **B5-skills** `MUL-6639` (5 sub-commits) — `server/internal/handler/skill.go` adds `ListSkillFileMetadata` (Postgres-computed `size`/`content_hash`) + `?include=metadata|content` query param (default = metadata on `/api/skills/{id}/files`, content on `/api/skills/{id}`); `server/internal/cli/stall.go` (new file) replaces `http.Client.Timeout` with stall-based detection (15s no-progress budget + 10-min ceiling) — 7 new handler tests + new `?include=metadata` regression coverage. (2) **B5-process** `MUL-6658` (17 sub-commits) — every agent backend (`claude`/`codex`/`opencode`/`antigravity`/`codebuddy`/`copilot`/`cursor`/`hermes`/`kimi`/`kiro`/`openclaw`/`pi`/`qoder`/`thinking`+`models` discovery probes) now spawns in its own Unix process group; `server/pkg/agent/runtime_ownership.go` (new file, 166 LOC) adds `newRuntimeCmd` + `startOwnedProcessTree` + `releaseProcessGroup`; `runtime_ownership_unix_test.go` (6 tests) pins group ownership + descendant kill + idempotency. **Skipped (Windows half + 12 fork-absent backends)**: `proc_windows.go`, `acp_terminal.go`, `deveco`, `dim`, `dsh`, `grok`, `mcode`, `qwen`, `qwenpaw`, `reasonix`, `traecli`, `zeroclaw` — fork doesn't ship these. **Correctly STOPPED** at pre-flight: (a) B5-starter (`f8ec870f` + `09a2410e8` MUL-6629 + MUL-6709 starter prompts pair, 2074 LOC across 137 files) — fork already has a fork-local `starter_prompts` feature (`packages/views/chat/components/chat-window.tsx:1644-1720` with 3 hardcoded buttons + 4-locale i18n keys) that CONCEPTUALLY DIFFERS from upstream's per-agent JSONB-driven system; (b) B5-locale (`21673268b` MUL-6660 source-context locale rename, 54 LOC) — depends on `8c563b49` source-context sub-issue feature that fork never ported; (c) B5-inbox (`b2b4699f` MUL-6632 inbox filters, 1282 LOC) — requires 10 missing fork files (`inbox-list.tsx`, `inbox-context-menu.tsx`, `inbox-view.ts`, etc.) + 1 type-shape mismatch (`useStatusOptions` returns `{groups, options, hasCustom}` not flat `StatusOption[]`) + 1 server projection gap (`issue_priority`). All 3 STOP reasons documented; agents correctly stopped rather than wholesale-adopting. **New pre-existing failures locked** (NOT introduced by this batch): `TestDashboardEndpoints` — bisected to `6cefe25be` baseline (fork-only failure); `TestQuickCreateIssueParentTrustBoundary` race (CLAUDE.md §Known Stability Surfaces); `TestDashboardPerAgentRollupsUseExactWindow` 900s delta leak (locked 0.5.75 batch). Verified: `pnpm typecheck` 6/6 PASS; `go test ./pkg/agent/... ./internal/cli/... ./internal/handler/...` all green except 3 pre-existing failures (locked). Ship log: [`.omc/0.5.77-ship-2026-08-27.md`](.omc/0.5.77-ship-2026-08-27.md).
...
> **Read the "Current release" header at the very top of this file first** — that paragraph captures the load-bearing contracts of whatever release is live (today: 0.5.70). It changes per ship. Memory files below are the deeper lessons.
