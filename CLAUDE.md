...
>
> **Current release: 0.5.88** (2026-08-29, shipped from `epic/0.5.72-followups`; `/Applications/Multica.app` = 0.5.88). Packages the two dev cycles committed since 0.5.86 — **0.5.87** (mythos async-engine unification — swarm orchestrator's stalled-run reaper + per-tick timeout + t=0 heartbeat ported into the mythos engine; full 10-lab built-in-plugin audit clean) and **0.5.88** (labs delegation loop closed end-to-end + frozen-lab metadata + user plugins join the interaction-model taxonomy + labs UX/animation polish) — verified by live API full-loop, an Electron UI audit, AND a pre-ship review that closed two delegation-loop contract gaps (`c39fbeb2b` frozen-lab delegate gate + `41c1f0a30` per-lookup deadline); paragraphs below. The causal graph loop stays closed both ways — write ~90% since 0.5.83 (Tier A native hooks + Tier B Semantica mirrors + nightly evolver + DB-anchored maintenance via mig 281), read ~70% since 0.5.85 (`handler/daemon.go::ClaimTaskByRuntime` injects compact causal subgraph context at claim time). Overall cooperation ≈ 70-75%. The 0.5.84 contracts (#8 trust ladder + #9 FLAG_ROUTE_SUFFIX + never-nag + TouchCausalNode callsite) all still hold.
>
