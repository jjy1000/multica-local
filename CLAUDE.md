...
>
> **Current release: 0.5.83** (2026-08-28, shipped from `epic/0.5.83-causal-graph` merged into `epic/0.5.72-followups`; `/Applications/Multica.app` = 0.5.83, official cold-start verify PASS ~12s incl. first-boot migrations 276-280). **0.5.83 — WL3: the issue causal graph + hidden agent team.** Migrations **276-280** (revive `issue_dependency` additive; `causal_node`/`causal_edge` whose type CHECK sets are contract duplicated verbatim into client zod; 279 dedup index + lock-CHECK widen; **280 reject-tombstone**). Load-bearing contracts: **flag key `causal_graph` is a VERBATIM literal** (catalog / router gate / `SourceCausalGraph` / mig 279 CHECK / manifest / manager-factory — import-cycle law); **a new `Source` constant MUST ALSO join `experimental.AllSources`** — `Claim` validates the slice, so a missing entry makes every install fail "unknown source" (0.5.83 lesson; latent through S1 because the REST surface never calls Claim; pinned by `TestAllSourcesContainsCausalGraph`); **trust ladder** — A native task hooks > B Semantica mirrors > C Pythia closure (0.5.84) > D LLM proposals; tier D lands `status='suggested'` at confidence ≤ 0.5 (server halves the claim) and is invisible to subgraph/path until a human confirms; **reject is a tombstone, never a delete** (proposers probe `FindCausalEdgeBetween` any-status — ICP-5 never-nag); **hidden team** `causal_graph_curator` (bound to the `multica-causal-graph-curator` builtin skill) / `causal_graph_historian` / `causal_graph_verifier` provision with **NO dispatch leader** (no `defaultLabLeaderForKey` case — timesfm precedent); the autonomous work is the nightly `causal_graph_evolver` JobSpec (comment-window scan + transitive A→B→C gap-fill; window derived from its own SUCCESS audit rows) plus the DB maintenance ticker (SemanticaGC mirror; **Start() takes NO ctx**; stale-marks volatile nodes >30d unobserved with constraint/assumption exempt, GCs unconfirmed suggestions >30d, keeps tombstones forever). Client: `IssueCausalGraphIcon` in the issue header actions cluster → popover minimap → `/experimental/causal-graph` (focused `?issue=` + workspace views + suggested queue via `api.rawRequest`); web stub parity; i18n namespace `causal-graph` ×4 registered in BOTH `locales/index.ts` AND `resources-types.ts`. Wire contract + deferral addendum: [`.omc/wl3-server-contract.md`](.omc/wl3-server-contract.md). Ship log: [`.omc/0.5.83-ship-2026-08-28.md`](.omc/0.5.83-ship-2026-08-28.md); release notes [`.omc/release-notes-0.5.83.md`](.omc/release-notes-0.5.83.md). **Post-ship verification (2026-08-28, live runtime test + 2 expert audits)**: the write side is a real automatic loop (live test: flag toggle installed the hidden team; a real 前端工程师 run on JYF-401 recorded `constraint→enables→action→causes→outcome` with the agent's actual result text; evolver SUCCESS in the packaged app), but **the read side is 0% — agents cannot see the graph** (no causal injection in claim-response) and the hidden team has no auto-dispatch; overall cooperation ≈ 40-45%. Six confirmed bugs head the 0.5.84 fix list — two are standing laws now: **a new lab flag key MUST also get a `FLAG_ROUTE_SUFFIX` row** (issue-labs-section.tsx; semantica 0.5.81 + causal 0.5.83 hit the same silent-link-death trap) and **every proposer endpoint needs the server-side never-nag probe** (`createCausalSuggestion` missed the `FindCausalEdgeBetween` probe the manual path has — duplicate suggested rows). Plus: 30-day stale TTL hides old chains (`TouchCausalNode` zero callers) + in-process ticker may never fire under daily restarts. Full audit: [`.omc/0.5.83-post-ship-verification.md`](.omc/0.5.83-post-ship-verification.md).
>
