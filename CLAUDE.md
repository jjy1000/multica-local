...
> **Current release: 0.5.5.2 (installed at `/Applications/Multica.app`).** `agent_creation_studio` and `agent_self_optimization` are now fully **product-level resources**, with the Labs tab UI cleaned up too. 0.5.5 boot-provisioned the `agent_creation_expert` + `智能体优化专家` leader agents; 0.5.5.1 decoupled the self-opt control surface from the experimental flag system (per-tick `flagOnForUser` is a stub always returning `true`, catalog `DefaultVal` is `true`, the 9 `experimentalFlagEnabled` gates in `self_opt_edits.go` / `agent_trust.go` are short-circuited to `false &&`); 0.5.5.2 closes the FE-side leak by adding `PRODUCT_LEVEL_LAB_KEYS` filter in `packages/views/settings/components/labs-tab.tsx` so the Settings → Labs tab no longer surfaces the two product-level flags (mirrors the `HIDDEN_LAB_KEYS` set in `issue-detail.tsx::LabPicker`). User control is via the **autopilot row's own `enabled` field** — `multica autopilot update --disabled` flips each of the 2 self-opt autopilots (SkillOpt-Multica daily + per-3-workday bulk) individually. The LabPicker entry list still drops these two keys. Catalog flag values stay in place for legacy `issue.lab_source='agent_creation_studio'` lookups; full catalog removal is deferred to 0.5.6. Previous 0.5.5 promoted the install path to boot-provision; 0.5.4 fixed the LabPicker click-through so 智能体创建 on a single click dispatches via 0.3.46 P0#4. 0.5.3 generalized self-optimization from agents to four subjects (migrations 232-233). **Main working directory was renamed `multica-main` → `multica-exploration-dev` (2026-08-01).** Shipped via **manual asar repack** because `app-builder-bin@5.0.0-alpha.13` ENOENTs on a fork-0.3.63 stale `multica-main` path (CLAUDE.md "Ship chain fallback: manual asar repack (0.3.63+)"). Still deferred: `dashboard.go` cost wire-up (needs `foldRestrictedAgents` fix — see `.omc/plans/upstream-integration-0.5.0-proposal-rev2.md`), 93 fork-only files still on old text-size utilities, cloud physical-deletion PRs A/B/C/D on kept branches (`fork-hygiene-a/b/c/d`).
