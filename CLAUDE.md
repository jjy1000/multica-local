...
> - **0.5.41 batch (5 commits)**: MUL-6413 timeline status glyph custom identity + MUL-7214 inbox row status glyph custom identity + MUL-6399 drop category headings from status filter + release-prep + post-ship docs
> - **0.5.42 batch (3 commits)**: MUL-6396 stop `task:message` fanout flooding every client (server clip env-gated + client drop-frame + 100ms coalesce + structuralSharing union-by-seq) + 5 regression tests for `unionTaskMessagesBySeq` + mobile truncated-refetch branch
>
> Verification: `pnpm typecheck` 6/6 + all three ship chains PASS (server PID 88073 → 18423 → 50859). Cold starts ~4s each, row parity 7/346/2469/119 unchanged. Pre-existing mythos panic confirmed pre-existing via worktree on 0.5.39 HEAD; unrelated.
>
...
> - **0.5.40 MUL-6362 structural regression guard** (`server/pkg/agent/launch_test.go::TestOnlyLaunchGoLogsAgentCommandArgs`): AST-walks every .go file in `pkg/agent` and fails on any `Debug/Info/Warn/Error/Log/LogAttrs` call carrying `args`/`argv`/`agent command` literal or `args`/`cmdArgs`/`argv`/`cmd.Args` identifier. Catches both direct reverts and renamed-equivalent reverts. Cheap (~50 LOC test, <100ms).
> - **0.5.42 MUL-6396 realtime fanout flood fix** (server: `daemon.go::truncateTaskMessageForBroadcast` env-gated by `MULTICA_CLIP_TASK_MESSAGE_BROADCAST` + client: `isTaskMessageTimelineHeld` drop-frame + 100ms `TASK_MESSAGE_FLUSH_MS` coalesce + `unionTaskMessagesBySeq` structuralSharing rule on `taskMessagesOptions`). Critical lesson: **`setQueryData` does not postpone garbage collection** — the flush re-checks the held gate immediately before each write; without it, closing a transcript while its run keeps streaming can have the entry disappear mid-window and writing would REBUILD it holding only that batch (under `staleTime: Infinity` the next open reads the stub as fresh and never fetches — silent, permanent). Server-side clip is OFF by default per upstream's "must-fix" finding; ops flips the env var after the client reader has saturated.
> - **0.5.42 MUL-6396 wire format**: `TaskMessagePayload.Truncated bool` (server `protocol/messages.go` +7) and `TaskMessagePayload.truncated?: boolean` (client `core/types/events.ts` +3). Three places consume it: web/desktop `use-realtime-sync.ts` coalesce flush (triggers `backfillTaskMessages`), mobile `chat-ws-updaters.ts` (invalidates query, currently unreachable — mobile gates on `chat_session_id`), Zod schema `apps/mobile/data/schemas.ts` (`z.boolean().optional()`).
> - **0.5.41 status-glyph custom-color thread** (MUL-6413 + MUL-7214 + MUL-6399): the StatusIcon component already supports `category` + `color` props (MUL-6243); 0.5.41 wires the catalog entry's color into the timeline (issue-detail.tsx ActivityBlock), inbox row, and status filter. Built-ins resolve to `null` so they keep their semantic token color; custom statuses carry their own `#rrggbb`. New `StatusOption.category` field added to the flat-list path (MUL-6399 collapse).
> - **0.5.41 MUL-6363 SKIP-NO-SURFACE**: the upstream refactor drops a duplicate `no_value_label` i18n key in favour of `pickers.custom_property.none`. Fork's `packages/views/locales/*/issues.json` does NOT have a `pickers.custom_property` namespace — the prerequisite commit `66af64b9a6f4` (support filtering custom properties by "no value") is still deferred. SKIP until `66af64b9a6f4` lands.
> - **0.5.41 109b67790 SKIP-DEPENDENCY**: the upstream fix wraps `DropdownMenuLabel` in `DropdownMenuGroup` to stop Base UI's `Menu.GroupLabel` from throwing outside a `Menu.Group` ancestor. Once MUL-6399 drops the category headings entirely (no `DropdownMenuLabel`), this fix becomes unnecessary. Upstream also added a `fixed = viewBaseline?.status.has(...)` check that depends on `viewBaseline` — fork's view store never ported that. SKIP until view store adds the baseline concept (or until MUL-6399's collapse renders this fix obsolete, which it does).
> - **0.5.42 MUL-6396 deferred items** (fork port committed, 2 new test files NOT ported): `daemon_task_message_broadcast_test.go` (+211) + `use-realtime-sync-task-messages.test.tsx` (+347). Adds 558 LOC of regression coverage for the env gate + timeline-held gate; can land in a follow-up commit.
>
> Deferred: MUL-6286 (actor properties — port WITH MUL-6305 fail-closed migration), MUL-5651 (port MUL-4257/4302/4304/4351 first), MUL-6321 OpenClaw slow hosts, MUL-6063 delegated task failures, MUL-5991 (jcode), 0c69f1f95 (Hermes resume-auth), MUL-6350 plugin rebuild 1/4 (watch series — 3/4 hook-engine branch also assessed this round, SKIP-DEAD-CASE for whole series: ~30k LOC wholesale, fork's user-plugin model is local subprocess + sandbox-isolated, zero overlap with upstream's 3rd-party iframe + HMAC-outbound, 4/4 unmerged), upstream new-commit scan (last: `aa6d6dadc` baseline; 23 commits scanned through `659bb41abd` — net **6 ported (3 in 0.5.40 + 3 in 0.5.41)** + **5 SKIP-NO-SURFACE/DEPENDENCY added to deferred: MUL-6264 chat-thread-list missing, MUL-6394 onIssueAuxiliaryRevision pattern missing, MUL-6363 needs `66af64b9a6f4` first, 109b67790 viewBaseline fork-lacks + MUL-6399 fix makes it obsolete, MUL-6409 use-issue-status-branches.ts missing**; remaining: ea07a29138 UUIDv7 + 9bd55678 inbox shell nav feedback + 66af64b9a6f4 custom-property "no value" + 34391460 Vercel SKIP + MUL-6403 changelog SKIP + MUL-6396 perf fanout flood (deep-eval needed) + MUL-6343 semantic activity (2366 LOC, own ship) + ea07a291 UUIDv7 + 9bd5567 inbox feedback), **MUL-6323** (worktree-gate blame redirect — agent died 429 mid-port leaving 23 files +1819 LOC / 2 wholesale files / 6 typecheck errors; reverted to HEAD. Retry = fresh session + quota + small-chunk pattern: Go batch → TS batch → main-thread commit), mythos child-done notification (service-layer gap, see 0.5.38), plus 9 SKIPs:
...
>   - **MUL-6264 chat agent names in session list** (`5e99a0597eda`, +70 LOC, 2 files) — SKIP-NO-SURFACE: fork 无 `packages/views/chat/components/chat-thread-list.tsx`. Fork 的 chat 架构是 `chat-window.tsx` + `chat-fab.tsx` + `use-chat-context-items.ts`,跟 upstream 的 thread-list 模型不同. 如果 fork 以后 port 这个 surface,这个 commit 是直接 cherry-pick
>   - **MUL-6394 comment create failing on sibling issue caches** (`c8048a0c8a4e`, +238/-70, 4 files) — SKIP-NO-SURFACE: fork **无 `onIssueAuxiliaryRevision`** (grep 0 hit),无 `bucketedListEntries/flatListEntries/tableRowEntries/issueArrayEntries` helpers. Fork 的 `packages/core/issues/cache-helpers.ts` 走 `addIssueToBuckets/findIssueLocation/patchIssueInBuckets` 不同的 model, upstream 要修的 bug 在 fork 不存在. 如果 fork 以后迁到新的 cache coordinator, 这个 regression test 是首选
>   - **MUL-6363 reuse custom_property.none i18n key** (`f9b7bf52ea3e`, +3/-7, 5 files) — SKIP-NO-SURFACE: fork 的 `packages/views/locales/*/issues.json` 无 `pickers.custom_property` namespace. 前置 commit `66af64b9a6f4` (feat: filtering custom properties by "no value", +294 LOC) 还在 deferred. SKIP 直到 `66af64b9a6f4` port
>   - **109b67790 status filter category heading crash fix** (`109b67790541`, +191/-35, 2 files) — SKIP-DEPENDENCY: 上游修 Base UI `Menu.GroupLabel` THROW 当 `DropdownMenuLabel` 没有 `Menu.Group` 祖先. 修了 fork 的 `viewBaseline?.status.has(...)` check + `fixedTitle` constant. 0.5.41 落地 MUL-6399 后类别标题完全消失, `DropdownMenuLabel` 不再被渲染, 这个 fix 变得不必要. fork 的 view store 没 `viewBaseline` 字段 — 等 view store 加 baseline concept 再考虑. 当前行为已正确 (0.5.41 MUL-6399 fix 替代)
>
> **0.5.38 (2026-08-19, superseded by 0.5.39).**
...
>   - **MUL-6335 skills bulk update** (`2014ae3613bb`) — SKIP-NO-ENDPOINT: fork lacks `POST /api/skills/{id}/refresh` (the single-skill refresh prerequisite from upstream PR `147cd8d84` was never ported — that PR adds a new SQL column + cache + schema). Bulk-update UI would 404 on every refresh, breaking the UX contract documented in upstream's commit message. Port prerequisite: port `147cd8d84` as a separate audit (~3k LOC Go + SQL).
>
> **0.5.35 (2026-08-18, superseded).** 4 atomic cherry-picks ~370 LOC (parallel agent pattern + main-thread commit). **MUL-6233 RELAND audit: RELAND-UNSAFE → SKIP** — upstream `1b6d42864` byte-identical to the reverted `4effd07ad`; the MUL-6293 tab-host fix that made upstream safe is NOT in fork. 6 cherry-picks skipped for structural divergence (MUL-6140/6323/6333/6321/6063 — details in release-notes-0.5.35.md). 2 MUL-6303 fork deviations worth a review pass (dropped `isWorkspaceDeletePending`; module-level `singletonOwner` symbol anti-pattern).
> **0.5.34 (2026-08-18, superseded).** MUL-6243 frontend closed end-to-end (TS types/schemas/client 4 methods + CLI format-only validator + SKILL.md, ~600 LOC, 4 commits). Lesson: small-scope briefs (≤600 LOC, tightly-scoped file list) avoid the agent autocompact thrash that 4 backend agents hit on the 0.5.33 port.
> **0.5.33 (2026-08-18, superseded).** MUL-6243 backend — migrations 250-258 (issue_status catalog + `issue_effective_status` SQL fn), `server/internal/issuestatus/` package, FF_CUSTOM_ISSUE_STATUSES **default OFF**, 17-file consumer normalization with 2 fork deviations (mythos supervise + daemon CompleteTask flip route through `Effective()`); + MUL-6291 jsdom removal (122 test files). Lesson: **ports > ~2000 LOC / >10 files → main thread does `git add`+`git commit`+`ship-mac`; agents do reading+editing in small chunks only**.
> **0.5.32 (2026-08-17, superseded).** MUL-6233 Cmd/Ctrl+, settings shortcut **REVERT-ALSO** (upstream revert `2829798b5` with an empty PR body is still a strong signal; fork code byte-identical → default REVERT-ALSO). MUL-6288 concurrent-index cleanup registry (fork had 20+ `CREATE INDEX CONCURRENTLY` migrations with NO cleanup — torn build → INVALID index → `IF NOT EXISTS` marks applied → uniqueness silently unenforced). MUL-6254 test port exposed `useActorName` unstable default (MUL-4985) + web v5 selector contract (error → data stays `undefined`, never null).
> **0.5.31 (2026-08-17, superseded).** AuthTokenGC — unified 6h GC for 3 dormant `expires_at` ladders (`task_token`/`workspace_invitation`/`daemon_token`; **DELETE queries already existed with ZERO callers**). Reusable retention-audit pattern: grep migrations → check GC callers → look for "delete query exists but zero callers". Migration 249 truncates abandoned auth tables. pgProbeUrl loopback-only DATABASE_URL guard.
> **0.5.30 (2026-08-17, superseded).** SemanticaGC (24h tick, 90d `.provenance` sweep, key rotation) + `SemanticaDecisionRecordSchema` zod pin + MUL-6254 auth-recovery UI full port (6 commits, `loginWithGoogle` dropped). Ship hardening: **4a/7 renderer integrity check** in ship-mac.sh (file exists + first byte `<` + size 200-5000) — 0.5.29 shipped a broken renderer silently (highlight.js module prepended before `<!doctype>`).
> **0.5.29 (2026-08-17, superseded).** Semantica × Multica data localization: wsId validated against WS_ID_REGEX before path interpolation; X-API-Key moved INSIDE the auth chain (was an unauthenticated credential-oracle surface); per-launch 32-byte key via IPC, in-memory only (file transport retired — F-013-class surface).
> **0.5.28 (2026-08-17, ×2, superseded).** Semantica patch: ready_timeout_ms 120s clamp (cold start exceeds 180s on cold cache), graph backup script, E2E smoke script, audit-then-decide lesson (R4 security pass prevented shipping net-negative). PORT/URL env-leak patch: completes 0.5.27 — the conditional `--server-url` fix was null on the cold-launch main-process path; `initTargetApiUrl` seeded from desktop.json + `env.PORT` pin.
> **0.5.27 (2026-08-17, superseded by 0.5.28).** Partial daemon `--server-url` fix (renderer-driven spawn path only).
> **0.5.26 (2026-08-17, superseded).** MUL-6107 bounded per-runtime GC replacing a bulk DELETE that cascade-destroyed task tables (NOT NULL prerequisite trap → migration 247/248 minimal-scope halves); MUL-6126 private runtimes owner-only (403 at 3 call sites). 29 upstream commits shipped total.
> **0.5.25 (2026-08-17, superseded).** RuntimeGC **actually** fixed after 8 dormant versions — eager `stopOne.Do(close())` before the loop + `Start()` never wired at boot (the 2026-07-28 audit claimed "fixed" but no fix commit ever landed — **`git log --grep` when an audit says "fixed"**). Migration 151's 30/90/120-day retention ladder now runs in production.
> **0.5.24 (2026-08-16, docs-only) / 0.5.23 (2026-08-16).** MUL-3963 invocation permissions (migration 245) + MUL-4525 §2 honest trigger outcomes, 11 commits. Closes the 0.5.21 deliberately-skipped test gate.
> **0.5.22 (2026-08-15, superseded).** CompleteTask issue-flip (trap: `agent_task_queue.status` terminal enum is `completed`, NOT `done` — `done` is an issue.status value); lab deliverable summary card; **Active Contract #6: lab auto-dispatch opt-out** (`Flag.AutoDispatch *bool`; `claude_science_lab` only; manual trigger `POST /api/experimental/claude-science/issues/{id}/run`).
> **0.5.21 (2026-08-15).** MUL-4304 agent→agent @mention reconcile — `reconcileCommentsOnCompletion` replays agent-authored comments via `keepExplicitMentionTriggers` ONLY (explicit @agent/@squad mentions; anti-loop unchanged); migration 240 `originator_user_id` + `accountable_user_id` (closes 167 latent gap: referenced but never ADDed).
> **0.5.20 (2026-08-14).** MUL-5799 sub-agent delegation interrupt — macOS sandbox paths exceed AF_UNIX `sun_path` cap (104B macOS / 108B Linux); per-task private TMPDIR injection.
> **0.5.19 (2026-08-14).** LabOutputPanel + iframe auth proxy (HMAC const-time compare, per-launch 32B secret, exp ≤5min; iframe `sandbox="allow-scripts"` never `allow-same-origin`). Spec-drift lesson: M3 was already frontend-only (endpoint existed) — verify before implementing.
> **0.5.18 (2026-08-13).** Lab environment batch (26 commits) + **8 fork-applicable HIGH vulns closed** (F-002 trust-gated bypassPermissions, F-005 env blocklist, F-006 artifact upload sanitize + attachment-only serve, F-008 plugin-skill ack dialog, F-013 installer-scoped visibility, F-027 API-url allowlist; F-007/F-028 verified non-issues — triage `.omc/audit/2026-08-05-vuln-scan/triage.md`).
> **0.5.17 (2026-08-12).** Lab usability (per-flag install button; llm_wiki_bridge real stdio verbs + env injection). Lessons: **audit findings must be verified against the code before fixing** (B1c/B2a/B2b all false positives); packaging must run from `apps/desktop/`.
> **0.5.16 (2026-08-12).** Phase 1 lab P0 fixes (7 blockers, 11 commits). Lessons: complex SSE changes → helper-first + wire-up in a follow-up commit; `git status` triage before each ship.
> **0.5.15 (2026-08-10).** 13 surgical cherry-picks (~150 attempted, 95.7% conflict rate) + 1 revert + 1 fixup (`da1cc2003` — upstream `#6199` only wired the slim brief path, breaking the fork's legacy verbose default; **ship gate must include `go test` — `pnpm typecheck` does not run Go tests**).
> **0.5.14 (2026-08-10).** Fork-local cleanup only (gitignore hygiene + historical docs). Zero Class-A cherry-pick candidates remain — every surgical fix already integrated through the 0.5.8 → 0.5.13 batches.
> **0.5.13 (2026-08-09).** #5674 daemon fail-fast (fork-local fusion), #6515 cancelled items demoted in search/palette/pickers, #6546 CLI `--compact`; snapshot switched to `pg_dump -Fc` (24 MB vs ~1.5 GB per-table CSV).
> **0.5.12 → 0.5.8 (2026-08-06..09).** Upstream safety/UI batches A/B/C: #6222 WS 64KiB inbound cap, #6279 daemon dial honours `HTTP(S)_PROXY`, #6175 `prev_*` stripped from `issue:updated` WS frames, #6366 context-overflow classification, #6422 structured `terminal_reason`, #6410 router-owned autopilot dispatch, #6380 subscription hardening, #6194 rollup windows, #5406 non-ErrNoRows propagated, #5355 pinned-executable self-heal, #6124 open-in-new-tab, #6437 tab MRU, #6440/6450 per-run token usage, #6426/6464 thread navigator, #6424/6435 image-sequence nav, #6095 WCAG annotation, Anthropic SDK error classification + `server/pkg/redact` restore. Chat follow-up queue cluster deliberately NOT ported (fork single-task chat = rewrite, not a cherry-pick).
...
> **Read the "Current release" header at the very top of this file first** — that paragraph captures the load-bearing contracts of whatever release is live (today: 0.5.39). It changes per ship. Memory files below are the deeper lessons.
