...
>
> **Current release: 0.5.81** (2026-08-27, shipped from `epic/0.5.81-lab-ux-consistency`; `/Applications/Multica.app` = 0.5.81, official cold-start verify PASS ~2s). **0.5.81** — labs task-issue-first UX batch (WL1 of the 0.5.81–0.5.83 roadmap): shared `<IssueBreadcrumb/>` back-link across all lab views (`b897aab5c`); ICP-3 lab⇄issue deep-link round trip — issue record rows deep-link out via `labRunHref()`, `?run=` scrolls/highlights back in via `useDeepLinkRun` on `useOptionalNavigation` (rAF retry reaches async-mounted rows, one-shot dedupe); receivers wired: mythos PastRunsPanel, pythia ForecastHistoryPanel, ExecutionLogSection both directions incl. past-list auto-expand; falsy run ids omit the `run=` half (stateless code-canvas / `user_*` plugins), whose durable-runs endpoint is a recorded prerequisite (`ee6b3869c`/`3d7d6f0af`/`6f7f003c3`). Auto-dispatch trigger defaults consolidated (`a3ffde24e`: pythia_oracle `ptrBool(false)` opt-out, claude_science_lab restored nil; pinned by `TestCatalogAutoDispatchContract`; only enqueue gates — leader rewrite unaffected). **Gate-integrity lesson (load-bearing)**: a DB-backed `go test ./...` finally reached `cmd/migrate` and caught nine concurrent-index up-migrations (262–272) missing from `concurrentIndexCleanups` — backfilled in `e7f0aa69c` (map registration only; migration SQL untouched); FULL-TURBO / scoped-package gates had masked this for several releases — final gates must be uncached and `./...` with `DATABASE_URL` exported. Fixture symlinks made checkout-relative (`19083369e`). **Packaged-app ops facts**: Multica.app bundles Postgres.app binaries (`~/Library/Application Support/Multica/pg`, reporting "17.10 (Postgres.app)") and boots them against `.../Multica/pgdata` on 127.0.0.1:5432 as part of ITS process tree — killing/killing-in-flight the app takes the DB down; `~/.multica/scripts/verify-desktop-cold-start.sh` reads expected version from the PRIMARY checkout (stale until merge-back — pass `EXPECTED_VER=0.5.81`); OrbStack's auto-run `pocketbase` container maps host :8090 and races the packaged server for its fixed port (stop it during verify windows). Ship log: [`.omc/0.5.81-ship-2026-08-27.md`](.omc/0.5.81-ship-2026-08-27.md). WL2 (TimesFM vendoring) & WL3 (issue causal graph) research-complete, implementation gated on their surfaced decision points: [`.omc/wl2-timesfm-vendoring-report.md`](.omc/wl2-timesfm-vendoring-report.md), [`.omc/wl3-causal-source-binding-report.md`](.omc/wl3-causal-source-binding-report.md). Historical chain detail for 0.5.77–0.5.80 stays in git history / prior headers (0.5.80's load-bearing contract remains: every navigation path into `/experimental/*` MUST arm the workspace-singleton release-suppression token). Authoritative upstream triage: [`.omc/upstream-integration-triage-2026-08-26.md`](.omc/upstream-integration-triage-2026-08-26.md) (through incremental #3; upstream head `76aada3a9`). **Open decision gate: upstream inbox architecture (MUL-6632, 19 files ≈ 4400 LOC)** — adopt vs fork-local filtering; until decided, inbox-family upstream commits stay SKIP-DIVERGENCE.
...
> **Read the "Current release" header at the very top of this file first** — that paragraph captures the load-bearing contracts of whatever release is live (today: 0.5.81). It changes per ship. Memory files below are the deeper lessons.
