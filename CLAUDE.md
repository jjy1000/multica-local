...
>
> **Current release: 0.5.74** (2026-08-26, `epic/0.5.72-followups`, 8 atomic PRs on top of 0.5.73 hotfix). Headline: **Lab integration polish batch** — closed 16 audit findings (5 confirmed + 11 new from Architect + Code-Reviewer) via 8 atomic PRs: (1) **PR 2** `app-sidebar.tsx` icon dict补 `swarm_topology` (Network) + `semantica` (Brain) — previously both fell back to FlaskConical; (2) **PR 3** `Registry.SidebarEntries` `slog.Warn` on parse fail + memo cache + handler pre-compute `sidebarEntriesByKey` (kills N+1 manifest JSON parses + 0.5.73 same-source silent fail); (3) **PR 4** LabPicker honor `always_show_in_lab_picker` DTO escape hatch (was dead code); (4) **PR 5** + closure `b9713a0a2` — UserPluginsSection 7 hardcoded Chinese strings → 4-language i18n `user_plugins.*` namespace (13 keys × 4 locales) + L283 ternary button states + `lab-i18n-keys.test.ts` 80-case pin; (5) **PR 6** semantica inline `Sidebar:` field migrated to manifest (single source of truth — dual-source maintenance risk closed); (6) **PR 7** mythos_swarm renamed `Mythos Swarm v1` (zh-Hans "Mythos 蜂群 v1" / ja "Mythos スウォーム v1" / ko "Mythos 스웜 v1") + icon `Network → MessagesSquare` to differentiate from swarm_topology's Network; (7) **PR 8** userFlags loop calls `SidebarEntries` (was missing — user plugin never appeared in sidebar group) + labs-tab `runInstallAll/runInstallFlag` invalidate `user-plugins` cache + LeaderAgent fallback text. PR 1 (i18n pythia/mythos key 补缺) **ALREADY_PRESENT** — Code-Reviewer P0 #1 was a false positive; 4 locale files already had `experimental_pythia` (line 42) + `experimental_mythos` (line 49) at identical nesting (verified by direct grep). Pre-existing `TestQuickCreateIssueParentTrustBoundary` race (shared `testRuntimeID` row across parallel handler tests, see Known Stability Surfaces below) noted as known issue — this batch touched 0 files in `quick_create_parent_test.go`, deferred to 0.5.75+. Verified: `pnpm typecheck` 6/6 PASS; `lab-i18n-keys.test.ts` **80/80 PASS**; Go `internal/experimental` + `internal/handler` tests all green (except pre-existing flaky, ships with note). Ship log: [`.omc/0.5.74-ship-2026-08-26.md`](.omc/0.5.74-ship-2026-08-26.md). Cross-session memory: `~/.claude/projects/-Users-jiangjianyan-jjy-multica-exploration-dev/memory/0.5.74-lab-integration-fix-2026-08-26.md`.
