...
>
> **Current release: 0.5.79** (2026-08-27, ship head `8fb88ce1b` on `epic/0.5.72-followups`; `/Applications/Multica.app` = 0.5.79, official cold-start verify PASS). Chain on top of 0.5.76: **0.5.77** — Batch 5 REVIEW-tier cherry-picks: MUL-6639 (skill file metadata + `cli/stall.go` stall detection) + MUL-6658 (per-backend process-tree ownership via `pkg/agent/runtime_ownership.go`), 22 atomic sub-commits, 3 batches correctly STOPped at pre-flight (starter-prompts conceptually differs / source-context unported / inbox needs 10 absent files) — full detail in [`.omc/release-notes-0.5.77.md`](.omc/release-notes-0.5.77.md). **0.5.78** — labs hardening (plan [`.omc/plans/labs-plugin-upstream-hardening-0.5.78-plan.md`](.omc/plans/labs-plugin-upstream-hardening-0.5.78-plan.md)): P2-1a plugin visibility rows now purged on delete + before reseed (`DeletePluginResourceVisibilityByFlagKey`) so `lab_managed` greying cannot outlive a plugin; P2-1b signed-URL TTL pinned at the HTTP boundary; P2-6b lab-picker assignee-clear narrowed to `ASSIGNEE_MUTEX_LABS = {mythos_swarm, swarm_topology}` (was: every lab); dashboard exact-window seeds re-anchored on the UTC calendar date (`07294db89`, kills the Asia/Shanghai morning 900s leak); semantica `surface.proxy_prefix` + `build-semantica-wheel.sh`; openscience missing binary demoted to informational. **0.5.79** — MUL-6703 split-port: "Import from local" skill archive upload — server multipart branch (`handler/skill_import_archive.go`: 16 MiB cap, zip-slip guard, root-on-shallowest-SKILL.md, shared `finishSkillImport` tail) + core `skills/pack-archive` (1 MiB file / 8 MiB bundle / 256 files / 16 MiB archive caps) + `ApiClient.importSkillArchive` + dialog route + locales ×4 + docs ×4. **Fork deviation kept (edit-time trap)**: the skills API has NO zod envelope layer (no SkillSchema/EMPTY_SKILL) — client calls use the raw-JSON idiom via `this.fetch`/`fetchRaw`; upstream skill ports must adapt, never import the envelope. 2026-08-27 post-ship audit: handler/daemon/agent suites green with real DB (rollups UTC fix holds; `TestDashboardEndpoints` passed; quick-create race stays locked-intermittent, see Known Stability Surfaces), turbo typecheck 6/6, dialog+picker vitest 21/21; locked pre-existing TS failures: `issue-detail.test.tsx` highlightCommentId ×2. Authoritative upstream triage: [`.omc/upstream-integration-triage-2026-08-26.md`](.omc/upstream-integration-triage-2026-08-26.md) (through incremental #3; upstream head `76aada3a9`). **Open decision gate: upstream inbox architecture (MUL-6632, 19 files ≈ 4400 LOC)** — adopt as a multi-session project vs keep fork inbox + fork-local filtering; until decided, inbox-family upstream commits stay SKIP-DIVERGENCE. Ship log: [`.omc/0.5.79-ship-2026-08-27.md`](.omc/0.5.79-ship-2026-08-27.md).
...
> **Read the "Current release" header at the very top of this file first** — that paragraph captures the load-bearing contracts of whatever release is live (today: 0.5.79). It changes per ship. Memory files below are the deeper lessons.
