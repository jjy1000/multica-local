...
>
> **Current release: 0.5.84** (2026-08-28, shipped from `epic/0.5.72-followups`; `/Applications/Multica.app` = 0.5.84, cold-start verify pending ship-mac). **0.5.84 — Causal Graph P0 fix batch** (closes all 6 P0 bugs from the 0.5.83 post-ship audit; no new flag, single migration **281 = `causal_graph_maintenance_state`** singleton row that powers the boot-anchored sweep). The 6 fixes: `FLAG_ROUTE_SUFFIX` row for `causal_graph` (silent-link trap closed for the 2nd time, regression-pinned at issue-labs-section.test.tsx); `createCausalSuggestion` probes `FindCausalEdgeBetween` any-status before INSERT (never-nag contract now enforced on REST + manual paths, tombstone law now REST-side too); `TouchCausalNode` callers on every issue/comment/task hot path via `Recorder.RefreshForIssue` + the new bulk `RefreshCausalNodesForIssue` CTE (30-day stale TTL no longer hides Tier A chains); maintenance ticker DB-anchored via mig 281 + boot sweep (daily desktop restarts no longer starve the GC); edge rendering split by `status` (active solid / suggested muted-dashed / rejected ghost-opacity); ring layout pre-computes `ringLengths` before insertion (N=3 nodes spread 0/2π/3/4π/3 as designed). **Two new standing laws** (Active Contracts #9): every new lab flag key MUST also get a `FLAG_ROUTE_SUFFIX` row (issue-labs-section.tsx; same trap semantica 0.5.81 + causal 0.5.83 both hit — twice); every proposer endpoint needs the server-side never-nag probe (`FindCausalEdgeBetween` ANY status). **Plus the TouchCausalNode callsite contract**: every issue/comment/task hot path MUST refresh `last_observed_at` via `Recorder.RefreshForIssue` — legitimate hook points: `handler/issue.go::UpdateIssue` L3228, `handler/comment.go::CreateComment` L1351, `service/task.go::{enqueueIssueTask, enqueueMentionTask, CompleteTask}`. Cooperation ≈ 40-45% unchanged (write side ~90%, **read side still 0%** — agents cannot see the graph while working). **0.5.85 next** (P1 headline): daemon claim-response subgraph injection (`handler/daemon.go::ClaimTaskByRuntime` — read-once at claim time, depth≤2, top-20 nodes/edges, skip Tier D + status≠active, hard cap 4000 tokens + 200ms ctx deadline + silent fallback on timeout/error). P2: Tier C Pythia hypothesis→evidence closure, historian/verifier automation, mention-flow doc notes, evolver window bound. Ship log: [`.omc/0.5.84-ship-2026-08-28.md`](.omc/0.5.84-ship-2026-08-28.md); release notes [`.omc/release-notes-0.5.84.md`](.omc/release-notes-0.5.84.md); audit [`.omc/0.5.83-post-ship-verification.md`](.omc/0.5.83-post-ship-verification.md).
>
