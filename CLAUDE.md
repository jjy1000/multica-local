...
> **Current release: 0.5.22 (committed 2026-08-16, awaiting packaging).**
...
- **Contract #8: Human interrupt via comment (zero new IPC).** User comment with `@<swarm_coordinator>` mention wakes the leader via the existing MUL-4304 reconcile path. The interrupt kind (`pause` / `cancel` / `redirect` / `inject_message`) maps to `swarm_interrupt.kind` for audit + to `swarm_role_message.type='human_interrupt'` for the leader's working memory. `cancel` is synchronous (handler flips `status='aborted'` immediately); `pause` + `redirect` + `inject_message` are async (orchestrator's 30s tick picks them up).
...
References:
- Migration 241 + sqlc queries (server/pkg/db/queries/swarm_run.sql)
