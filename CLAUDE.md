...
>
> **Current release: 0.5.104** (2026-09-08, shipped `97b76aa8f`+`0dc1a4a88`+`d09c8e4d2`+`83f2e6400`; **Pythia 推演可信化批次 — post-ship verified: real oracle deliberations land on JYF-455 (15:22/15:30, `lab_source=oracle`, 24-58s genuine LLM rounds; llm-call went 0 → 200s)** — five fixes rooted in live-log forensics on test issue「模拟推演方案」. (1) `pythiaRuntimeEnv` picked the first alphabetical `desktop-*` profile (the tokenless pre-fork `desktop-api.multica.ai` leftover) and gave up, so the engine spawned with `MULTICA_REQUIRED=1` but NO runtime URL/token → every `oracle.what_if` raised instantly → placeholder narrative returned as HTTP 200 → Go labelled it `oracle`: **"真实引擎推演" reports were 100% filler, zero LLM calls ever reached `/api/runtime/llm-call`**. Now derives the profile from daemon's target URL (`activeDesktopProfileName()`) with a validating full-scan fallback. (2) Honesty-law hole closed: engine flags internal fallback with `synthetic: true`, Go relabels to `synthetic_oracle_failover` (mock-data badge shows). (3) Round control: default 10 → 3 (`defaultIssueForecastRounds`), issue text may pin rounds (「推演5轮」 phrase parsing in `packages/views/issues/utils/forecast-rounds.ts`, tight-adjacency on purpose), create-flow no longer awaits the 45s SSE. (4) Cancellable deliberation: shared AbortController registry (`pythia-forecast-trigger.ts`) + ⏹ stop button on the PythiaPanel in-progress card; server SSE loop already honours disconnect and persists partial rounds. (5) Flag-off honesty: with the flag off the guard 404s every pythia endpoint (this bit the user as the misleading「推演未在 90 秒内返回」stuck panel — real cause was a vanished `experimental_pref` row, see Known Stability Surfaces); PythiaPanel now renders「实验室已停用」and LabProgressCard renders idle, never stuck/failed. (6) Post-ship verify caught layer two: `/api/runtime/llm-call` 401'd every engine call — bare `jwt.Parse` rejects the desktop PAT the engine carries; auth now branches mul_ → PAT validation (16e869b66). (7) Oracle round timeout 4s → 120s + trigger window 90s → 5min — real LLM rounds take 25-58s, the old budget cut every genuine round to failover (d09c8e4d2). (8) Engine `import math` function-local shadowing crashed the success path with UnboundLocalError the moment real predictions arrived — upstream latent bug (83f2e6400). Known UX gap (0.5.105 candidate): the lab page (/experimental/pythia) shows no in-progress state for issue-initiated deliberations; progress lives only in the issue-detail PythiaPanel (same-renderer sessionStorage). Notes: [`.omc/0.5.104-ship-2026-09-08.md`](.omc/0.5.104-ship-2026-09-08.md). Prior: 0.5.103 (board count/body parity + Pythia engine auto-start — its auto-start is what surfaced fix (1)), 0.5.102 (board-dedupe hotfix; 3-layer dedupe stays as defense in depth), 0.5.101 (upstream port MUL-7050 + MUL-7016 B.4; MUL-7008 deferred), 0.5.100 (3 MULs + ship-chain verify fatal), 0.5.97 (lab result rendering overhaul). Full per-release history: `.omc/<ver>-ship-<date>.md`.
>
