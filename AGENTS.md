...
>
> **Current release: 0.5.108** (2026-09-17; **同日第二波上游价值移植批, 9 提交 5 port / 4 skip**). 移植账本 [`.omc/upstream-sync-2026-09-17.md`](.omc/upstream-sync-2026-09-17.md): (1) MUL-7358 opencode ≥1.1.54 注册地板（≤1.1.53 内嵌 Bun 无视 TMPDIR/TMP/TEMP, 每 run 向共享 /tmp 泄 4-8 MB 非内容寻址文件, #8392 实测 2960 文件/11.16 GiB/根分区 99%; 唯一拦截点=注册时拒绝; fork MinVersions 表新增该行+表头"协议地板 vs 宿主损害地板"注释）。(2) #8517 空 mention picker 放键（Searching.../No results 态 ArrowUp/Down/Enter/Tab `return false` 还给宿主编辑器, 保换行/提交快捷键/焦点导航; "全禁用行保持 inert"半 fork 无对应概念未移植）。(3) MUL-7312 desktop 半: PATH 兜底**只追加缺失目录不前置**（前置 /usr/local/bin 会让陈旧系统 Node 12 遮蔽 nvm/fnm 恢复的 Node, 打断 shebang CLI 的 --version 探测; 新 `path-fallback.ts` + 测试）——fork macOS 主场景直接受益。(4) MUL-7458 Copilot 折叠错误双侧修复: daemon `executeAndDrain` drain 循环单缓冲 + `sealPendingLocked`（旧双缓冲 ticker flush 固定"先全 thinking 后全 text"封帧, 且 tool_use/tool_result/error 的 seq 在锁外取——ticker 竞争可在事件到达与 append 之间插入更晚的 seq, 服务端按 seq 排序即重排交错回复; 现每次 seq 分配与 append 同锁）+ FE settled 渲染 canonical `message.content`（preface+middle 归过程折叠, 尾部 transcript 文本被 canonical 答案替换, Copy 用 canonical, 空 content 旧行回退可见 timeline）+ MessageFooter 对 no_response/空内容关 Copy。(5) MUL-7312 codebuddy 半: `CODEBUDDY_CODE_DISABLE_BACKGROUND_TASKS=1` 末位追加（os/exec Windows 大小写不敏感 last-wins 去重下压过任何 custom_env 覆盖; fork finalize 是内联 switch, 守卫落成新 case）+ `system/task_started|task_progress|task_updated|task_notification` 出现即 failed（后台 Bash 不再假报成功——adapter 首个 result 后关 stdin, 跨轮 task_notification 无法观测）。SKIP 证据在案: MUL-6734（纯 Windows）、MUL-7450（fork 无 integrations-tab, lark 已 status 闸）、MUL-7019（fork 无 wecom）、MUL-7461（守卫对象 MULTICA_TASK_CONFIG_ROOT fork 不存在）。同日运维: 打包 app 采纳的外部救场 PG 被外部 SIGTERM 后僵尸 8.5h（/health 不探库恒 ok, DB 宕机映射 daemon 401 + snapshot 404, 全天 0 个 5xx）, pg_ctl 恢复 + 4 个产品缺口记录在账本。顺延: MUL-7409 结构化 409 指引（方案已归档, blockers 必须按 fork `IsAgentRuntimeEligibleForGC` 谓词改写否则文案撒谎）、codebuddy stderr-resume 拒绝半、autopilot 鉴权族/MUL-7344/心跳 lease/模型目录/ACP ghost（承 0.5.107）。Notes: [`.omc/0.5.108-ship-2026-09-17.md`](.omc/0.5.108-ship-2026-09-17.md). Prior: 0.5.107 (labs env+插件修复 + 上游 242 提交价值移植批), 0.5.106 (claude_science_lab 技能链路修复), 0.5.105 (实验室审计修复批), 0.5.104 (Pythia 真实 LLM 推演链), 0.5.103 (看板计数断链 + Pythia auto-start), 0.5.102 (board-dedupe hotfix). Full per-release history: `.omc/<ver>-ship-<date>.md`.
>
