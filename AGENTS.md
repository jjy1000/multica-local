...
>
> **Current release: 0.5.106** (2026-09-11; **claude_science_lab 技能链路修复 + 沙盒会话复用 + 动效补强 — 对标上游 open-science 差距评估后的修复批次**). (1) **技能打包修复**: build-claude-science-manifest.mjs 原把技能目录拷成 `SKILL.md` 目录（真正文嵌套一层），安装器读目录 EISDIR 静默存空 → 294 个技能 Content 全空；`loadManifestAsset` 兼容目录布局（读 `<dir>/SKILL.md`），vendor+resources 两棵树就地拍平（294 文件/0 目录），构建脚本产出规范布局，重装即修复空 Content 行 + frontmatter description 解析入库. (2) **技能端点重做**: `/api/experimental/claude-science/skills` 原挂认证组外（0.5.105 H1 同类）+ 按已废弃保留 slug 工作区查询 + 被安装器 `Hide()` 全隐藏 → 三重失效恒空列表；现入 `claude_science_lab` flag 组（flag-off → 404）、caller∪legacy 工作区、新查询 `ListClaudeLabSkillSummariesByWorkspace` 放行本 lab 自见行、响应带 description；新增 `skills/{name}` 详情（DB 优先/manifest 兜底）与 `{name}/file` 配套文件端点（扩展名白名单+穿越防护+256KB 上限）. (3) **按需技能加载**: CLI 新增 `claude-science-runtime skills` / `skill <name> [--file]`、execute `--session`；两个 builtin SKILL.md 改为「目录发现→按需加载」流. (4) **沙盒会话复用**: execute 接受 `session_id` 复用根会话工作目录（文件级状态延续，notebook 式）；工作目录锚定会话行 ID，顺带修复产物 RelPath 用行 ID 而物理目录用随机 UUID 导致产物字节端点恒 404 的老 bug；过期根会话 410. (5) **前端**: Knowledge tab 重做（搜索/分类/计数/展开正文+配套文件），Code tab 编辑器化（textarea + 新会话/续跑双按钮，替代硬编码 hello-world），forecast 概率图 pathLength draw-in + 末点脉冲、tab 切换 crossfade、预测概率条宽度过渡（全部复用 motion tokens + 尊重 reduced-motion）；4 locale ×9 新键. (6) **测试**: 新增 4 组 Go 测试（嵌套布局读取/安装内容修复/端点+穿越防护/会话续跑+产物字节回归），全量 go test + typecheck + lint + vitest 绿. Notes: [`.omc/0.5.106-ship-2026-09-11.md`](.omc/0.5.106-ship-2026-09-11.md). Prior: 0.5.105 (实验室审计修复批 — H1 semantica 端点入认证组+flag 闸、H3 swarm_topology 诚实删除、H2 三清单两端钉住、M1-M6 全修), 0.5.104 (Pythia 真实 LLM 推演链 — env profile/PAT/轮超时/窗口/math 八层修复, oracle 23.9s 真实推演), 0.5.103 (看板计数断链 + Pythia 引擎 auto-start), 0.5.102 (board-dedupe hotfix, 三层去重保留), 0.5.101/0.5.100 (upstream ports + ship-chain verify fatal), 0.5.97 (lab result rendering overhaul). Full per-release history: `.omc/<ver>-ship-<date>.md`.
>
> **In progress — NOT shipped: 0.5.107** (2026-09-13; **labs 环境 + `user_*` 插件链路审计修复批**). (1) **沙盒密钥泄漏面（实测确证）**: `claude_science_runtime.go` 的 execute 从未设 `cmd.Env`，Go 语义下 nil Env 静默继承服务端全量环境——agent 写的 snippet 一次 `os.environ` 就能读到 daemon 注入的 `MULTICA_API_TOKEN`、JWT secret、`DATABASE_URL`、`ANTHROPIC_*` 和真实 `HOME`(→ `~/.multica/profiles/<name>/config.json`)，而 `python3 -I` 只隔离 site-packages、不隔离环境；现走 `runtimeSessionEnv` 显式白名单（同仓插件路径 `pluginRuntimeEnv` 早已这么做，实验室沙盒是那条未加固的兄弟路径）。(2) **Labs 插件开关点了弹回**: Switch 绑 `user_plugin.status`、写的是 `experimental_pref`，两套状态无耦合 → 关着的 lab 显示为开（section + plugin shell 两处），已改绑真实源。(3) **关 flag 仍能执行**: `RunUserPlugin` 原先只查 `status=="active"`（新建行即 active、flag 默认 off），现补 `experimentalFlagEnabled` 闸 → 409。(4) **batch 层 lab↔assignee 锁缺口**: 该 switch 只硬编码 `mythos_swarm`/`swarm_topology`，0.5.86 拓宽后的 4 内置 lab + `user_*` 显式带 `assignee_*` 时静默持久化矛盾数据而单条 PATCH 400——同型缺陷第 3 次（0.5.21→0.5.60 swarm、0.5.86→本批），现走 `assigneeLabLockError` 单一事实源，平价测试的键集**从 catalog 派生**。(5) 产物摄入 32 MiB 上限（原整文件进内存算 sha256，snippet 可控 OOM）+ 字节端点 `Content-Disposition: attachment`/`nosniff` 对齐 0.5.18 F-006。(6) `SetPanicFlagContext` 零生产者 → panic 自动拉黑 flag 的安全网此前永不触发，现接到 `Registry.RunInstall`。(7) 删 `ScanUserPluginDir` 死磁盘发现路径 + 修 `catalog.go` 失真注释。(8) `server/CLAUDE.md` §Labs 的 mutex 与新增「handler 子进程必须显式设 `cmd.Env`」跨切规则改写，镜像重生成。**版本号未动**: `apps/desktop/package.json` 与已装应用仍 0.5.106，ship 链未跑。全记录（含 3 条被撤销的审计结论 + 9 条已知残留）: [`.omc/0.5.107-dev-2026-09-13.md`](.omc/0.5.107-dev-2026-09-13.md).
>
