...
> **Current release: 0.5.81** (2026-08-27, shipped from `epic/0.5.81-lab-ux-consistency`; shipped & verified: `/Applications/Multica.app` = 0.5.81, official cold-start verify PASS ~2s). Recent chain: **0.5.80** — labs fullscreen-takeover fix; workspace-singleton release-suppression guard (arm in EVERY nav path into `/experimental/*`, consume in layout unmount). **0.5.81** — labs task-issue-first UX batch: shared `<IssueBreadcrumb/>` back-link in all lab views; lab⇄issue deep-link round trip (`labRunHref()` out, `?run=` + `useDeepLinkRun` back with rAF retry & one-shot dedupe; receivers mythos/pythia/ExecutionLogSection wired; code-canvas & plugin-shell reasoned-SKIP pending a user-plugins runs endpoint); auto-dispatch defaults consolidated (`a3ffde24e`) pinned by catalog contract test. **Gate-integrity**: DB-backed `go test ./...` exposed nine unregistered concurrent-index builds (262–272) → MUL-6288 registry backfill `e7f0aa69c`; final gates must be uncached + `./...` + `DATABASE_URL` exported. Packaged-app ops facts: the app bundles Postgres.app binaries (`~/Library/Application Support/Multica/pg`+`pgdata`, owns :5432 with its process tree), the cold-start verifier reads expected version from the PRIMARY checkout pre-merge (use `EXPECTED_VER=`), and OrbStack's `pocketbase` container maps host :8090 which collides with the packaged server port. WL2/WL3 research reports: [`.omc/wl2-timesfm-vendoring-report.md`](.omc/wl2-timesfm-vendoring-report.md) · [`.omc/wl3-causal-source-binding-report.md`](.omc/wl3-causal-source-binding-report.md). Open decision gate unchanged: upstream inbox architecture (MUL-6632) — until decided, inbox-family upstream commits stay SKIP-DIVERGENCE. Historical release notes archived at `.omc/_legacy/release-notes-archive.md` (0.5.12→0.5.41); 0.5.43–0.5.81 notes at `.omc/release-notes-0.5.{43..81}.md`; load-bearing contracts live in root [`CLAUDE.md`](CLAUDE.md) Known Stability Surfaces / Active Contracts / Fork-Applicable HIGH Vuln Contracts.
