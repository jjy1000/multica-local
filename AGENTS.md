...
>
> **Current release: 0.5.107** (2026-09-17; **labs 环境 + `user_*` 插件链路修复 + 上游 v0.4.38→v0.4.44 价值移植批**). **(A) labs/插件修复批**（记录 [`.omc/0.5.107-dev-2026-09-13.md`](.omc/0.5.107-dev-2026-09-13.md), 含 3 条被撤销的审计结论 + 已知残留）: (1) 沙盒密钥泄漏面（实测确证）: `claude_science_runtime.go` execute 从未设 `cmd.Env`，nil Env 静默继承服务端全量环境（`MULTICA_API_TOKEN`/JWT secret/`DATABASE_URL`/`ANTHROPIC_*`/真实 HOME），现走 `runtimeSessionEnv` 显式白名单。(2) Labs 插件开关绑真实源（原绑 `user_plugin.status` 写 `experimental_pref` 点击弹回）。(3) `RunUserPlugin` 补 `experimentalFlagEnabled` 闸 → 409。(4) batch 层 lab↔assignee 锁走 `assigneeLabLockError` 单一事实源，平价测试键集从 catalog 派生。(5) 产物摄入 32 MiB 上限 + 字节端点 `Content-Disposition: attachment`/`nosniff`。(6) `SetPanicFlagContext` 接到 `Registry.RunInstall`；删 `ScanUserPluginDir` 死路径。** (B) 上游移植批**（242 提交窗口分诊 + 执行记录 [`.omc/upstream-sync-2026-09-16.md`](.omc/upstream-sync-2026-09-16.md)）: (7) MUL-7355 claude fallback 用量按 response ID 去重（虚高计费修复）+ MUL-6943 Fable 5.1 定价（`claudeVersionEnd` 端锚防 4x 缓存读错账）。(8) MUL-7423 cached worktree git 身份隔离（`repocache/identity.go` + config.lock 原子换名编辑器）+ 工具输出 UTF-8 安全截断 + daemon 日志全 ID（AST 守卫钉住 taskLog 不重复字段）。(9) MUL-7326 重复入队竞态折叠（`ErrDuplicatePendingTask` + 丢竞者并入 queued winner）+ `GetTaskStatus` 瘦身查询（去可选源链解析）。(10) **MUL-3428 安全**: Auth/DaemonAuth 剥离客户端 `X-Agent-ID`/`X-Task-ID`（成员可重放 task-runs 里的 id 对伪造 agent 身份；仅 `mat_` 分支可写）+ 731af7ccd runtime 删除主动推 `runtime_gone` 帧（hub/notifier/sweeper/workspace delete 全接线）。(11) MUL-7028 会话过期真正终结会话（`sessionExpired()` 先弃凭据再清态 + session-cleanup + 桌面 logout/expiry 分离**保 daemon 不停**）+ MUL-7436 滑动续期（SAFE 请求半程换发 + `POST /api/auth/refresh` + FE 单飞 + `AUTH_TOKEN_TTL` 60s 下限钳）；**sid 双 cookie CSRF 层与 CloudFront 有意不移植**（fork CSRF 绑原始 token 值，该竞态窗口不存在）。已知残留: executeAndDrain 完成竞态的尾消息窗口（测试侧已错峰）、handler 侧 runtime-gone 的 Redis loopback 升级差 handler.go 一个 `DaemonRuntimeGone` 槽位、`mergeLegacyRuntimes`/`DeleteRuntimeProfile` 删除路径未接推送。顺延下批（方案已备）: autopilot 鉴权族（MUL-6951→7090→7108-B, mig 290 `created_by`）、MUL-7344 claim 快照、571e61128 心跳 lease、模型目录发现、MUL-7120 ACP ghost。Notes: [`.omc/0.5.107-ship-2026-09-17.md`](.omc/0.5.107-ship-2026-09-17.md). Prior: 0.5.106 (claude_science_lab 技能链路修复 + 沙盒会话复用 + 动效补强), 0.5.105 (实验室审计修复批 — H1 semantica 端点入认证组+flag 闸、H3 swarm_topology 诚实删除、H2 三清单两端钉住、M1-M6 全修), 0.5.104 (Pythia 真实 LLM 推演链 — env profile/PAT/轮超时/窗口/math 八层修复, oracle 23.9s 真实推演), 0.5.103 (看板计数断链 + Pythia 引擎 auto-start), 0.5.102 (board-dedupe hotfix, 三层去重保留), 0.5.101/0.5.100 (upstream ports + ship-chain verify fatal), 0.5.97 (lab result rendering overhaul). Full per-release history: `.omc/<ver>-ship-<date>.md`.
>
