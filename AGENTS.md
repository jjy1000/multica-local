...
>
> **Current release: 0.5.105** (2026-09-08, shipped `<bump-sha>`; **实验室审计修复批次 — 外部 AI 对实验室功能的 H1-H3/M1-M6 审计逐条实证后修复, H3 经用户裁决「诚实删除」**). (1) **H1 安全修复**: `GET /api/experimental/semantica/decisions` 原挂在认证组外的裸 router 上且零 flag 短路 — `requestUserID` 裸读 `X-User-ID` 头, 未认证调用方可伪造成员身份（比审计判断的「有成员 ACL 兜底」更严重）; 现移入认证组 + `RequireExperimentalFlag("semantica")`（flag-off → 404）, 回归钉 `semantica_decisions_test.go`. (2) **H3 swarm_topology 诚实删除**: service/swarm(1933 LOC) + swarm_run/swarm_routes/product_swarm_coordinator + CLI `cmd_swarm` + 桌面路由/视图/图组件 + 4 语言 swarm.json 全链删除; `TopologySpec` 本就永远写 `{}` 必然跑不起来; catalog 保留 `Frozen:true` 墓碑(后继 mythos_swarm), DB 四表原地不动(forward-only); swarm_gc 的通用半边(0.5.60 orphan 清扫)重构为 `experimental.ResourceGC` 继续驱动, swarm-run 归档/回收半边随运行时消亡; 新增 `IsFrozen` + `frozenLabSourceBindError` 退役闸 — 新绑定 Create/Update 400、Batch per-issue skip. (3) **H2 清单可校验**: `TestCatalogFlagViewParity`(Go) 与 FLAG_ROUTE_SUFFIX 精确键集断言(TS) 两端相向钉住, 第 11 个 lab 落地必炸一侧测试; web LABS 补 causal-graph 卡片(此前路由存在却只能深链进入); lab-i18n-keys 侧边栏钉住 7/9 → 9/9(低优顺带). (4) **M2**: user_plugin PUT status 进程内立即生效 — disabled/deleted 即时注销, 不再等下次 boot; `toast.delete_success` 的 reclaim 计数首次被真正使用. (5) **M1**: plugin-shell-view + issue-labs-section 约 25 处硬编码中文进 experimental/issues locale(×4 语言), 含 `toLocaleString("zh-CN")` 改运行时 locale. (6) **M6 技能幻影命令**: multica-pythia 10 动词 → 4 真实动词(ask/events/predictions/view/scorecard/models/links 为幻影, 引擎 HTTP 面存在但 CLI 从未包装); multica-working-on-issues 的 property 全节重写为 upstream-only(fork 无 CLI/handler/core API, 比审计说的更死), source-map 幻影行退役. (7) **M5 文档批**: lifecycle map 加 2026-09-08 附注修正全部过期计数(catalog 8→10、install handler 5→7、ProxyRoutes 2→4、MythosService.Stop 有 shutdown 调用者等); server/CLAUDE.md AutoDispatch 「仅 claude_science_lab」→ 三件套(pythia/timesfm/causal_graph); 根 CLAUDE.md+AGENTS.md 的「cmd/server 因 dbstartup 不可构建」替换为实测 BUILD_OK; **docs-sync 新增第 6 类检查: 两文件 Current release 版本行必须一致 — 上线即抓到 0.5.104 vs 0.5.102 真实漂移**. Notes: [`.omc/0.5.105-ship-2026-09-08.md`](.omc/0.5.105-ship-2026-09-08.md). Prior: 0.5.104 (Pythia 真实 LLM 推演链 — env profile/PAT/轮超时/窗口/math 八层修复, oracle 23.9s 真实推演), 0.5.103 (看板计数断链 + Pythia 引擎 auto-start), 0.5.102 (board-dedupe hotfix, 三层去重保留), 0.5.101/0.5.100 (upstream ports + ship-chain verify fatal), 0.5.97 (lab result rendering overhaul). Full per-release history: `.omc/<ver>-ship-<date>.md`.
>
