...
> **Current release: 0.5.84** (2026-08-28, shipped from `epic/0.5.72-followups`; `/Applications/Multica.app` = 0.5.84, cold-start verify pending ship-mac). **0.5.84 — Causal Graph P0 fix batch** (post-ship audit closure; single migration **281 = `causal_graph_maintenance_state`** singleton row). Closes all 6 P0 bugs: `FLAG_ROUTE_SUFFIX` row for `causal_graph` (silent-link trap closed 2nd time, regression-pinned at issue-labs-section.test.tsx); `createCausalSuggestion` probes `FindCausalEdgeBetween` any-status before INSERT (never-nag now enforced REST-side too, tombstone law mirrored from manual path); `TouchCausalNode` callers on every issue/comment/task hot path via `Recorder.RefreshForIssue` + bulk `RefreshCausalNodesForIssue` CTE (30-day stale TTL no longer hides Tier A chains); maintenance ticker DB-anchored via mig 281 + boot sweep (daily restarts no longer starve GC); edge rendering split by `status` (active solid / suggested muted-dashed / rejected ghost-opacity via `resolveEdgeTone(type, status)`); ring layout pre-computes `ringLengths` before insertion (N=3 spread 0/2π/3/4π/3 as designed). **Two new standing laws** (Active Contracts #9): every new lab flag key MUST also get a `FLAG_ROUTE_SUFFIX` row (issue-labs-section.tsx; same trap semantica 0.5.81 + causal 0.5.83 hit twice); every proposer endpoint needs the server-side never-nag probe (`FindCausalEdgeBetween` ANY status). **Plus the TouchCausalNode callsite contract**: every issue/comment/task hot path MUST refresh `last_observed_at` via `Recorder.RefreshForIssue` — hook points: `handler/issue.go::UpdateIssue` L3228, `handler/comment.go::CreateComment` L1351, `service/task.go::{enqueueIssueTask, enqueueMentionTask, CompleteTask}`. Cooperation ≈ 40-45% unchanged (write side ~90%, **read side still 0%**). **0.5.85 next** (P1 headline): daemon claim-response subgraph injection (`handler/daemon.go::ClaimTaskByRuntime` — read-once at claim time, depth≤2, top-20 nodes/edges, skip Tier D + status≠active, hard cap 4000 tokens + 200ms ctx deadline + silent fallback on timeout/error). P2: Tier C Pythia hypothesis→evidence closure, historian/verifier automation, mention-flow doc notes, evolver window bound. Ship log: [`.omc/0.5.84-ship-2026-08-28.md`](.omc/0.5.84-ship-2026-08-28.md); release notes [`.omc/release-notes-0.5.84.md`](.omc/release-notes-0.5.84.md); audit [`.omc/0.5.83-post-ship-verification.md`](.omc/0.5.83-post-ship-verification.md). Recent chain: **0.5.83** — WL3 issue causal graph + hidden agent team: migs **276-280** (revive `issue_dependency` additive; `causal_node`/`causal_edge` with type CHECKs duplicated into client zod; 279 dedup + lock widen; **280 reject-tombstone**); flag key `causal_graph` VERBATIM; trust ladder A>B>C>D (D = suggested-only, confidence ≤ 0.5); `TestAllSourcesContainsCausalGraph` pin; `multica-causal-graph-curator` skill; nightly evolver + maintenance ticker (Start() NO ctx); client `IssueCausalGraphIcon` → `/experimental/causal-graph`. **0.5.82** — WL2 TimesFM 2.5 forecasting lab: flag key `timesfm` VERBATIM; route split `/experimental/timesfm-lab` view vs bare `/experimental/timesfm` REST proxy; engine-down honesty (503, no synthetic envelope; `provenance` never stripped); ICP-1 records-only; `timesfm_oracle` leader + `multica-timesfm` skill; mig 275 taught the lock-CHECK widen. **0.5.81** — WL1 labs task-issue-first UX: `<IssueBreadcrumb/>`; ICP-3 deep-link round trip (wall-clock 2s poll budget). **Gate-integrity**: final gates must be uncached, sequential `go test ./...` with `DATABASE_URL` exported (0.5.83: concurrent CPU load flakes wall-clock timing tests — serial rerun 47 packages, 0 FAIL; 0.5.84 ship gate 5 packages 0 FAIL on first try, no flakes). Packaged-app ops facts: the app bundles Postgres.app (`~/Library/Application Support/Multica/pg`+`pgdata`, owns :5432 with its process tree); the cold-start verifier reads expected version from the PRIMARY checkout (correct post-merge-back, use `EXPECTED_VER=` pre-merge); OrbStack's `pocketbase` container maps host :8090 (restart policy set to `no` 2026-08-27). **0.5.80's nav law stands**: every navigation path into `/experimental/*` MUST arm the workspace-singleton release-suppression token. **WL5 / 0.5.85 next**: P1 daemon claim-response subgraph injection (`handler/daemon.go::ClaimTaskByRuntime`); P2 Tier C Pythia closure, historian/verifier automation, mention-flow doc notes, evolver window bound. Open decision gate unchanged: upstream inbox architecture (MUL-6632) — until decided, inbox-family upstream commits stay SKIP-DIVERGENCE. Historical release notes archived at `.omc/_legacy/release-notes-archive.md` (0.5.12→0.5.41); 0.5.43–0.5.84 notes at `.omc/release-notes-0.5.{43..84}.md`; load-bearing contracts live in root [`CLAUDE.md`](CLAUDE.md) Known Stability Surfaces / Active Contracts / Fork-Applicable HIGH Vuln Contracts.
