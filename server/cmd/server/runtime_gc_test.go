...
func TestRuntimeGC_KeepsTerminalTaskHistory(t *testing.T) {
	...
	// Fork model: agent.runtime_id is NOT NULL (deliberately not ported from
	// upstream's migration 309 DROP NOT NULL), so an agent "moves off" a runtime
	// by being re-pointed at a different runtime, not by setting NULL. Bind the
	// fixture agent to a separate home runtime so the GC candidate is agent-less.
	agentHomeRuntimeID := createRuntimeGCFixtureRuntime(t, ctx, "history-agent-home")
	agentID := createRuntimeGCFixtureAgent(t, ctx, agentHomeRuntimeID, "history")
...
func TestRuntimeGC_RuntimeLockRejectsConcurrentEnqueue(t *testing.T) {
	...
	enqueueErr := <-writerDone
	var pgErr *pgconn.PgError
	if !errors.Is(enqueueErr, pgx.ErrNoRows) && !(errors.As(enqueueErr, &pgErr) && pgErr.Code == "23503") {
		t.Fatalf("concurrent enqueue error = %v, want pgx.ErrNoRows (upstream ownership fence) or SQLSTATE 23503 (fork: no lock_task_owner_rows fence, agent_task_queue_runtime_id_fkey ON DELETE CASCADE rejects the insert)", enqueueErr)
	}
	var tasks int
