...
func main() {
	...
	// name a flag via runtime.SetPanicFlagContext so the recovered
	// panic knows which experimental feature to mark broken. When no
	// flag is named the sentinel still re-panics, just without a
	// blacklist write — a non-experimental panic should crash the
	// process normally.
	defer func() {
		if r := recover(); r != nil {
			if flagKey, ctx, ok := experimental.PopPanicFlagContext(); ok {
				if err := experimental.MarkBroken(flagKey, experimental.ReasonPanic, ctx); err != nil {
					slog.Error("safety: failed to mark flag broken after panic", "flag", flagKey, "error", err)
				} else {
					slog.Warn("safety: experimental flag marked broken by panic", "flag", flagKey, "reason", ctx)
				}
			}
			// Re-panic so the process still terminates with a real
			// stack trace; the on-disk blacklist is the side effect
			// we want preserved.
			panic(r)
		}
	}()
	...
	// MULTICA_*_FILE knob.
	//
	// The actual flag Service is constructed below after the DB pool is
	// ready, because the experimental.UserPrefProvider needs `queries`.
	// Nothing in this file between here and that block reads `flags`, so
	// execenv.SetFeatureFlags is also deferred to that block.
	...
	// Reassemble the feature flag Service so the per-user experimental
	// provider sits between env overrides (Ops kill switch) and the YAML
	// static rules. Order matters: env > user_pref > yaml > catalog default.
	// 0.3.6 labs UI ships behind this provider.
	...
	if inner := flags.Provider(); inner != nil {
		if cp, ok := inner.(*featureflag.ChainProvider); ok {
			// Rebuild chain with experimental.UserPrefProvider inserted
