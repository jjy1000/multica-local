...
const (
	...
	// time-bounded exit. The 2h default is conservatively above any
	// reasonable "queued behind a long-running task" window for an online
	// runtime, so we don't expire legitimately-pending work, while still
	// draining the historical 87k autopilot backlog within ~24h once
	// enabled. Self-hosted deployments with low task-concurrency runtimes
	// may need to raise this — the desktop fork sets
	// MULTICA_TASK_QUEUED_TTL=5m via server-manager.ts to fail-fast on
	// stale queued tasks (single-user, not self-hosted).
...
// longer than queuedTTL without ever being claimed. See defaultTaskQueuedTTL
// for the rationale on the 2h default + MULTICA_TASK_QUEUED_TTL override.
// Capped to queuedExpireBatchSize per tick so a big backlog can't monopolise
// the DB.
