...
func init() {
	...
	// 0.3.8: experimental Labs-gated subcommands (Claude Science,
	// Pythia). Cobra requires that any GroupID referenced by a
	// subcommand be declared on the root command via AddGroup,
	// otherwise Execute() panics with `group id 'X' is not
	// defined`. We register the group here once so both
	// claudeScienceCmd and pythiaCmd can attach to it.
	rootCmd.AddGroup(&cobra.Group{
		ID:    groupExperimental,
		Title: "Experimental Services — optional, Labs-gated integrations with their own subprocess managers.",
	})
