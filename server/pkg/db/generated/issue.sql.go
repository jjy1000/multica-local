...
INSERT INTO issue (
    ...
    $16, $17
) RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, lab_source
`
...
func (q *Queries) ListIssuesByAssignee(ctx context.Context, arg ListIssuesByAssigneeParams) ([]Issue, error) {
	rows, err := q.db.Query(ctx, listIssuesByAssignee,
		arg.WorkspaceID,
		arg.AssigneeType,
		arg.AssigneeID,
		arg.Limit,
...
const listIssuesByLabSource = `-- name: ListIssuesByLabSource :many
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, lab_source FROM issue
WHERE workspace_id = $1
  AND lab_source = $2
...
type ListIssuesByLabSourceParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	LabSource   pgtype.Text `json:"lab_source"`
	Limit       int32       `json:"limit"`
}
...
func (q *Queries) ListIssuesByLabSource(ctx context.Context, arg ListIssuesByLabSourceParams) ([]Issue, error) {
	rows, err := q.db.Query(ctx, listIssuesByLabSource, arg.WorkspaceID, arg.LabSource, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
