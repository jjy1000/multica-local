...
const getAutopilotByWorkspaceAndTitle = `-- name: GetAutopilotByWorkspaceAndTitle :one
SELECT id, workspace_id, title, description, assignee_id, status, execution_mode, issue_title_template, created_by_type, created_by_id, last_run_at, created_at, updated_at, assignee_type, project_id FROM autopilot
WHERE workspace_id = $1 AND title = $2
LIMIT 1
`

type GetAutopilotByWorkspaceAndTitleParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	Title       string      `json:"title"`
}
...
func (q *Queries) GetAutopilotByWorkspaceAndTitle(ctx context.Context, arg GetAutopilotByWorkspaceAndTitleParams) (Autopilot, error) {
	row := q.db.QueryRow(ctx, getAutopilotByWorkspaceAndTitle, arg.WorkspaceID, arg.Title)
	var i Autopilot
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
