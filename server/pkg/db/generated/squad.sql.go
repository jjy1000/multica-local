...
const listVisibleSquadMembers = `-- name: ListVisibleSquadMembers :many
SELECT id, squad_id, member_type, member_id, role, created_at FROM squad_member
WHERE squad_id = $1
  AND NOT EXISTS (
...
func (q *Queries) ListVisibleSquadMembers(ctx context.Context, squadID pgtype.UUID) ([]SquadMember, error) {
	rows, err := q.db.Query(ctx, listVisibleSquadMembers, squadID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listVisibleSquadsByWorkspace = `-- name: ListVisibleSquadsByWorkspace :many
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions FROM squad
...
func (q *Queries) ListVisibleSquadsByWorkspace(ctx context.Context, workspaceID pgtype.UUID) ([]Squad, error) {
	rows, err := q.db.Query(ctx, listVisibleSquadsByWorkspace, workspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
