...
-- Assignee-scoped issue lookup. Used by the Squad detail "Tasks" tab to render
-- the issues a given squad currently owns (`assignee_type='squad'`). The
-- single-assignee filter is intentional — list/board surfaces needing
-- multi-assignee grouping should go through `ListIssues` with `assignee_ids`
-- or `assignee_types` instead, which avoid the per-actor round-trip.
--
-- Defense-in-depth: the workspace_id predicate makes the tenant invariant a
-- SQL-layer guarantee rather than relying on the loader layer (mirrors
-- ListIssues / GetIssueInWorkspace contracts).
SELECT * FROM issue
...
INSERT INTO issue (
    ...
    sqlc.narg('stage'), sqlc.narg('lab_source')
) RETURNING *;
...
UPDATE issue SET
    ...
    lab_source = sqlc.narg('lab_source'),
    updated_at = now()
WHERE id = $1
RETURNING *;
...
-- name: ListIssuesByLabSource :many
-- Lab-scoped issue listing (0.3.29 Claude Lab tab data source).
-- Returns all issues in the workspace tagged with a given lab_source
-- (e.g. 'claude_science_lab'), newest first. Used by the Claude Lab
-- `Plan` tab and the lab-runtime `execute` payload picker.
-- The workspace_id predicate is the tenant guard — same defense-in-
-- depth pattern as ListIssues; callers must additionally verify
-- membership in the loader layer.
SELECT * FROM issue
WHERE workspace_id = $1
  AND lab_source = $2
