...
-- Migration 143 added message_kind (default 'message', app-validated no CHECK
-- constraint). For CompleteTask empty-output rows we now write
-- message_kind='no_response' so the frontend renders the localized
-- "no text reply this turn" placeholder bubble (see PR-4 in
-- .omc/release-notes-0.3.4.md). The MigrationDefault 'message' preserves
-- all legacy / user / assistant rows.
INSERT INTO chat_message (chat_session_id, role, content, task_id, failure_reason, elapsed_ms, message_kind)
VALUES ($1, $2, $3, sqlc.narg(task_id), sqlc.narg(failure_reason), sqlc.narg(elapsed_ms), sqlc.narg(message_kind))
RETURNING *;
