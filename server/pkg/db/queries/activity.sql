...
-- The outer query re-sorts ascending so every caller keeps the chronological
-- contract it already had. Capping with ORDER BY created_at ASC instead
-- discards the newest rows, which made a busy issue's timeline appear to stop
-- at some point in the past with no indication anything was missing
-- (MUL-5492).
SELECT * FROM (
