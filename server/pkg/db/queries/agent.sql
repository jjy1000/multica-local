...
-- name: ListVisibleAgentsByWorkspace :many
-- ListAgents filtered by the experimental-resource-lock overlay. GUI
-- agent pickers + agent picker on issue-create flow hit this so a
-- hidden lab agent does not surface as a selectable assignee.
SELECT * FROM agent
WHERE workspace_id = $1
  AND archived_at IS NULL
  AND NOT EXISTS (
    SELECT 1 FROM experimental_resource_lock l
    WHERE l.resource_type = 'agent'
      AND l.resource_id = agent.id
      AND l.hidden = true
  )
ORDER BY created_at ASC;
...
-- Find-or-create helper for PR 6's claude_science installer. Migration
-- 046 added UNIQUE(workspace_id, name) to agent; this query lets the
-- installer reuse an existing row across re-installs / re-toggles
-- rather than INSERT and fail the constraint.
SELECT * FROM agent
...
-- >>> MUL-4195 / MUL-4304 / MUL-4015: at-least-once comment delivery
-- (Renumbered from upstream 150+ queries; copied verbatim except for nothing.)
--
-- Note: CreateAgentBuilder / DeleteSystemAgentByID / runtime_connected_apps were
-- dropped from this cherry-pick — the local fork has not migrated the `system_key`
-- column on agent or `runtime_connected_apps` on agent_task_queue. The agent-builder
-- subsystem is out of scope for 0.3.7.
