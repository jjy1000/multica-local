...
-- name: GetAutopilotByWorkspaceAndTitle :one
-- 0.3.27 B4: idempotent lookup for install-time autopilot provisioning.
-- The autopilot table does NOT have a unique constraint on title, so an
-- INSERT-on-conflict pattern is unsafe; instead callers do SELECT-first
-- then Create if missing. Used by install_constitution_agent and
-- install_agent_self_opt to bring the autopilot rows online the first
-- time the user enables the flag.
SELECT * FROM autopilot
