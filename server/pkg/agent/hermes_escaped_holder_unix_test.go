...
// hermesEscapedHolderBound is what the daemon's shutdown may cost: a forced
// shutdown whose own cost is one cmd.Wait(), bounded by the 10s cmd.WaitDelay —
// an escaped holder keeps the pipes open, and Wait closing their parent ends at
// the delay is what frees the readers — plus slack for a loaded CI runner. This
// fork has no upstream hermesReaderDrainGrace machinery; the WaitDelay is the
// whole budget here.
var hermesEscapedHolderBound = 10*time.Second + 6*time.Second
...
// within the test's shutdown bound — the 10s cmd.WaitDelay plus slack, which is
// what makes the forced shutdown terminate at all. The escaped process itself
...
// goroutine before the forced shutdown runs at all, so the deferred cleanup is
// the only thing that closes the pipes and joins the readers.
//
...
// Messages. The Result alone does not prove it: Messages closes only once the
// goroutine's deferred cleanup — which joins the readers — has run, and the
// early-failure path sends the Result before that cleanup starts.
