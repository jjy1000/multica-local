...
func (b *claudeBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	go func() {
		...
		for scanner.Scan() {
			...
			switch msg.Type {
			case "assistant":
				b.handleAssistant(msg, msgCh, &output, usage, seenUsage)
			case "user":
			...
			case "result":
				...
				// GH #6402: a context-exhausted turn arrives with BOTH is_error
				// and terminal_reason on the frame. The structured reason names
				// the condition without depending on the CLI's prose, so it wins
				// over is_error — see claudeTerminalReasonFailure. It also means
				// the run produced NO answer: clear output so the provider's
				// notice can never be published as the agent's conclusion.
				if terminalReasonError := claudeTerminalReasonFailure(msg.TerminalReason, msg.ResultText); terminalReasonError != "" {
					finalStatus = "failed"
					finalError = terminalReasonError
					output.Reset()
				} else if msg.IsError {
					finalStatus = "failed"
...
func (b *claudeBackend) handleAssistant(msg claudeSDKMessage, ch chan<- Message, output *strings.Builder, usage map[string]TokenUsage, seenUsage map[string]struct{}) {
