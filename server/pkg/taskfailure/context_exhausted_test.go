...
// NOTE: upstream's TestNormalizeDaemonReasonUpgradesPromptTooLong is not ported.
// NormalizeDaemonReason does not exist in this fork (see the #6366 commit): the
// fork ships daemon + server in the same .app, so the mixed-version fleet that
// shim upgrades does not exist here, and the classification itself reaches every
