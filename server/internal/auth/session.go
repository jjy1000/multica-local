...
//
// Localized fork: upstream binds the CSRF token to a `sid` claim so a rotated
// auth cookie keeps old CSRF values valid. This fork never adopted the sid
// machinery — its CSRF stays bound to the token value, which is safe here
// because renewal is confined to SAFE methods (each request composes the auth
// cookie and the CSRF header from one cookie-jar snapshot, so a rotation can
// never invalidate the very request that triggered it) and bearer-token
// clients carry no CSRF at all.
...
const (
	...
	// packages/core/platform/session-renewal.ts.
...
// that is not one of our own UI session JWTs. PATs (mul_) and agent task
// tokens (mat_) authenticate fine at the middleware but must never be
// exchanged for an interactive session.
...
//
// Only `exp` and `iat` move. `sub`, `email` and `name` are copied forward
// verbatim, which is what makes renewal free of any database read: the
// claims were signed by this server, so they are already authoritative for
// exactly the fields the auth middleware reads back out.
