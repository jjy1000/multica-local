...
func sessionClaims(t *testing.T, exp time.Time) jwt.MapClaims {
	t.Helper()
...
// still gets logged out. Half the TTL is the contract, so pin both sides of
// the boundary.
...
func TestSessionRenewCheckInterval_TracksTTL(t *testing.T) {
	cases := []struct {
		...
		{"10m", time.Minute, "manual test plan TTL"},
...
func TestSessionRenewCheckInterval_StaysAboveTheClientFloors(t *testing.T) {
	// packages/core/platform/session-renewal.ts floors at 5 seconds.
...
func TestRenewSessionToken_CarriesIdentityForward(t *testing.T) {
	...
	old := sessionClaims(t, time.Now().Add(24*time.Hour))
	...
	for _, claim := range []string{"sub", "email", "name"} {
...
func TestParseSessionToken_RejectsExpiredToken(t *testing.T) {
	expired := signSession(t, sessionClaims(t, time.Now().Add(-time.Minute)))
...
func TestSlidingSession_ContinuousUseOutlivesTTL(t *testing.T) {
	...
	ttl := AuthTokenTTL()
	claims := sessionClaims(t, time.Now().Add(ttl))

	// Use the app every 10 days for 120 days — well past the old 30-day wall.
	now := time.Now()
	...
	if claims["sub"] != "user-1" {
		t.Errorf("identity changed across renewals: %q", claims["sub"])
	}
}
