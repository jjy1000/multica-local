...
func autopilotHandler(
	...
	return func(ctx context.Context, in HandlerInput) (HandlerResult, error) {
		...
		// Advance next_run_at so the detail UI / list-page "Next run"
		// column points at the slot AFTER the one we just fired rather
		// than staying pinned on the dispatched occurrence. Anchored on
		// max(planTime, now) so a clock skew between DB time and app
		// time cannot re-point the column at the slot that just fired
		// (MUL-3749). ComputeNextRun uses the local clock which is fine
		// for a display column; the canonical scheduling decision
		// remains the DB-time-anchored NextOccurrencesUTC call above.
		_ = advanceTriggerNextRun(
			ctx, queries, trigger.ID,
			trigger.CronExpression.String, trigger.Timezone.String,
			in.PlanTime,
		)
...
// advanceTriggerNextRun computes the next cron occurrence strictly
// after max(planTime, now()) and writes it into autopilot_trigger.
// next_run_at. The display-only column is fed by both the scheduler's
// post-dispatch advance AND the trigger create/update handlers; the two
// paths must agree on the same cron helper so a re-save in the UI never
// jumps the column backward relative to what the scheduler last wrote.
//
// Errors are intentionally swallowed — the canonical record is the
// autopilot_run row and a stale next_run_at is a UX nuisance, not a
// correctness issue. The scheduler has already leased the plan, so a
// failure here must not be promoted into a lease-affecting error.
func advanceTriggerNextRun(
	ctx context.Context,
	queries *db.Queries,
	triggerID pgtype.UUID,
	cronExpr, tz string,
	...
	next, err := service.NextOccurrenceAfterUTC(cronExpr, tz, anchor)
	if err != nil {
		return err
	}
