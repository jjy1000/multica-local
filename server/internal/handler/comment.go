...
type CommentResponse struct {
	...
	// create/edit responses: old clients ignore it. A saved comment whose
	// mention was blocked (no invoke permission, target unavailable, runtime
	// offline, attribution fail-closed) now reports that here instead of
	// silently dropping the trigger, so the client can show "comment posted,
	// but N targets were not triggered".
...
// mention (MUL-4525 §2). TargetID is the id the user mentioned — the agent id,
// or the SQUAD id for a squad mention — so the client correlates it back to the
// mention it rendered without the server echoing a private target's
// name/owner. ReasonCode is the stable, enumeration-safe admission reason.
...
type commentAgentTrigger struct {
	...
	Squad  *db.Squad
	// AlreadyPending is set by the compute path when an existing queued/dispatched
	// task for the same (issue, agent) pair is detected (MUL-4195). The enqueue
	// path then either folds the new comment into the queued task via
	// mergeCommentIntoPendingTask, or defers to completion reconciliation when
	// the only active task is already dispatched/running.
	AlreadyPending bool
}
...
func (h *Handler) PreviewCommentTriggers(w http.ResponseWriter, r *http.Request) {
	...
	for _, trigger := range triggers {
		// MUL-4195: an AlreadyPending trigger represents a comment that will
		// fold into an existing queued task rather than enqueue a new one.
		// Surface only the genuinely-new triggers in the preview so the UI
		// shows "no new agents will wake" for plain follow-ups that just
		// re-coalesce. (Same de-duplication the legacy hasPending guard did.)
...
func (h *Handler) CreateComment(w http.ResponseWriter, r *http.Request) {
	...
	// MUL-4015: set when the agent posts with X-Task-ID. Persists on the
	// comment row so leader→worker mention hops can resolve the originating
	// member via resolveOriginatorFromTriggerComment.
		...
		if taskIDHeader := r.Header.Get("X-Task-ID"); taskIDHeader != "" {
			...
			if parseErr == nil {
				...
				if err == nil && task.IssueID.Valid && uuidToString(task.IssueID) == uuidToString(issue.ID) {
					// MUL-4015: stamp the task id as comment.source_task_id so
					// the leader→worker mention hop preserves the originating
					// member across ResolveOriginatorFromTriggerComment.
					sourceTaskID = task.ID
					// MUL-4417: reply-thread authorization. Replaces the
					// single-trigger-only check with taskCoversReplyParent —
					// parent_id may be the trigger OR any coalesced comment
					...
					if task.TriggerCommentID.Valid || len(task.CoalescedCommentIds) > 0 {
						if !taskCoversReplyParent(task, parentID) {
							...
							writeError(w, http.StatusConflict,
								"parent_id must equal this task's trigger or coalesced comment id (one of: "+formatReplyParentList(accepted)+")")
							return
						}
					}
	...
	resp.TriggerOutcomes = h.triggerTasksForComment(r.Context(), issue, comment, parentComment, authorType, authorID, "", suppressAgentIDs)
...
func (h *Handler) triggerTasksForComment(ctx context.Context, issue db.Issue, comment db.Comment, parentComment *db.Comment, actorType, actorID string, originatorUserID string, suppressAgentIDs []pgtype.UUID) []CommentTriggerOutcome {
...
// (MUL-4525 §2 round-2). The enqueue step is keyed by executing agent id
// (deduped at resolve time), so two mentions that resolve to the same agent
// share one result — the fanout to per-target outcomes happens later in
// commentTriggerOutcomes, so a coalesced run never drops a named target's
// outcome. queued / coalesced / deferred are success-shaped (the run was
// handled, no duplicate task); only a real enqueue failure is blocked.
//
...
// context ran (MUL-4525 §2 round-3).
...
// (MUL-4525 §2 round-2). Outcomes are later fanned from these to every explicit
...
// (MUL-4525 §2 round-2): a target that resolved to an executing agent takes
// that agent's enqueue status, so several mentions coalescing into one run each
// still get their own outcome; a terminal target (blocked / self-suppressed)
// carries its own status. A target whose executing agent has no enqueue result
// — because the composer suppressed (unchecked) it — yields no outcome, since
...
func commentTriggerOutcomes(targets []commentMentionTarget, enqueued map[string]commentEnqueueResult) []CommentTriggerOutcome {
	...
	for _, t := range targets {
		if t.ExecAgentID != "" {
			...
			// context executed (MUL-4525 §2 round-3, Elon review).
...
// mentions that will NOT trigger if the comment is posted as-is (MUL-4525 §2
// round-2). A resolvable/executing target instead appears in the preview
// `agents` list, so only terminal blocked targets surface here.
...
// but do NOT touch runtime_mcp_overlay / runtime_connected_apps (the local
// fork's TaskService does not yet expose BuildRuntimeMCPOverlayForMerge, and
// runtime_connected_apps was never migrated). Re-stamping the originator
// alone keeps the at-least-once guarantee; the overlay stays as the
// original task's, which is the same behaviour the upstream fallback
// commentRoutingEscalationDelay takes.
//
// actorType/actorID identify the comment author so the merge can re-stamp the
...
func (h *Handler) mergeCommentIntoPendingTask(ctx context.Context, issue db.Issue, trigger commentAgentTrigger, newTriggerCommentID pgtype.UUID, actorType, actorID string) commentMergeResult {
	...
	row, err := h.Queries.MergeCommentIntoPendingTask(ctx, db.MergeCommentIntoPendingTaskParams{
		...
		NewRuntimeMcpOverlay: nil, // local 0.3.7: not exposed on TaskService yet.
	})
	if err != nil {
		if isNotFound(err) {
...
func (h *Handler) computeCommentAgentTriggers(ctx context.Context, issue db.Issue, content string, parentComment *db.Comment, actorType, actorID string, opts commentTriggerComputeOptions) ([]commentAgentTrigger, []commentMentionTarget) {
	...
	// Implicit routing (assignee / thread-parent / conversation) does NOT add
	// targets: the user did not name those, so a no-route there is not a
	// silent no-op.
	...
	if actorType == "member" && !h.commentMentionsOthersButNotAssignee(content, issue) &&
		...
		if agent, err := h.Queries.GetAgentInWorkspace(ctx, db.GetAgentInWorkspaceParams{
			...
			// MUL-4195: a queued task for this (issue, agent) already exists.
			// Mark AlreadyPending so the enqueue path folds the new comment into
			// the queued task via mergeCommentIntoPendingTask instead of dropping it.
			hasPending, _ := h.hasPendingTaskForIssueAndAgent(ctx, issue.ID, agent.ID, opts)
	...
	mentionTriggers, mentionTargets := h.computeMentionedAgentCommentTriggers(ctx, issue, content, parentComment, actorType, actorID, opts)
...
func (h *Handler) computeAssignedSquadLeaderCommentTrigger(ctx context.Context, issue db.Issue, content string, parentComment *db.Comment, authorType, authorID string, opts commentTriggerComputeOptions) (commentAgentTrigger, bool) {
	...
	// P1 mitigation (memory multica-0.3.2): a plain-text member comment
	// ("完成了吗", "继续吧", "卡哪了") on a squad-assigned issue already
	// wakes the squad leader via this branch (the
	// `commentRoutesViaMention` gate above only suppresses when there's
	// an explicit @mention in the comment or its parent). The follow-
	// through — leader evaluating "completed?" and either re-dispatching
	// or closing the issue — lives in the Operating Protocol section 6 of
	// squad_briefing.go, NOT here. What we DO add here is a guard: skip
	// waking the leader when a plain-text member comment would land on a
	// leader with a still-running task, because the leader is already
	// busy and a second concurrent run would be a wasted dispatch.
	...
	// MUL-4195: when a queued task already exists for the squad leader, set
	// AlreadyPending so the enqueue path folds this comment into it instead
	// of dropping the deliberate user instruction.
...
// computeMentionedAgentCommentTriggers parses @agent / @squad mentions from
// comment content and returns a trigger for each mentioned target plus the
// per-target BLOCKED outcomes for mentions that will not fire (MUL-4525 §2).
// Blocked outcomes come only from the explicit-mention path — the implicit
// routing fallbacks (assignee, thread parent, conversation) were never named
// computeMentionedAgentCommentTriggers resolves which agents a comment triggers
// (deduped by executing agent), plus the per-target list for every EXPLICIT
// @agent / @squad mention (MUL-4525 §2 round-2). Targets come only from the
// explicit-mention path — the implicit routing fallbacks (assignee, thread
// parent, conversation) were never named by the user, so a no-route there is
// not a silent no-op.
//
// The invoke gate is evaluated BEFORE any archived/runtime state is read, so a
// caller who cannot invoke a private target only ever sees the generic
// invocation_not_allowed and can never enumerate the target's existence or
// state from the reason code.
func (h *Handler) computeMentionedAgentCommentTriggers(ctx context.Context, issue db.Issue, content string, parentComment *db.Comment, authorType, authorID string, opts commentTriggerComputeOptions) ([]commentAgentTrigger, []commentMentionTarget) {
	...
	// squad-leader mention UPGRADE an already-added plain @agent trigger for
	// the same agent — the leader task is a strict superset (it sets
	// is_leader_task and squad_id so the daemon injects the squad briefing),
	// so the merged run must carry that role regardless of mention order
	// (MUL-4525 §2 round-3, Elon review).
	...
	for _, m := range mentions {
		if m.Type == "squad" {
			...
			// most recent task on this issue was a leader task for this squad,
			// so we do not re-fire the leader from its own @mention. The
			// outcome must reflect reality, not assume success (MUL-4525 §2
			// round-3, Elon review): report `deferred` only when a real
			...
			if err != nil {
				// Do not reveal whether the squad id exists: the generic
				// invocation_not_allowed is enumeration-safe.
			...
			// MUL-4195: dedup but mark AlreadyPending so the enqueue path can fold
			// this mention-triggered comment into the existing queued task instead
			// of silently dropping the user's instruction.
			hasPending, _ := h.hasPendingTaskForIssueAndAgent(ctx, issue.ID, leaderID, opts)
		...
		// this @mention-triggered comment into the existing queued task instead
		// of silently dropping the user's instruction.
		hasPending, _ := h.hasPendingTaskForIssueAndAgent(ctx, issue.ID, agentUUID, opts)
...
// resolved (MUL-4525 §2 round-2). Tracked separately from the execution
// triggers because several mentions can resolve to the same executing agent
// (e.g. @Agent A and @Squad S whose leader is A), and each still needs its own
...
func (h *Handler) UpdateComment(w http.ResponseWriter, r *http.Request) {
	...
	// Cancel any active tasks whose planned batch contains this comment so the
	// agent does not run with the now-stale content. We capture the cancelled
	// rows so the trailing retriggerCancelledTaskSurvivors pass can re-route
	// every surviving queued task (its planned input would otherwise vanish
	// and resume against a stale session — the Exiting-loop failure mode after
	// editing or deleting a trigger comment).
	...
	if oldContent != comment.Content && triggerIssue != nil {
		var parentComment *db.Comment
		...
		resp.TriggerOutcomes = h.triggerTasksForComment(r.Context(), *triggerIssue, comment, parentComment, actorType, actorID, "", suppressAgentIDs)
	}
...
func (h *Handler) DeleteComment(w http.ResponseWriter, r *http.Request) {
	...
	// We capture the cancelled rows so the trailing
	// retriggerCancelledTaskSurvivors pass can re-route every surviving queued
	// task (its planned input would otherwise vanish and resume against a
	// stale session — the Exiting-loop failure mode after deleting a trigger).
...
// authorized to reply under (MUL-4417). A comment-triggered task may reply to
// its trigger comment OR to any earlier comment that was folded into the same
// run while it had not yet started (coalesced_comment_ids). Assignment-
// triggered tasks (no TriggerCommentID, no coalesced set) accept any parent
// because they were not started by a specific comment. Used by CreateComment
// to reject resumed-session --parent drift while still admitting the legitimate
// case where a session resumes and replies to a coalesced mid-run comment.
//
// A nil/invalid parentID is accepted only when the task has NO accepted
// parent (assignment-triggered). When the task has any accepted parent, an
// empty parent is rejected — top-level comments are not allowed because the
// task is mid-conversation.
func taskCoversReplyParent(task db.AgentTaskQueue, parentID pgtype.UUID) bool {
	if task.TriggerCommentID.Valid {
		...
		// No trigger and no coalesced set — assignment-triggered; accept any
		// parent (including empty top-level comments).
		return true
	}
	for _, id := range task.CoalescedCommentIds {
		if id.Valid && uuidToString(parentID) == uuidToString(id) {
			return true
		}
	}
	return false
...
// formatReplyParentList renders the accepted reply parents as a
// comma-separated UUID list for the 409 error body. Keeps the actionable
// message short even when a coalesced set is large.
func formatReplyParentList(ids []pgtype.UUID) string {
	if len(ids) == 0 {
		return "(none)"
	}
...
// retriggerCancelledTaskSurvivors re-routes every active queued task whose
// planned batch (trigger_comment_id + coalesced_comment_ids) was just cancelled
// by a comment edit or delete, excluding the comment that triggered the cancel
// itself. Closes the Exiting-loop failure mode where the user's edit/delete
// cancelled the immediately-active task but left its queued siblings with a
// stale planned batch — those siblings would otherwise resume against a stale
// session, write "Exiting. Resolved thread, complete." and the server would
// happily wake the next sibling in turn.
//
// We replay every surviving comment chronologically and enqueue it against
// exactly the agent whose task was cancelled, so the normal MUL-4195
// AlreadyPending / merge path coalesces a same-(issue, agent) burst into one
// fresh run instead of re-creating the storm. Local-fork simplification: we
// use the comment's own AuthorID as the originator since the local comment
// create path does not yet plumb a separate originatorUserID parameter — the
// only loop-prone case this guards against (member → assigned agent, which is
// the user's "Exiting." storm) sits squarely on that path.
//
// Local note (0.3.7): resolveOriginatorFromTriggerComment is the official
// path that supports agent-authored comments being re-routed through their
// @mention. That function is not on the local TaskService yet (the @mention
// loop guard is 0.3.8 scope), so we intentionally drop agent-authored
// comments here and let them keep their original routing. Member-authored
// comments cover the dominant failure mode that produced the Exiting storm.
func (h *Handler) retriggerCancelledTaskSurvivors(ctx context.Context, issue db.Issue, cancelled []db.AgentTaskQueue, excludedCommentID pgtype.UUID) {
	...
	for commentID := range targetsByComment {
		...
		// Local 0.3.7 simplification: skip non-member authors — agent-authored
		// comments would need originator resolution that lands in 0.3.8.
