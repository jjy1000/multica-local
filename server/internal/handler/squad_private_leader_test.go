...
// TestChildDone_SquadPrivateLeader_PlainMemberWakesLeader verifies that
// when a plain member completes a child issue whose parent is assigned
// to a private-leader squad, the leader IS woken. Child-done no longer
// re-checks leader invocation permission (MUL-4063 / GH #4928): the
// parent was already assigned to the squad — which passed the
// invocation gate — so waking that squad's own leader to advance the
// next stage is a coordination handoff, not a fresh invocation. This
// mirrors the ungated agent-parent path (triggerChildDoneAgent); agent
// and squad child-done now follow one path.
...
// TestChildDone_SquadPrivateLeader_AgentActorWakesLeader is the core
// MUL-4063 regression: an AGENT (a squad worker) closes a child under a
// private-leader squad parent, and the child's completing agent has NO
// human originator who could invoke the private leader. This is the
// exact process-squad pipeline shape that used to strand — the removed
// canEnqueueSquadLeader gate failed closed here — while a
// direct-to-leader-agent parent advanced fine. The leader must now be
// woken.
func TestChildDone_SquadPrivateLeader_AgentActorWakesLeader(t *testing.T) {
	...
	// Squad creator is the agent owner — squad_creator_scope requires the
	// creator to be the one dispatching tasks to it, so the agent owner
	// (not the workspace owner) is the squad's creator here. Mirrors the
	// pattern privateAgentTestFixture uses for the agent row.
	var squadID string
	if err := testPool.QueryRow(ctx, `
		INSERT INTO squad (workspace_id, name, description, leader_id, creator_id)
		VALUES ($1, 'Private Leader ChildDone AgentActor Test', '', $2, $3)
		RETURNING id
	`, testWorkspaceID, agentID, ownerID).Scan(&squadID); err != nil {
		t.Fatalf("create squad: %v", err)
	}
	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM squad WHERE id = $1`, squadID)
	...
	// Clear the leader task the assign enqueued so the child-done wake is
	// the only thing that can create one.
	testPool.Exec(ctx, `DELETE FROM agent_task_queue WHERE issue_id = $1`, parent.ID)
	...
	// The worker agent's running task on the CHILD. Its originator is the
	// plain member — who cannot invoke the private leader — proving the
	// wake no longer depends on the completer being able to invoke the
	// leader.
	...
	// Worker agent moves the child to done (agent actor via
	// X-Agent-ID/X-Task-ID).
	w = httptest.NewRecorder()
