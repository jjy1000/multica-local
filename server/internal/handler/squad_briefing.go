...
6. **Handle plain-text follow-ups from a human (P1 fix, memory multica-0.3.2).**
   A member (human) comment without an explicit @mention markdown can
   still wake you as the squad leader — see comment.go:1419 in the
   server. When you wake up and see a follow-up like "完成了吗",
   "继续吧", "卡哪了", "改一下 X", or any other plain question about
   the issue's state, you MUST:
   - Read the issue's last 3-5 comments to see whether a delegated
     worker has already completed its task.
   - If a worker completed and the deliverable is missing/incorrect,
     re-delegate with an @mention to the same worker (or a different
     one if the original failed twice). State the specific gap in ONE
     sentence — do not restate the issue.
   - If a worker completed and the deliverable is present and correct,
     set the issue status to ` + "`" + `done` + "`" + ` via
     ` + "`" + `multica issue update <id> --status done` + "`" + ` and post a one-line
     summary comment. Then record ` + "`" + `multica squad activity ... no_action --reason "issue closed after human confirmation"` + "`" + `
     and exit. Without this branch the human sees "still running" in the
     GUI forever (memory multica-0.3.2 root cause: task:completed invalidates
     only chat caches; see packages/core/realtime/use-realtime-sync.ts P0 fix).
   - Never silently no_action without re-evaluating the issue state.
     The pre-fix behaviour was "leader wakes, says 'not done', exits" — that
     was a protocol violation against the human's intent.`
...
// briefing (Operating Protocol + Roster + squad Instructions) onto the
// claiming leader agent's Instructions field.
//
// Inputs (gate):
//   - task.IsLeaderTask: stamped at enqueue time (migration 090). True means
//     this is a leader-role task — the only situation a leader briefing is
//     appropriate.
//   - task.SquadID: stamped at enqueue time (migration 127). The squad whose
//     leader this task targets.
//
// The historical alternative — gating on `issue.AssigneeType == "squad"` —
// was wrong: the comment-mention path leaves issue.assignee_type pointing
// at the comment's target (the leader agent), not at the squad. Pre-fix,
// the leader woke with no squad context and degraded into doing the work
// itself (MUL-3724). Keying off task.IsLeaderTask + task.SquadID makes the
// briefing follow the task, not the issue.
//
// Backfill (history safety):
//
//	When task.SquadID is NULL (pre-migration-127 in-flight task), fall
//	back to the legacy issue.AssigneeType=="squad" lookup. This path
//	becomes dead code as soon as all new enqueues populate the column.
//
// Defense-in-depth (always):
//
//	squad.LeaderID == agentID re-check — handles leader-swapped-after-enqueue
//	and squad-hard-deleted-after-enqueue (no FK). No stale briefing is
//	emitted. The agent instructions are never silently mutated when the
//	squad is unknown or has been replaced.
//
// Returns the resolved squad row + a boolean. Caller does the briefing
// injection (append-or-replace semantics; see buildSquadLeaderBriefing and
// the issue-bound dispatch block in daemon.go).
func shouldInjectSquadLeaderBriefing(
	...
	if task.SquadID.Valid {
		...
		if err != nil {
			return db.Squad{}, false // dangling squad_id or unknown workspace → silent skip
		}
		if util.UUIDToString(squad.LeaderID) != agentID {
			return db.Squad{}, false // leader swapped after enqueue → silent skip
		}
		return squad, true
	}

	// Fallback: pre-migration-127 in-flight tasks. The legacy gate matched
	// when the issue itself was squad-assigned. This is the only place this
	// behavior is now expressed; once migration 127 has been running long
	// enough that no in-flight tasks predate it, this branch is dead code.
