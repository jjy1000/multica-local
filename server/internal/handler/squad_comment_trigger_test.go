...
//     of its own, also does NOT enqueue the squad leader when the parent
//     was a member-authored comment that @mentions another agent — the
//     @mention path will fire for the mentioned agent, and the leader
//     would double-trigger (MUL-3744). The leader-skip helper now
//     recognizes the parent-inheritance route and applies the same skip.
//
...
func TestCreateComment_SquadLeaderSkipOnlyInspectsCurrentMention(t *testing.T) {
	...
	//    The leader-skip helper must recognize that the @mention path will
	//    fire (via parent inheritance) and skip the leader so it does not
	//    double-trigger (MUL-3744). Leader must NOT wake up.
	postMemberComment(map[string]any{
		"content":   "any update?",
		"parent_id": parent.ID,
	})
	if got := countQueued(fx.LeaderID); got != 0 {
		t.Fatalf("after plain reply to @mentioned parent: expected 0 leader tasks (MUL-3744 parent-inheritance skip), got %d", got)
	}
}
