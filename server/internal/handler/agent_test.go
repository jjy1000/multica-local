...
// TestCreateAgent_AutoEnqueuesIntroChatSession verifies migration 138 wiring:
// when CreateAgent succeeds with an online runtime, the handler creates a
// chat_session flagged is_agent_intro and enqueues the first chat task for
// the agent's self-introduction turn.
func TestCreateAgent_AutoEnqueuesIntroChatSession(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}

	agentName := "intro-test-agent-" + t.Name()
	t.Cleanup(func() {
		// Cascade: drop tasks first (FK to agent_session), then sessions, then agent.
		testPool.Exec(context.Background(), `
			DELETE FROM agent_task_queue WHERE chat_session_id IN (
				SELECT id FROM chat_session WHERE creator_id = $1 AND title = $2
			)
		`, testUserID, "Intro · "+agentName)
		testPool.Exec(context.Background(),
			`DELETE FROM chat_session WHERE creator_id = $1 AND title = $2`,
			testUserID, "Intro · "+agentName)
		testPool.Exec(context.Background(),
			`DELETE FROM agent WHERE workspace_id = $1 AND name = $2`,
	...
	w := httptest.NewRecorder()
	req := newRequest(http.MethodPost, "/api/agents", body)
	req = withChatTestWorkspaceCtx(t, req)
	testHandler.CreateAgent(w, req)
	if w.Code != http.StatusCreated {
		t.Fatalf("CreateAgent: status %d, body: %s", w.Code, w.Body.String())
	}

	// 1. Chat session must exist with title "Intro · <name>" and is_agent_intro=true
	var sessionID string
	var isIntro bool
	if err := testPool.QueryRow(context.Background(), `
		SELECT id, is_agent_intro FROM chat_session
		WHERE creator_id = $1 AND title = $2
	`, testUserID, "Intro · "+agentName).Scan(&sessionID, &isIntro); err != nil {
		t.Fatalf("intro session not created: %v", err)
	}
	if !isIntro {
		t.Fatalf("is_agent_intro=false on auto-created intro session, want true (migration 138 wiring broken)")
	}

	// 2. A chat task must be enqueued against this session (MUL-4351 path
	//    binds chat_input_task_id = task.id so the daemon claim reads the
	//    implicit intro prompt via the 144 partial index).
	var taskID string
	var chatInputTaskID pgtype.UUID
	if err := testPool.QueryRow(context.Background(), `
		SELECT id, chat_input_task_id FROM agent_task_queue
		WHERE chat_session_id = $1 AND status = 'queued'
	`, sessionID).Scan(&taskID, &chatInputTaskID); err != nil {
		t.Fatalf("intro task not enqueued: %v", err)
	}
	if !chatInputTaskID.Valid {
		t.Fatalf("intro task chat_input_task_id is NULL, want bound (MUL-4351 wiring broken)")
	}
	if uuidToString(chatInputTaskID) != taskID {
		t.Fatalf("intro task chat_input_task_id (%s) != task.id (%s), want equal",
			uuidToString(chatInputTaskID), taskID)
	}

	// 3. session.last_read_at must be set so the IM unread count stays at 0
	//    until the agent's reply lands (avoids a phantom unread badge right
	//    after agent create).
	var lastReadAt pgtype.Timestamptz
	if err := testPool.QueryRow(context.Background(),
		`SELECT last_read_at FROM chat_session WHERE id = $1`, sessionID).Scan(&lastReadAt); err != nil {
		t.Fatalf("load last_read_at: %v", err)
	}
	if !lastReadAt.Valid {
		t.Fatalf("intro session last_read_at is NULL, want NOT NULL (set by EnqueueChatTask -> MarkChatSessionRead fallback)")
	}
}

// TestCreateAgent_NoIntroWhenRuntimeOffline verifies the guard: when the
// chosen runtime is offline, the agent still gets created but no intro
// session is enqueued (the task would just queue forever and show as a dead
// thread).
func TestCreateAgent_NoIntroWhenRuntimeOffline(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}

	agentName := "offline-runtime-test-agent"
	t.Cleanup(func() {
		// Restore the test runtime to online (it might have been flipped to offline
		// below). The harness's setupHandlerTestFixture leaves the test runtime
		// online, so this is a no-op in practice — kept defensive.
		testPool.Exec(context.Background(),
			`UPDATE agent_runtime SET status = 'online' WHERE id = $1`, testRuntimeID)
		testPool.Exec(context.Background(),
			`DELETE FROM chat_session WHERE creator_id = $1 AND title = $2`,
			testUserID, "Intro · "+agentName)
		testPool.Exec(context.Background(),
	...
	// Force the test runtime offline for this test only.
	if _, err := testPool.Exec(context.Background(),
		`UPDATE agent_runtime SET status = 'offline' WHERE id = $1`, testRuntimeID); err != nil {
	...
	req = withChatTestWorkspaceCtx(t, req)
	testHandler.CreateAgent(w, req)
	// CreateAgent still returns 201 even when the runtime is offline — the
	// agent row is created, only the intro is skipped.
	if w.Code != http.StatusCreated {
		t.Fatalf("CreateAgent: status %d, body: %s", w.Code, w.Body.String())
	}

	// No intro session should have been created.
	var count int
	if err := testPool.QueryRow(context.Background(), `
		SELECT COUNT(*) FROM chat_session
		WHERE creator_id = $1 AND title = $2
	`, testUserID, "Intro · "+agentName).Scan(&count); err != nil {
		t.Fatalf("count intro sessions: %v", err)
	}
	if count != 0 {
		t.Fatalf("offline runtime created %d intro sessions, want 0 (skipped when runtime offline)", count)
	}
}
