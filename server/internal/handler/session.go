...
//
// Token is present only for bearer-authenticated callers (Desktop), which
// hold the session as a string and have to persist the new one. A
...
// Desktop (localStorage). They call it at launch, when returning to the
// foreground, and on use; the server owns the "is it time yet" decision so no
// client ever has to read `exp` or trust its own clock.
//
// Only our own UI session JWTs are refreshable. A PAT (mul_) and an agent
// task token (mat_) both authenticate perfectly well at the middleware, and
// each of them must be refused here: neither is an interactive session, and
// exchanging a machine credential for one would turn a scoped, revocable
// token into an unscoped, unrevocable one. An expired JWT cannot reach this
// handler at all (the middleware rejects it), and ShouldRenewSession refuses
// a non-positive remaining lifetime anyway — renewal extends a live session,
// it never resurrects a dead one.
func (h *Handler) RefreshSession(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// Reached by PAT / task-token callers: authenticated, but holding
		// something that is not a session. 400 rather than 401 — the
		// credential is fine, the request is not.
		writeError(w, http.StatusBadRequest, "only interactive sessions can be refreshed")
		return
	}

	// Defense in depth: the middleware set X-User-ID from these very claims,
	// so a mismatch means a header was forged past it. Refuse to mint a
