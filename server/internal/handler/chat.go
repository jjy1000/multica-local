...
func (h *Handler) ListChatSessions(w http.ResponseWriter, r *http.Request) {
	...
	if status == "all" {
		...
		// Migration 135 added last_read_at; use the new IM-style unread count
		// query so the frontend can render a real number instead of a dot.
		// Falls back to last_read_at -> unread_since -> epoch (cold start)
		// so newly-applied migrations on pre-existing chat data still derive
		// a sensible number.
		rows, err := h.Queries.ListChatSessionsByCreatorWithUnreadCount(r.Context(), db.ListChatSessionsByCreatorWithUnreadCountParams{
			WorkspaceID: parseUUID(workspaceID),
			CreatorID:   parseUUID(userID),
		})
		if err != nil {
		...
		for _, s := range rows {
			...
			lastReadAt := ""
			if s.LastReadAt.Valid {
				lastReadAt = timestampToString(s.LastReadAt)
			}
			resp = append(resp, ChatSessionResponse{
				ID:           uuidToString(s.ID),
				WorkspaceID:  uuidToString(s.WorkspaceID),
...
func (h *Handler) MarkChatSessionRead(w http.ResponseWriter, r *http.Request) {
	...
	// Also advance the IM-style cursor so the next ListChatSessionsByCreator
	// derives unread_count against this point instead of the legacy
	// unread_since flag. Migration 135 added last_read_at; TouchChatSessionLastRead
	// is the new cursor-stamping path.
	if err := h.Queries.TouchChatSessionLastRead(r.Context(), session.ID); err != nil {
		slog.Warn("touch chat last_read_at failed",
			"session_id", uuidToString(session.ID), "error", err)
	}
...
type ChatSessionResponse struct {
	...
	// IM-style unread count (assistant messages strictly after last_read_at).
	// Migration 135 added last_read_at; this replaces the boolean has_unread
	// flag with a real number for chat list rendering. Frontend renders the
	// number when > 0, otherwise falls back to has_unread.
...
// PinChatSession flags a chat session so it sticks to the top of the user's
// session list. Migration 139 added chat_session.pinned_at (NULL = unpinned).
// The chat UI sorts by pinned_at DESC within the same workspace so the
// pinned set is the visual "shortcut bar" above the activity-sorted rest.
//
// Authorization: same as MarkChatSessionRead — the session's creator passes
// the private-agent gate at create time but their workspace role may have
// changed since. gateChatSessionForUser re-checks on every action.
func (h *Handler) PinChatSession(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.Queries.PinChatSession(r.Context(), session.ID); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to pin session")
		return
	}

	// Reuse chat:session_updated so the frontend refreshes pinned_at
	// without learning a new event kind.
	resolvedSessionID := uuidToString(session.ID)
	h.publishChat(protocol.EventChatSessionUpdated, workspaceID, "member", userID, resolvedSessionID, protocol.ChatSessionUpdatedPayload{
		ChatSessionID: resolvedSessionID,
...
// UnpinChatSession is the inverse of PinChatSession. Same authorization
// pattern. Idempotent — unpinning an already-unpinned session returns 204.
func (h *Handler) UnpinChatSession(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.Queries.UnpinChatSession(r.Context(), session.ID); err != nil {
