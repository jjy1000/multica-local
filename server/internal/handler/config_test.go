...
func TestGetConfigReportsCdnSignedMode(t *testing.T) {
	...
	// Localized build: cdn_signed is always false because the
	// CloudFront-signing stack was removed with cloud-billing /
	// cloud-runtime. The /api/config field is preserved for back-compat
	// with existing consumers that branch on it.
	cfg := fetch()
	if cfg.CdnSigned {
		t.Fatalf("cdn_signed: want false in localized build, got true")
	}
	if cfg.CdnDomain != "cdn.example.com" {
		t.Fatalf("cdn_domain: want cdn.example.com from mockStorage, got %q", cfg.CdnDomain)
	}
}
