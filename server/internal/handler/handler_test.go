...
func TestMain(m *testing.M) {
	...
	testHandler = New(queries, pool, hub, bus, emailSvc, nil, analytics.NoopClient{}, Config{AllowSignup: true})
	...
	// 0.3.19 P2: register the install handlers the production
	// router binds. Tests that exercise the install endpoint need
	// the same bindings or the dispatcher returns 500 with
	// "no install handler registered for flag".
	if testHandler.ExperimentRegistry != nil {
		testHandler.ExperimentRegistry.RegisterInstallHandler(string(experimental.SourceClaudeScience),
