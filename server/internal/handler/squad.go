...
// canMutateSquad returns true if the caller (workspace member) is allowed
// to mutate a squad. Owners always can; otherwise the caller must be the
// squad creator (squad.creator_id matches member.user_id).
//
// Pre-fix bug: every workspace admin could mutate every squad. This gate
// restricts destructive squad mutations (update / delete / member ops) to
// the squad creator while preserving workspace-owner override.
func (h *Handler) canMutateSquad(member db.Member, squad db.Squad) bool {
	if member.Role == "owner" {
		return true
	}
	// Legacy rows (pre-127) had a zero UUID creator_id — fall back to
	// admin-allowed so old squads remain manageable. The schema now
	// requires a real creator_id on insert.
	if isZeroUUID(squad.CreatorID) {
		return member.Role == "admin"
	}
	return squad.CreatorID.Bytes == member.UserID.Bytes
}

// isZeroUUID reports whether the pgtype.UUID is the all-zero UUID (i.e.
// unset / legacy row). pgx represents a NULL UUID as Valid:false, but a
// zero UUID is Valid:true with Bytes all zero — both should fall back to
// the admin path when gating squad mutations.
...
func (h *Handler) ListSquads(w http.ResponseWriter, r *http.Request) {
	...
	// 0.3.31: hide mythos_swarm's "Mythos Swarm" squad when the flag
	// is off. Mirrors the agent / autopilot / skill filters above;
	// the squad list was the only Labs list endpoint without one
...
func (h *Handler) UpdateSquad(w http.ResponseWriter, r *http.Request) {
	...
	// Squad creator scope: only the squad creator (or a workspace owner) may
	// mutate squad metadata. Without this gate any workspace admin can edit
	// or archive a squad they did not create.
	if !h.canMutateSquad(member, squad) {
		writeError(w, http.StatusForbidden, "only the squad creator or workspace owner may update this squad")
		return
	}
	wsUUID, ok := parseUUIDOrBadRequest(w, workspaceID, "workspace_id")
	if !ok {
...
func (h *Handler) DeleteSquad(w http.ResponseWriter, r *http.Request) {
	...
	if !h.canMutateSquad(member, squad) {
		writeError(w, http.StatusForbidden, "only the squad creator or workspace owner may archive this squad")
		return
	}
...
func (h *Handler) AddSquadMember(w http.ResponseWriter, r *http.Request) {
	...
	if !h.canMutateSquad(member, squad) {
		writeError(w, http.StatusForbidden, "only the squad creator or workspace owner may add members")
		return
	}
	wsUUID, ok := parseUUIDOrBadRequest(w, workspaceID, "workspace_id")
	if !ok {
...
func (h *Handler) RemoveSquadMember(w http.ResponseWriter, r *http.Request) {
	...
	if !h.canMutateSquad(member, squad) {
		writeError(w, http.StatusForbidden, "only the squad creator or workspace owner may remove members")
		return
	}
...
func (h *Handler) UpdateSquadMemberRole(w http.ResponseWriter, r *http.Request) {
	...
	if !h.canMutateSquad(member, squad) {
		writeError(w, http.StatusForbidden, "only the squad creator or workspace owner may change member roles")
		return
	}
...
func commentRoutesViaMention(content string, parentComment *db.Comment, authorType string) bool {
	...
	// Defense-in-depth: shouldInheritParentMentions returns false for nil
	// parents (comment.go:1560), but the explicit guard here prevents a nil
	// dereference on parentComment.Content in case that guard is accidentally
...
// ListSquadIssues returns every issue currently assigned to this squad
// (`assignee_type = 'squad'`). Backs the Squad detail page's Tasks tab.
//
// Reuses the workspace-scoped loader (`loadSquadInWorkspace`) so 404 on
// unknown squads stays consistent with the rest of /api/squads/:id/* — a
// 200 with an empty list is reserved for "squad exists, owns nothing yet".
//
// Limit defaults to 100; the Tasks tab is not paginated today, so this caps
// a pathological squad (e.g. one assigned every bug backlog entry) before it
// can slow down the list render. Bump alongside the tab if that ever shifts.
const squadIssuesListLimit = 100

func (h *Handler) ListSquadIssues(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.Queries.ListIssuesByAssignee(r.Context(), db.ListIssuesByAssigneeParams{
		WorkspaceID:  wsUUID,
		AssigneeType: pgtype.Text{String: "squad", Valid: true},
		AssigneeID:   squad.ID,
		Limit:        squadIssuesListLimit,
	})
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to list squad issues")
		return
	}
	prefix := h.getIssuePrefix(r.Context(), wsUUID)
	out := make([]IssueResponse, 0, len(rows))
	for _, row := range rows {
		out = append(out, issueToResponse(row, prefix))
	}
