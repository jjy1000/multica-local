...
func (h *Handler) attachmentToResponse(a db.Attachment) AttachmentResponse {
	...
	if false {
		// CloudFront signed URLs were removed with the cloud-billing/
		// cloud-runtime stack in the localized build. The block is
		// retained (as a no-op) so the surrounding attachment metadata
...
func (h *Handler) storageURLIsPubliclyReadable(rawURL string) bool {
	if h.Storage == nil {
		// (The h.CFSigner != nil branch was removed in the localized
		// build — CloudFront signed URLs no longer exist. The block is
		// reduced to the storage presence check; raw `a.Url` is now safe
		// to persist whenever storage is configured to serve publicly.)
		return false
	}
...
func (h *Handler) DownloadAttachment(w http.ResponseWriter, r *http.Request) {
	...
	case attachmentDownloadModeCloudFront:
		// CloudFront attachment downloads were removed in the localized
		// build — a persistent URL is served directly by the storage
		// backend instead. Fall through to the persistent-URL branch.
		http.Redirect(w, r, att.Url, http.StatusFound)
		return
	case attachmentDownloadModePresign:
...
func (h *Handler) resolveAttachmentDownloadMode(rawURL string) attachmentDownloadMode {
	...
	if false {
		// attachmentDownloadModeCloudFront is no longer reachable in the
		// localized build — the cloudfront signing path was removed.
		return attachmentDownloadModeCloudFront
	}
