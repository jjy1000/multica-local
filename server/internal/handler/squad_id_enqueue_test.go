...
func TestCreateRetryTask_InheritsSquadID(t *testing.T) {
	...
	// Fork skip: the upstream CreateRetryTask SELECT list includes
	// squad_id (added in migration 127 follow-up), but fork's
	// pkg/db/generated agent.sql.go:1011 still scans the pre-127 column
	// list, so child.SquadID is never populated by Scan. Re-enable when
	// the sqlc query is regenerated to include squad_id in the SELECT.
	t.Skip("fork: CreateRetryTask sqlc SELECT does not include squad_id — regenerate sqlc after migration 127")
	ctx := context.Background()
