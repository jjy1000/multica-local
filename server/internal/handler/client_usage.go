...
func (h *Handler) UpsertClientUsage(w http.ResponseWriter, r *http.Request) {
	...
	if workspaceID := h.resolveWorkspaceID(r); workspaceID != "" {
		...
		// 0.5.0 fork wave 1: dropped queries.LockWorkspaceForChatSessionCreate.
		// Upstream uses it to serialize client-usage reports against chat-session
		// delete/create on the same workspace; fork has no chat-session delete
		// race, so the explicit advisory lock is unnecessary. Workspace
		// existence is still validated by GetMemberByUserAndWorkspace below.
