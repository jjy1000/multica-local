...
func TestCreateComment_WorkerAgentCommentWakesSquadLeader_MUL4015(t *testing.T) {
	...
	// Fork skip: this test seeds rows with `originator_user_id` (migration 129)
	// and expects the `permission_mode` column (migration 130). Both belong
	// to the upstream Composio integration epic (MUL-3715 / MUL-3869 /
	// MUL-3963) which the localized fork explicitly excludes — see
	// .omc/plans/multica-fork-vs-upstream-divergence-map.md and
	// memory/0.3.2-backlog.md. Re-enable once migration 129 lands in fork.
	t.Skip("fork: requires migrations 129 (Composio allowlist) + 130 (agent invocation permission) which are Composio-epic and intentionally not ported")
...
func TestCreateComment_WorkerAgentCommentDoesNotWakeLeader_WhenLeaderTaskPending(t *testing.T) {
	...
	// Fork skip: requires migration 129 `originator_user_id` column
	// (Composio epic, intentionally excluded).
	t.Skip("fork: requires migration 129 (Composio epic) — not ported")
	ctx := context.Background()
	fx := newSquadCommentTriggerFixture(t)
	issueID := uuidToString(fx.Issue.ID)
...
func TestCreateComment_WorkerAgentCommentWakesPrivateSquadLeader_MUL4015(t *testing.T) {
	...
	// Fork skip: requires migration 130 `permission_mode` + `agent_invocation_target`
	// (Composio epic, intentionally excluded). The test's primary value is
	// verifying the private-squad edge case — covered indirectly by the
	// public-squad test in this file, which is also skipped here.
	t.Skip("fork: requires migration 130 (Composio epic) — not ported")
	ctx := context.Background()
