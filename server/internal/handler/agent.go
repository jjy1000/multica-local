...
type AgentTaskResponse struct {
	...
	// BypassPermissions (0.5.18 F-002): the daemon's claude spawn may use
	// auto-approval (--permission-mode bypassPermissions) only when the
	// agent's trust score is at/above agent_trust.BypassPermissionsThreshold.
	// Computed server-side at claim time; the daemon has no DB.
...
func (h *Handler) ListAgents(w http.ResponseWriter, r *http.Request) {
	...
	// `WHERE id NOT IN (...)` clause applied inside
	// ListVisibleAgentsByWorkspace. include_archived callers get
	// the unfiltered list (audit / admin path).
	var agents []db.Agent
	var err error
	if r.URL.Query().Get("include_archived") == "true" {
		agents, err = h.Queries.ListAgents(r.Context(), parseUUID(workspaceID))
	} else if r.URL.Query().Get("include_all") == "true" {
		// 0.3.33: opting out of the visibility filter — used by the
		// install handlers that need to reference lab agents by
		// name (e.g. assignDefaultLabAgent looks up "research").
		agents, err = h.Queries.ListAgents(r.Context(), parseUUID(workspaceID))
	} else {
		agents, err = h.Queries.ListVisibleAgentsByWorkspace(r.Context(), parseUUID(workspaceID))
	}
	if err != nil {
	...
	// 0.3.26 Labs gate: drop agents hidden by *every* Labs flag whose
	// visibility table hides agents and whose flag is currently off. The
	// previous 0.3.17 / 0.3.20 implementation hard-coded two flags and
	// missed `claude_science_lab`, `mythos_swarm`, `pythia_oracle`,
	// `llm_wiki_bridge`, `code_canvas`, and `chat_pin_ui` — agents from
	// those catalogs leaked into AssigneePicker even when their flag was
	// off. We iterate AllFlagKeys() so adding a new flag automatically
	// gets the gate; same defensive fail-open semantics as the autopilot
	// list handler — if a hidden-set lookup errors we log and keep the
	// roster.
	for _, key := range experimental.AllFlagKeys() {
		agents = filterLabsHiddenByDefault(
			r.Context(), h.Queries, agents,
			key, experimental.HideAgent,
			func(a db.Agent) pgtype.UUID { return a.ID },
			"list agents: resolve hidden set failed for "+key,
		)
	}
...
func (h *Handler) CreateAgent(w http.ResponseWriter, r *http.Request) {
	...
	// Migration 138: spin up an auto-created "self-introduction" chat session
	// for the new agent. The intro turn runs server-side (daemon/prompt
	// ChatIntro prefix) and produces the first user-visible reply in the
	// thread. The session is flagged `is_agent_intro=true` so the chat UI
	// renders an "Intro" badge distinguishing it from real conversations.
	//
	// Skipped when no runtime is available: the intro task would just queue
	// and never execute, which is worse than skipping it (the user sees a
	// dead thread).
	if runtime.Status == "online" {
		if err := h.maybeEnqueueAgentIntro(r.Context(), workspaceID, created, ownerID); err != nil {
			slog.Warn("agent intro enqueue failed",
				"agent_id", uuidToString(created.ID),
...
// maybeEnqueueAgentIntro creates a chat_session for a freshly-created agent
// and enqueues the first task — a self-introduction reply driven by the
// daemon's ChatIntro prompt prefix. The session is flagged is_agent_intro
// (migration 138) so the chat UI renders an "Intro" badge distinguishing
// it from a real conversation.
//
// The intro reply has no user message in scope (it's a self-prompted turn
// that the daemon constructs from the agent's name + instructions). Because
// of MUL-4351 (chat_input_task_id ownership), the daemon knows the input
// batch is "the implicit intro prompt", not a user message, and the
// no_response / message-kind defaulting is governed by the daemon's prompt
// layer (see server/internal/daemon/prompt.go ChatIntro).
//
// Best-effort: failures are logged but do not block the agent create
// response. A failed intro enqueue means the user has to send a real
// message to start a conversation — acceptable degradation.
func (h *Handler) maybeEnqueueAgentIntro(ctx context.Context, workspaceID string, agent db.Agent, creatorID string) error {
	session, err := h.Queries.CreateChatSession(ctx, db.CreateChatSessionParams{
		WorkspaceID: parseUUID(workspaceID),
		AgentID:     agent.ID,
		CreatorID:   parseUUID(creatorID),
		Title:       "Intro · " + agent.Name,
	})
	if err != nil {
		return fmt.Errorf("create intro session: %w", err)
	}

	// Migration 138 wiring: the chat UI uses this flag to render the "Intro"
	// badge next to the agent avatar in the session list.
	if err := h.Queries.SetChatSessionAgentIntro(ctx, session.ID); err != nil {
		return fmt.Errorf("flag intro session: %w", err)
	}

	// The intro turn needs the same chat_input_task_id ownership marker the
	// regular chat path uses, so the daemon's claim handler picks it up via
	// the MUL-4351 bound batch read (migration 144 partial index). We
	// reuse TaskService.EnqueueChatTask — it sets chat_input_task_id=task.id
	// in one INSERT, and stamps the user's last_read_at for the new
	// session so the IM unread count stays at 0 until the agent's reply
	// lands.
	if _, err := h.TaskService.EnqueueChatTask(ctx, session, parseUUID(creatorID), false); err != nil {
		return fmt.Errorf("enqueue intro task: %w", err)
	}
	return nil
}
