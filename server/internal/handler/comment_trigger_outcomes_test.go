...
// round-2 must-fix 1 acceptance test (MUL-4525 §2): when a comment names BOTH
// @Agent A and @Squad S whose leader is A, the run is correctly coalesced to
// ONE task, but each explicitly-named target still gets its own outcome —
// execution dedup must not drop a named target's result.
