// Package handler — comment_trigger_outcomes_test.go (0.5.22 MUL-4525 §2 port).
//
// Fork-local port of upstream commit 51325eeab's acceptance tests for the
// `trigger_outcomes` wire shape and the `preview.blocked[]` enumeration-safe
// blocked-reason contract. Upstream uses `service.resolveLabLeader` and
// `service.IsValidLabMode` helpers that the fork does not have; this port
// uses the fork-local helpers (`createCommentTriggerPreviewSquad`,
// `privateAgentTestFixture`) already provided by the trigger-preview test
// fixtures.
//
// What's covered here:
//   - TestCreateComment_MixedMentionSurfacesPartialTriggerOutcomes — Bohan's
//     exact scenario: one invocable agent + one squad whose private leader
//     the author cannot invoke. The comment saves (one blocked mention must
//     not reject it) and reports per-target outcomes.
//   - TestCreateComment_BlockedMentionReasonDoesNotEnumeratePrivateAgent —
//     the enumeration-safety rule: a blocked mention against a private agent
//     and a blocked mention against a nonexistent agent both return the
//     generic invocation_not_allowed, so a blocked reason can never confirm
//     a private agent's existence.
package handler
...
func TestCreateComment_MixedMentionSurfacesPartialTriggerOutcomes(t *testing.T) {
	...
	// owner can VIEW it but cannot INVOKE it (no admin bypass). The squad
	// inherits the leader's private status, so a member @mention of the squad
	// resolves to a leader invocation that the gate denies.
...
// of mention order, and each explicitly-named target still gets its own
// outcome (MUL-4525). The old first-mention-wins dedup could drop the leader
// role when @Agent A came first — this asserts the role independent of order.
...
// multi-squad case: two DIFFERENT squads share the same leader and both are
// @mentioned. The single leader agent runs ONCE carrying one squad's context;
// the other squad's mention folds into that run and is reported coalesced —
// never a second task, and never both reported queued (MUL-4525).
