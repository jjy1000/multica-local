...
// TestLoadConfig_AutoUpdateDefault_CloudOff locks in the localized-build
// behaviour: even a daemon pointed at the hosted cloud must default
// AutoUpdateEnabled to false. The historical opt-in default
// (`isOfficialCloudServer(serverBaseURL)`) was removed during localization
// so telemetry / self-update is uniformly disabled across cloud, self-host,
// and dev deployments. Operators who genuinely want the poller back must
// set MULTICA_DAEMON_AUTO_UPDATE=true (see
// TestLoadConfig_AutoUpdateEnv_ForcesOnForSelfHost).
func TestLoadConfig_AutoUpdateDefault_CloudOff(t *testing.T) {
	stageFakeAgent(t)
	cfg, err := LoadConfig(Overrides{
	...
	if cfg.AutoUpdateEnabled {
		t.Fatalf("AutoUpdateEnabled = true for Multica Cloud server in localized build, want false")
	}
}
