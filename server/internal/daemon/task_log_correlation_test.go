...
// Two task ids whose first eight hex chars collide. That is not a coincidence
// worth guarding against once: task ids are UUIDv7, whose first 32 bits are
// the high half of a millisecond timestamp, so every pair of tasks born
// inside the same ~65s window shares those eight chars. Truncating an id for
// a log field therefore merges concurrent runs into one indistinguishable
// stream exactly when telling them apart matters (MUL-6957).
const (
...
// half of MUL-6957. Two tasks whose ids share a prefix must stay
// distinguishable in the daemon log, so a line can be joined to the task
// JSON, the env-root ownership manifest, and the other run's lines.
...
// taskLogSourceFilesFloor keeps a rename or a file move from turning the
// structural guard below into a vacuous pass (the failure mode MUL-5524 hit).
