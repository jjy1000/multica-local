...
const (
	...
	// real work short, so this is 2h (MUL-6737).
	//
	// Set MULTICA_AGENT_IDLE_WATCHDOG=0 to disable the whole watchdog suite.
	// The in-flight-tool budget (AgentToolWatchdog) is derived from this value
	// in LoadConfig unless MULTICA_AGENT_TOOL_WATCHDOG overrides it; 0 there
	// still means "never force-stop while a tool is in flight" (MUL-3064).
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	// The in-flight-tool budget defaults to the idle budget (MUL-6737): the
	// tool window only ever existed because 30 min was too short for a real
	// build/install/test, and now that the idle budget is sized for the longest
	// legitimate silent step it already covers those. The derivation tracks in
	// BOTH directions: raising MULTICA_AGENT_IDLE_WATCHDOG no longer leaves tool
	// calls pinned to the old ceiling, and lowering it now also lowers the tool
	// budget. MULTICA_AGENT_TOOL_WATCHDOG still overrides for the deliberate
	// "tools may run longer than the model may think" case, and 0 keeps its
	// meaning: never force-stop while a tool is in flight.
	...
	//
	// Localized build: auto-update is permanently disabled. The env-var
	// override is kept so a future re-enable can flip it back on without
	// code changes, but the default is `false` for every deployment —
	// cloud, self-host, dev — and the autoUpdateLoop goroutine in daemon.go
	// is not started regardless of this value.
