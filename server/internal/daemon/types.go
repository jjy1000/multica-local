...
type TaskResult struct {
	...
	// McpToolCalls counts stream tool_use events whose tool name carries the
	// `mcp__` prefix (MCP server invocations). Internal-only — it travels to
	// the server through the usage channel, not serialized on results.
	McpToolCalls int `json:"-"`
}
