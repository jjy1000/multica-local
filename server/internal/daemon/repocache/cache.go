...
func (c *Cache) CreateWorktree(params WorktreeParams) (*WorktreeResult, error) {
	...
	if isGitWorktree(worktreePath) {
		...
		c.applyCoAuthoredBySetting(worktreePath, params)
...
// applyCoAuthoredBySetting reconciles this checkout's hook file with the
// workspace setting.
//
...
// us).
func (c *Cache) applyCoAuthoredBySetting(worktreePath string, params WorktreeParams) {
	enabled := params.CoAuthoredByEnabled
	if published, ok := c.readCoAuthoredByState(params.WorkspaceID); ok {
		enabled = published
	}
	if enabled {
		if err := installCoAuthoredByHook(worktreePath, c.CoAuthoredByStatePath(params.WorkspaceID)); err != nil {
