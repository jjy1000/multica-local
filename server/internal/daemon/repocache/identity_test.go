...
// Rehost note: upstream drives these tests through an existing-checkout fixture
// with two modes (linked worktree vs isolated clone). The fork's CreateWorktree
// only has the linked-worktree path (no IsolatedGitMetadata support), so the
// fixture below always produces a linked worktree and the isolated-clone test
// is reduced to pinning the non-worktree guard directly.
...
// identityCheckoutFixture is a source repository with one tracked file, a
// cache synced from it, and one workdir every checkout in the test targets.
type identityCheckoutFixture struct {
	source  string
	cache   *Cache
	workDir string
}

func newIdentityCheckoutFixture(t *testing.T) *identityCheckoutFixture {
	t.Helper()
	...
	return &identityCheckoutFixture{
		source:  source,
		cache:   cache,
		workDir: t.TempDir(),
	}
...
func (f *identityCheckoutFixture) checkout(t *testing.T, taskID string) *WorktreeResult {
	t.Helper()
...
// taskBranchName mirrors the branch formula CreateWorktree uses for AgentName
// "Agent": agent/{sanitized agent name}/{short task id}.
func taskBranchName(taskID string) string {
	return "agent/agent/" + shortID(taskID)
}
...
func TestCheckoutIdentityMigratesExistingWorktree(t *testing.T) {
	...
	for _, enabled := range []bool{false, true} {
		t.Run(fmt.Sprintf("worktreeConfig=%v", enabled), func(t *testing.T) {
			...
			result := f.checkout(t, firstTaskID)
			if enabled {
...
// TestIsolateIdentitySkipsNonWorktreePath pins the guard upstream exercises via
// its isolated-clone checkout mode, which the fork's CreateWorktree does not
// have: a checkout that is not a linked worktree must be left untouched.
func TestIsolateIdentitySkipsNonWorktreePath(t *testing.T) {
	identityTestEnvironment(t)
	f := newIdentityCheckoutFixture(t)
	plain := t.TempDir()
	if err := isolateWorktreeIdentity(f.cache.Lookup("ws-1", f.source), plain); err != nil {
		t.Fatalf("isolateWorktreeIdentity on a non-worktree path: %v", err)
	}
	if _, err := os.Stat(filepath.Join(plain, "config.worktree")); !os.IsNotExist(err) {
		t.Fatalf("non-worktree path config was written: %v", err)
	}
}
...
func TestCheckoutIdentityConfigLockFailureIsRetryable(t *testing.T) {
	...
	assertCommitIdentity(t, result.Path, "User", "user@example.com")
	// Fork adaptation: the fork's updateExistingWorktree switches to a fresh
	// branch on every reuse (timestamp suffix on collision), so the failed
	// attempt above left the original task branch behind unoccupied. Delete it
	// so the retry below can take it without another same-second collision —
	// upstream instead keeps a checkout that is already on its task branch
	// (MUL-7284, not ported to the fork) and never collides here.
	bare := f.cache.Lookup("ws-1", f.source)
	gitIdentityCommand(t, "-C", bare, "branch", "-D", taskBranchName(firstTaskID))
	if err := os.Remove(lock); err != nil {
		t.Fatal(err)
	}
	f.checkout(t, firstTaskID)
}
...
func TestCheckoutIdentityRejectsForeignWorktree(t *testing.T) {
	...
	err := isolateWorktreeIdentity(f.cache.Lookup("ws-1", f.source), foreign)
...
func TestCheckoutIdentityEnabledWithSharedConfigLocked(t *testing.T) {
	...
	first := f.checkout(t, firstTaskID)
	f.workDir = t.TempDir()
	second := f.checkout(t, secondTaskID)
	bare := f.cache.Lookup("ws-1", f.source)
	...
	// Fork adaptation: upstream drives this through CreateWorktree because its
	// reuse path keeps a checkout that is already on its task branch
	// (MUL-7284's updateExistingCheckout, not ported to the fork). The fork's
	// updateExistingWorktree always switches branches, and `git checkout -b`
	// from a remote-tracking startpoint records the branch's upstream
	// configuration through Git's own common config lock — a Git-internal
	// write outside identity isolation's responsibility. Exercise the identity
	// setup directly (the same call CreateWorktree makes after its branch
	// work) under the held lock instead.
	if err := isolateWorktreeIdentity(bare, first.Path); err != nil {
		t.Fatalf("identity setup for first checkout under a held config lock: %v", err)
	}
	if err := isolateWorktreeIdentity(bare, second.Path); err != nil {
		t.Fatalf("identity setup for second checkout under a held config lock: %v", err)
	}
