...
// revisiting every task exit when it is added. Fork note: the field set is
// trimmed to what the fork's client API can send (no session rollout /
// durable-workdir metadata — those fields were never backported).
...
func (d *Daemon) Run(ctx context.Context) error {
	...
	// Durable replay of terminal complete/fail callbacks that were persisted
	// but never acknowledged (daemon crash, server outage, auth storm). Runs
	// after preflightAuth so recovered reports use the current credential.
	...
	go d.gcLoop(ctx)
	// autoUpdateLoop is intentionally not started: localized build disables
	// GitHub self-update. The function and its tests are retained so a
	// future re-enable doesn't require re-deriving the polling logic.
...
//
// Localized build: upgrade is disabled. The function is preserved as a stub
// so the call sites (autoUpdateLoop, handleUpdate) and their tests compile
// without change; the report-status path will deliver the error back to
// the server / UI unchanged.
func (d *Daemon) runUpdate(targetVersion string) (string, error) {
	d.logger.Info("CLI update disabled (localized build)", "target_version", targetVersion)
	return "", fmt.Errorf("CLI update is disabled in this build")
}
...
func (d *Daemon) sendTerminalTaskReport(ctx context.Context, report terminalTaskReport, schedule []time.Duration) error {
	...
	case terminalTaskReportComplete:
		return d.client.completeTaskWithRetrySchedule(ctx, report.taskID, report.output, report.branchName, report.sessionID, report.workDir, schedule)
	case terminalTaskReportFail:
		return d.client.failTaskWithRetrySchedule(ctx, report.taskID, report.errorMessage, report.sessionID, report.workDir, report.failureReason, schedule)
	default:
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (TaskResult, error) {
	...
	default:
		...
		//
		// Ported from upstream ca47495fc (MUL-7053 #8032). The Hermes
		// annotation that sits next to this line upstream
		// (`annotateHermesProviderUnconfigured`) is NOT in the fork, so only
		// the codex leg is added.
...
// failure against the retired route. It is a CONSTANT, for the same correctness
// reason as upstream's `hermesProviderUnconfiguredHint` — and is worded to
// stay clear of every phrase the resume guards and taskfailure.Classify match,
// since it is persisted in agent_task_queue.error and re-scanned there
// indefinitely.
//
...
//
// Ported from upstream ca47495fc (MUL-7053 #8032, third sub-commit).
