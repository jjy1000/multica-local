...
Full walkthrough in [references/self-test-checklist.md](references/self-test-checklist.md).
`scripts/lab-plugin-smoke.sh` automates create → run inline → run subprocess →
assert artifacts; the delegate leg is manual and only applies to agent-labs with
a `capabilities.leader`.
