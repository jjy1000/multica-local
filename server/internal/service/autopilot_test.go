...
// TestAgentSelfOptimizationAutopilotIDsSeeded locks the cached
// skip-set against drift. If the visibility constants in
// experimental.AgentSelfOptimizationAutopilotIDs change, this test
// fires and forces an explicit acknowledgement in code review (the
// cached map must match — runtime skip behaviour is security-relevant).
func TestAgentSelfOptimizationAutopilotIDsSeeded(t *testing.T) {
	t.Parallel()
	if len(agentSelfOptimizationAutopilotIDs) == 0 {
		t.Fatal("agentSelfOptimizationAutopilotIDs is empty; visibility constants are not loaded")
	}
	want := experimental.AgentSelfOptimizationAutopilotIDs()
	if len(agentSelfOptimizationAutopilotIDs) != len(want) {
		t.Fatalf("cache has %d entries, visibility returns %d", len(agentSelfOptimizationAutopilotIDs), len(want))
	}
	for _, id := range want {
		if _, ok := agentSelfOptimizationAutopilotIDs[pgtype.UUID{Bytes: id, Valid: true}]; !ok {
			t.Errorf("visibility ID %s missing from cached skip-set", id)
		}
	}
}
