...
// agentSelfOptimizationAutopilotIDs is the cached set of autopilot
// UUIDs hidden by the agent_self_optimization flag. Loaded once at
// NewAutopilotService; the IDs are deployment constants in
// experimental/visibility.go and never change at runtime, so caching
// is safe across the process lifetime.
var agentSelfOptimizationAutopilotIDs = func() map[pgtype.UUID]struct{} {
	out := make(map[pgtype.UUID]struct{}, len(experimental.AgentSelfOptimizationAutopilotIDs()))
	for _, id := range experimental.AgentSelfOptimizationAutopilotIDs() {
