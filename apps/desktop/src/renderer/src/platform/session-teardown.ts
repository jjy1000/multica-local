 ...
 *
 * Account-transition reporting is not part of either path: the auth store's
 * status flip drives `DesktopAuthSessionBridge`, which reports both events to
 * the main process the same way. An account switch stays safe without the
 * expiry path stopping anything, because it is handled at the other end:
 * `daemon:sync-token` mints a fresh PAT and restarts the daemon whenever the
 * user id changes.
 *
