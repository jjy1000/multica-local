export interface RuntimeConfig {
  ...
  wsUrl: string;
  appUrl: string;
  // PR 1 (Stage C): optional PG backend preference. Absent or
  // "auto" means "follow the runtime priority chain"
  // (docker > local > native). Persisted as a JSON string by the
  // desktop config writer; Stage D will add a "native" option once
  // the bundled Postgres.app binary lands in userData.
  // PR 3 (Stage D-2): the picker narrowed to two outcomes — "native"
  // (bring up bundled PG) or "external" (reuse a reachable multica
  // PG). Legacy "docker" / "local" strings from v0.2.x installs are
  // still accepted by parseRuntimeConfig for forward-compat but are
  // mapped to "native" by server-manager at read time.
  pgBackend?: "auto" | "docker" | "local" | "native" | "external";
}
...
export function parseRuntimeConfig(raw: string): RuntimeConfig {
  ...
  const pgBackend = parsePgBackend(obj.pgBackend);
  return {
    schemaVersion: 1,
...
function parsePgBackend(raw: unknown): "auto" | "docker" | "local" | "native" | undefined {
  if (typeof raw !== "string") return undefined;
  if (raw === "auto" || raw === "docker" || raw === "local" || raw === "native") {
    return raw;
  }
  return undefined;
}
