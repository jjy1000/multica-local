...
export function createRendererWebPreferences(
  ...
  return {
    ...
    //   2. The only PDFs that reach an iframe here come from the attachment
    //      preview flow (`api.rawRequest` → server → signed download URL via
    //      `GET /api/attachments/{id}/download`); user-supplied URLs are
    //      routed through `setWindowOpenHandler` → `openExternalSafely` and
    //      cannot land in this renderer. The fork does not have CloudFront;
    //      download URLs are minted by the local backend on demand and are
    //      not user-typed.
