...
-- Ported 2026-08-16 from upstream commit 9c876d7a0 (PR #4844) — see
-- .omc/MUL-3963-MUL-4525-port-plan.md for the full port context.
--
...
-- ----------------------------------------------------------------------------
ALTER TABLE agent
    ADD COLUMN IF NOT EXISTS permission_mode TEXT NOT NULL DEFAULT 'private'
...
CREATE TABLE IF NOT EXISTS agent_invocation_target (
...
CREATE INDEX IF NOT EXISTS agent_invocation_target_agent_id_idx
...
CREATE INDEX IF NOT EXISTS agent_invocation_target_target_idx
...
-- ----------------------------------------------------------------------------
-- Refresh the agent_task_queue.originator_user_id column comment to the
-- MUL-3963 rules (the overlay now FOLLOWS invocation permission and uses
-- the agent OWNER's connection; the old "originator must equal owner" gate
-- is gone). This COMMENT ON COLUMN statement also drives the sqlc-generated
-- doc comments in models.go.
-- (Fork note 0.5.22: agent.composio_toolkit_allowlist doesn't exist in the
--  fork, so the corresponding COMMENT ON COLUMN was dropped from the port.
--  The originator_user_id comment still carries the MUL-3963 semantics.)
-- ----------------------------------------------------------------------------
