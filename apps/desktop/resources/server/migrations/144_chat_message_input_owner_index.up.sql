-- 144_chat_message_input_owner_index (MUL-4351)
-- ----------------------------------------------------------------------------
...
-- (`ListChatInputMessages`), keyed on the task's `chat_input_task_id`. Before
-- this there was no index on `chat_message.task_id` at all (only the
-- `chat_session_id/created_at` index from migration 033), so the lookup would
-- seq scan. This partial index covers exactly the user-message batch reads and
-- the existing task_id-keyed writes (`LinkChatMessageToTask`,
...
--
-- Renumbered from upstream 160 to 144.
-- ----------------------------------------------------------------------------
