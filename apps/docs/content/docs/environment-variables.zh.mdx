...
| `AUTH_TOKEN_TTL` | `720h`（30 天） | 浏览器 JWT 与 cookie 有效期；接受 Go duration 或正整数秒数。会话为滑动式：剩余有效期低于 TTL 一半时，下一个已认证请求会重新签发会话，因此持续使用的账号不会被定时登出。最小值为 `60s`（更短的值会在启动时告警并向上钳制） |
| `LOG_LEVEL` | 应用默认 | 日志级别 |
...
| `MULTICA_AGENT_TEMP_BASE` | `/tmp`（Linux/macOS） | 仅适用于 Linux/macOS。守护进程为每个 spawned 智能体创建的私有 task 临时目录的父目录（见 [issue #3999 / MUL-5799](https://github.com/multica-ai/multica/pull/5116)）。必须是现有、可写的绝对路径；无效值会使 task 启动失败，不会回退到 `/tmp`。请选择较短的路径——部分智能体 CLI（尤其是 Hermes）会在 `$TMPDIR` 下绑定 AF_UNIX socket；Linux 的 `sun_path` 上限为 108 字节，macOS 为 104 字节 |
