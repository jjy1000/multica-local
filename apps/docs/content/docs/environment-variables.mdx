...
| `AUTH_TOKEN_TTL` | `720h` (30 days) | Lifetime of browser JWTs and cookies; accepts a Go duration or a positive integer of seconds. Sessions slide: past half the TTL, the next authenticated request re-issues the session, so continuous use never hits a scheduled logout. Minimum `60s` (shorter values are clamped up with a startup warning) |
...
| `MULTICA_AGENT_TEMP_BASE` | `/tmp` (Linux/macOS) | Linux/macOS only. Parent directory for the per-task private temp dir the daemon creates for every spawned agent (see [issue #3999 / MUL-5799](https://github.com/multica-ai/multica/pull/5116)). Must be an existing, writable absolute path; an invalid value fails task startup instead of silently falling back to `/tmp`. Pick a short path — some agent CLIs (notably Hermes) bind AF_UNIX IPC sockets under `$TMPDIR`, and `sun_path` is capped at 108 bytes on Linux and 104 on macOS |
