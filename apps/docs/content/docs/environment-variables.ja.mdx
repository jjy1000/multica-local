...
| `AUTH_TOKEN_TTL` | `720h`（30 日） | ブラウザ JWT と cookie の有効期間。Go duration または正の整数秒を受け付けます。セッションはスライド式です。TTL の半分を下回ると次の認証済みリクエストで再発行され、継続利用では期限切れで強制ログアウトされません。最小値は `60s`（それより短い値は起動時に警告付きでクランプされます） |
...
| `MULTICA_AGENT_TEMP_BASE` | `/tmp`（Linux/macOS） | Linux/macOS 専用。スポーンされたエージェントごとにデーモンが作成するプライベート一時ディレクトリの親ディレクトリ（[issue #3999 / MUL-5799](https://github.com/multica-ai/multica/pull/5116) 参照）。既存かつ書き込み可能な絶対パスである必要があり、無効な値の場合は `/tmp` にフォールバックせずタスクの起動に失敗します。短いパスを選んでください。一部のエージェント CLI（特に Hermes）は `$TMPDIR` の下に AF_UNIX ソケットをバインドし、`sun_path` の上限は Linux で 108 バイト、macOS で 104 バイトです |
