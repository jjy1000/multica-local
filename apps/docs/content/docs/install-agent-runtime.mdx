...
| Minimum version | `1.1.54`. Earlier builds ignore `TMPDIR`/`TMP`/`TEMP` when extracting a native module and leak one 4-8 MB file into the shared system temp dir per run; the daemon refuses them at registration. |
