...
| `AUTH_TOKEN_TTL` | `720h`(30일) | 브라우저 JWT와 cookie 유효 기간. Go duration 또는 양의 정수 초 허용. 세션은 슬라이딩됩니다. TTL의 절반 이하로 떨어지면 다음 인증된 요청이 세션을 재발행하여, 계속 사용하는 계정은 예정된 로그아웃에 걸리지 않습니다. 최솟값은 `60s`(그보다 짧은 값은 시작 시 경고와 함께 클램프됨) |
...
| `MULTICA_AGENT_TEMP_BASE` | `/tmp` (Linux/macOS) | Linux/macOS 전용. 데몬이 스폰된 에이전트마다 생성하는 비공개 작업별 임시 디렉터리의 상위 디렉터리([issue #3999 / MUL-5799](https://github.com/multica-ai/multica/pull/5116) 참조). 기존이고 쓰기 가능한 절대 경로여야 하며, 잘못된 값은 `/tmp`로 자동 폴백하지 않고 작업 시작을 실패시킵니다. 짧은 경로를 선택하세요. 일부 에이전트 CLI(특히 Hermes)는 `$TMPDIR` 아래에 AF_UNIX 소켓을 바인드하며, `sun_path` 제한은 Linux에서 108바이트, macOS에서 104바이트입니다 |
