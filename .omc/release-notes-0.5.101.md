...
2. **`feat(dbreader)`** — MUL-7016 B.4 **partial** (foundation only). New `server/internal/dbreader/` package with a `Selector` type that owns replica fallback, a passive circuit breaker, and per-business routing. The `Handler` struct gains a `ReadSelector` field (initialized via `dbreader.NewPrimaryOnly(queries)`). The full runtime wiring — replica pool construction in `cmd/server/main.go`, the `RegistryOptions.ReplicaPool` field + `DBRouting` registration in `server/internal/metrics/registry.go`, and the `[]namedDBPool` refactor in `server/internal/metrics/db.go` — is **deferred to 0.5.101.1** because the fork has not yet back-ported `server/internal/dbstartup` (MUL-6502, 805 LOC, 19 files), and the wiring depends on `newReplicaDBPool` + `startupSettings.ConnectTimeout` + the multi-pool metrics signature that the wholesale `dbstats.go` swap would have introduced.

The third planned piece — MUL-7008 (WS claim poll 3m) — is **deferred to 0.5.101.1** because its actual scope (34 files, +736/-60, including a new `BatchClaimReadyTasks` sqlc CTE + new wsrpc.go + new capability negotiation) is significantly larger than the agent's deep-dive estimate (+250 LOC).
