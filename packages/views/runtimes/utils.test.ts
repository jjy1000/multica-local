...
describe("summarizeTaskUsage", () => {
  // claude-opus-4-8: 5 / 25 / 0.50 / 6.25 per million.
  const opus = {
    provider: "anthropic",
  ...
  // gpt-5.4: 2.50 / 15 / 0.25 / 2.50 per million.
  const terra = {
    provider: "openai",
    model: "gpt-5.4",
    input_tokens: 31_000,
  ...
  it("prices each model slice at its own rate, not the combined total", () => {
    // Priced separately: 1.9985 (opus) + 0.3245 (gpt-5.4).
    // Priced as one blob at either rate, the answer would be wrong.
    const s = summarizeTaskUsage([opus, terra]);
    expect(s?.cost).toBeCloseTo(1.9985 + 0.3245, 5);
    expect(s?.models).toEqual(["claude-opus-4-8", "gpt-5.4"]);
  });
