...
import { Users, Plus, Trash2, ArrowUpRight, Crown, Camera, Loader2, Pencil, FileText, Save, ListTodo } from "lucide-react";
...
// ---------------------------------------------------------------------------
// SquadOverviewPane — right column with three tabs
// (Members | Tasks | Instructions). Mirrors AgentOverviewPane:
// dirty-guard via AlertDialog when switching tabs with unsaved Instructions.
//
// `tasks` rides on the same `ActorIssuesPanel` the Agent detail page uses,
// keyed by (actorType="squad", actorId=squad.id). The panel already
// post-filters by assignee_type after a single `myIssueListOptions` fetch
// (assignee_id is UUID-unique across member / agent / squad namespaces),
// so a dedicated /squads/:id/issues round-trip is unnecessary here.
// ---------------------------------------------------------------------------
type SquadDetailTab = "members" | "tasks" | "instructions";
  ...
  return (
    <div className="flex min-h-[60vh] flex-col overflow-hidden rounded-lg border bg-background md:h-full md:min-h-0">
      <div className="flex shrink-0 items-center gap-0 overflow-x-auto border-b px-2 md:px-4">
        <button
          type="button"
          onClick={() => requestTabChange("members")}
        ...
        <button
          type="button"
          onClick={() => requestTabChange("tasks")}
          className={`flex shrink-0 items-center gap-1.5 whitespace-nowrap border-b-2 px-3 py-2.5 text-xs font-medium transition-colors ${
            activeTab === "tasks"
              ? "border-foreground text-foreground"
              : "border-transparent text-muted-foreground hover:text-foreground"
        ...
        <button
          type="button"
          onClick={() => requestTabChange("instructions")}
          className={`flex shrink-0 items-center gap-1.5 whitespace-nowrap border-b-2 px-3 py-2.5 text-xs font-medium transition-colors ${
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        {activeTab === "tasks" && (
          <div className="flex h-full min-h-[520px] flex-col">
            <ActorIssuesPanel actorType="squad" actorId={squad.id} />
          </div>
        )}
        {activeTab === "instructions" && (
          <div className="flex h-full flex-col p-4 md:p-6">
