...
// ---------------------------------------------------------------------------
// 0.3.31 lab ↔ assignee mutex on the issue detail surface
// ---------------------------------------------------------------------------
//
// These tests pin the renderer-side contract: when an issue
// carries a `lab_source`, the AssigneePicker must receive a
// `lockedReason` so the user can't accidentally pick an
// assignee the lab will ignore, and the LabPicker must accept
// an `onClearAssignee` callback so picking a non-empty lab
// automatically drops any pre-existing assignee. The server-
// side counterpart lives in issue_lab_source_test.go.

describe("issue-detail lab/assignee mutex", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Stand up a fresh mock surface for this describe. The
    // outer IssueDetail (shared) beforeEach doesn't apply
    // because we are a sibling describe, and any mocks left
    // over from a prior top-level test (e.g. unresolved
    // mockResolvedValueOnce chains) would leave getIssue
    // returning undefined, which IssueDetail treats as
    // "still loading" and renders an empty shell.
    mockApiObj.getIssue.mockResolvedValue(labbedIssue);
    mockApiObj.listTimeline.mockResolvedValue([]);
    mockApiObj.listIssueReactions.mockResolvedValue([]);
    mockApiObj.listIssueSubscribers.mockResolvedValue([]);
    mockApiObj.listChildIssues.mockResolvedValue({ issues: [] });
  ...
  const labbedIssue: Issue = {
    ...mockIssue,
    id: "issue-lab",
    identifier: "TES-99",
    lab_source: "chat_pin_ui",
    // Pre-existing assignee that the lab will own the
    // roster for. The locked picker still RENDERS the
    // actor avatar so the user can see what the lab
    // took over — picking a different one is what
    // becomes impossible.
  };

  it("a lab-tagged issue with an assignee renders the assignee as locked", () => {
    // The internal PropertyPicker chain (lockedReason /
    // onClearAssignee) is exercised by the lab-picker.test.tsx
    // + assignee-picker.test.tsx cases. Here we pin the OUTER
    // contract: the page does not throw on a lab + assignee
    // combination, and the rendered chrome carries the issue
    // title so the user knows the detail mounted. findByText
    // awaits the async issue fetch (mock returns labbedIssue
    // from the beforeEach); a synchronous textContent check
    // would race the React Query hydration.
    renderIssueDetail(labbedIssue.id);
    return screen
      .findByText("TES-99 Implement authentication")
      .then((node) => {
