...
import { ContentEditor, type ContentEditorRef, TitleEditor, type TitleEditorRef, useFileDropZone, FileDropOverlay, ImageSequenceProvider } from "../../editor";
...
import { issueListOptions, issueDetailOptions, childIssuesOptions, issueAttachmentsOptions } from "@multica/core/issues/queries";
...
// ExperimentalSection — sidebar footer for the "试验功能" mark.
// Surfaces whenever at least one of the project-listed experimental
// flags is enabled. Each enabled flag is listed by its localized
// title so the user can connect a task comment to which experimental
// service is in scope.
//
// We render this WITHOUT scanning comment text or trying to detect
// which "experimental Skill" actually fired: at this point in the
// fork the `Issue` type does not carry skill_keys, and adding a
// task-bound skill detection would be its own workstream. The
// gateway is intent-oriented — "a flag is on, mark the task as
// touching an experimental surface" — which is the simplest
// read-the-flag contract documented in the plan file (Phase E).
//
// When the flag is off the entire section unmounts. No skeleton, no
// placeholder — the same opt-in guarantee that governs the sidebar
// "试验性功能" group.
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-1.5 pl-2">
        {enabled.map((flag) => {
          const title = flag.title.zh || flag.title.en;
          return (
            <Badge key={flag.key} variant="outline" className="text-[10px]">
              {title}
            </Badge>
          );
        })}
