 ...
 * root per card/row. The per-row roots were both the largest render cost on
 * the board (N × (MenuRoot + Trigger) even though at most one menu is ever
 * open) and a freeze source: the menu's own status change re-buckets the
 * issue, which unmounts the anchored row — taking the open modal menu with
 * it. An open Base UI menu unmounted mid-open leaks its backdrop and popup,
 * and the backdrop swallows every click in the window (app-wide "卡死").
 * With the root at surface level, row unmounts can't touch the open menu.
 *
 * Items delegate: the per-item wrapper only attaches an `onContextMenu` that
 * reports (issue, cursor position) up to the provider's singleton, which
 * anchors the menu at the cursor via a virtual anchor element.
 *
...
export function IssueContextMenuProvider({
  ...
  // me" state off that attribute — see `data-[popup-open]:bg-accent`).
  // React never renders this attribute, so managing it imperatively here
  // keeps the affordance without re-rendering every row on open/close.
 ...
 * (ListView / BoardView / SwimLaneView / GanttView each mount one).
 */
