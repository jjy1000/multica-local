    ...
    create.mutate(
      ...
      },
      {
        onSuccess: (created) => {
          ...
          // no ASCII to slug gets one that cannot be guessed back from the
          // name — "客户确认" becomes `in_review_2` (MUL-6749) — so staying
          // silent would leave the admin no way to learn it short of reopening
          // the row. The dialog is already closing; a toast is the one surface
          // still visible.
          if (created?.key) {
