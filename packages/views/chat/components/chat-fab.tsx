...
export function ChatFab() {
  ...
  // Sum unread counts (0.3.4 ListChatSessionsByCreatorWithUnreadCount) when
  // present, fall back to the legacy has_unread boolean for older servers.
  // unread_count is per-session so the badge shows the actual pending
  // message count, not "9+ sessions have unread replies".
  const unreadSessionCount = sessions.reduce((acc, s) => {
  ...
  return (
    <Tooltip>
      <TooltipTrigger
        onClick={handleClick}
        className={cn(
          "absolute bottom-2 right-2 z-50 flex size-10 cursor-pointer items-center justify-center rounded-full bg-surface-raised text-muted-foreground shadow-[var(--floating-shadow)] ring-1 ring-surface-border transition-[background-color,color,box-shadow] hover:bg-surface-hover hover:text-foreground active:bg-surface-hover",
