...
// 0.3.23: accept optional `wsId` prop for pre-workspace surfaces
// (e.g. the Claude Lab `Chat` tab — see
// packages/views/experimental/components/experimental-chat-pane.tsx).
// When `wsId` is provided, the hook is bypassed entirely; the caller
// is responsible for ensuring a real workspace id (we don't fabricate
// one). When `wsId` is omitted, behaviour is unchanged — the
// component reads the active workspace from context as before.
//
// Hard rule: do not pass an empty string. Callers should either omit
// the prop or hand in a valid UUID. The runtime check is below.
interface ChatWindowProps {
  wsId?: string;
}

export function ChatWindow({ wsId: wsIdOverride }: ChatWindowProps = {}) {
  const { t } = useT("chat");
  ...
  // state: window open + a session active + (has_unread || unread_count > 0) → PATCH.
  // Migration 135 + 0.3.4 ListChatSessionsByCreatorWithUnreadCount replaces
  // the boolean with an integer; fall back to has_unread for older servers.
  const activeSession = sessions.find((s) => s.id === activeSessionId);
  const currentHasUnread =
    (activeSession?.unread_count ?? 0) > 0 || activeSession?.has_unread === true;
  useEffect(() => {
...
function SessionDropdown({
  ...
  useEffect(() => {
    ...
    const unreadSessionIds = new Set(sessions.filter((s) => (s.unread_count ?? 0) > 0 || s.has_unread).map((s) => s.id));
  ...
  const otherUnreadCount = sessions
    ...
    .reduce((acc, s) => {
      if (typeof s.unread_count === "number") return acc + s.unread_count;
      if (s.has_unread) return acc + 1;
      return acc;
    }, 0);
  ...
  const renderRow = (session: ChatSession) => {
    ...
    const showUnread = ((session.unread_count ?? 0) > 0 || session.has_unread) && !isCurrent;
    const isAgentIntro = session.is_agent_intro === true;
    const introBadgeText = isAgentIntro ? t(($) => $.window.agent_intro_badge) : null;
    ...
    return (
      ...
      >
        ...
        {introBadgeText && (
          <span
            className="shrink-0 rounded-full bg-brand/15 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-brand"
            aria-label={introBadgeText}
            title={introBadgeText}
          >
            {introBadgeText}
          </span>
        )}
