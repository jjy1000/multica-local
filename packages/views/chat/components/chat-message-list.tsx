          ...
          Header: () => (
            <div className="mx-auto w-full max-w-4xl px-5 pt-4">
              {isFetchingOlderMessages && (
                <div className="text-center text-caption text-muted-foreground">{t(($) => $.message_list.loading_older)}</div>
              )}
            </div>
          ),
          Footer: () => (
...
function AssistantMessage({
  ...
  // Migration 143 (MUL-4351) — explicit "no_response" terminal outcome.
  // The agent completed a direct-chat turn without any text reply. We render
  // a localized placeholder bubble so the turn boundary is visible without
  // faking a blank content bubble. Old clients (or rows where the server
  // still wrote a placeholder content) fall through to the normal render.
  if (message.message_kind === "no_response") {
    return (
      <div className="w-full space-y-1.5">
        <div className="text-body italic text-muted-foreground px-3 py-2 rounded-md bg-muted/40 border border-dashed">
          {t(($) => $.window.no_text_reply)}
        </div>
        <MessageFooter
          message={message}
          timeline={timeline}
...
function MiddleTextRow({
  ...
  return (
    <div className="py-0.5 text-caption text-muted-foreground prose prose-sm dark:prose-invert max-w-none [&>*:first-child]:mt-0 [&>*:last-child]:mb-0">
