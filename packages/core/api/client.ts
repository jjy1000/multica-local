...
export class ApiClient {
  ...
  // fails on non-web hosts:
  //   1. Origin — site-relative `/api/...` resolves to the renderer's
  //      document origin (Electron dev `localhost:5173`, packaged `file://`,
  //      mobile webview), NOT the API host. Prefixing `baseUrl` pins the
  //      request to the configured API origin. On web `baseUrl` is "" (the
  //      Next.js rewrite proxies `/api/*` same-origin), so the path stays
  //      relative and this is a no-op.
  //   2. Auth — desktop runs in token mode, so a cross-origin cookie is
  //      never attached; the Bearer token is the only working credential.
  //      `authHeaders()` injects it (plus the CSRF double-submit header used
  //      by cookie-mode web, the workspace slug, and client identity).
  //
  // Returns the raw Response WITHOUT the throw-on-error / JSON-decode
  // behaviour of the typed `fetch<T>` path, because these callers own their
  // own status handling (204 on delete, streamed SSE bodies, best-effort
  // GETs). Caller-supplied headers win over the injected defaults.
  ...
  async usernameLogin(name: string): Promise<LoginResponse> {
  ...
  /**
   * Reorders the active statuses within one category (drag-reorder in the
   * settings page). Owner/admin only; returns the updated catalog.
   * (MUL-6243)
   */
  ...
  async listExperimentalFlags(): Promise<ExperimentalFlag[]> {
    ...
    const parsed = parseWithFallback(raw, ExperimentalFlagsListSchema, { flags: [] }, {
      endpoint: "GET /api/experimental-flags",
    });
    return parsed.flags;
  }
  ...
  // 0.3.21: trigger an explicit install pass for an installable Labs
  // resource. The flag toggle endpoint already restores lock-table rows
  // and writes the install marker, but does NOT create the underlying
  // workspace / skill / agent / squad rows from the bundled manifest —
  // that requires the dedicated install handler which writes the manifest
  // payload (291 skills for claude_science, 5 agents, 5 squads, etc.).
  // Returns 202 Accepted when the install runs synchronously inside the
  // request lifetime; callers should re-fetch workspace state afterwards
  // because the rows land via a single tx and the response is sent before
  // the rerender.
  async installExperimentalResources(key: string): Promise<void> {
    await this.fetch(
      `/api/experimental-resources/${encodeURIComponent(key)}/install`,
      { method: "POST" },
    );
  }
...
// clientErrorMessage returns the server's error message only when the error
// is a CLIENT error (4xx). Handlers write those for the user —
// "autopilot is not active", "Idempotency-Key is too long" — so they are
// worth rendering. A 5xx message is internal detail (Go error chains,
// pgx table/constraint names, internal ids) that must never reach a toast
// (MUL-6472), and a non-ApiError is a transport failure whose message
// ("Failed to fetch") says nothing a user can act on. Both return undefined
// so the caller falls back to its own localized sentence.
