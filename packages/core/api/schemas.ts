 ...
 * (Desktop). A cookie-authenticated browser gets the renewed session as a
 * Set-Cookie and must never be handed a readable JWT, so the field is absent
...
export const IssueSchema = z.object({
  ...
  // issue list. Nothing reads this field to make a decision — useStatusLabel
  // resolves the label from the catalog — so dropping it costs a fallback to
  // the key. (MUL-6749)
