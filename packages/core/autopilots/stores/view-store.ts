...
export interface AutopilotListFilters {
  ...
  triggerKinds: string[];
  creators: string[];
  // `triggers` mirrors triggerKinds; the AutopilotListToolbar
  // (packages/views/autopilots/components/autopilot-list-toolbar.tsx)
  // reads filters.triggers. Keep both in sync — writer side uses
  // triggerKinds (matches the agent/automation trigger taxonomy);
  // toolbar reads triggers. The store keeps them equivalent.
  triggers: string[];
}
      ...
      toggleFilter: (key, value) =>
        set((state) => {
          ...
          const next = list.includes(value)
            ...
            : [...list, value];
          // Keep triggerKinds / triggers equivalent — writer side
          // talks to the store via "triggerKinds", toolbar reads
          // "triggers". Single source of truth, two named keys.
          const patch: Partial<AutopilotListFilters> = { [key]: next };
          if (key === "triggerKinds") patch.triggers = next;
          else if (key === "triggers") patch.triggerKinds = next;
