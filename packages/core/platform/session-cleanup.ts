...
/**
 * Erase everything the finished session left on this client: per-workspace
 * persisted state, the desktop tab layout, the last-workspace cookie, and the
 * React Query cache.
 *
 ...
 * machine the previous user's drafts, chat selection, and tab titles.
 *
...
export function clearClientSessionData(
  ...
  // Clear workspace-scoped storage BEFORE clearing the React Query cache
