...
import { chatKeys, backfillTaskMessages, isTaskMessageTimelineHeld, mergeTaskMessagesBySeq } from "../chat/queries";
...
export function useRealtimeSync(
  ...
  useEffect(() => {
    ...
    // P0 fix (memory multica-0.3.2): the prior implementation did
    // `if (!payload.chat_session_id) return;` with the comment
    // `// issue tasks handled elsewhere`. Grep across 438 app/core files
    // found no matching handler — the issue-bound branch was effectively
    // dropped, so task:completed events on issue tasks left the issue
    // detail cache (issueKeys.detail(wsId, id)) stale and the GUI
    // rendered "still running" forever even when DB status was 'completed'.
    //
    // Mobile's apps/mobile/data/realtime/use-issue-realtime.ts:222 does
    // this correctly — invalidate issue detail on every
    // task:completed/failed for issue-bound tasks. Mirror that here so
    // web/desktop catches up. We deliberately do NOT also invalidate
    // issueKeys.tasksAll() — the task: prefix path (see
    // debouncedRefresh map above) already invalidates the "issues,tasks"
    // subtree on every `task:*` event, so adding it here would
    // double-invalidate and visibly flicker the issue panel on busy
    // workspaces. Per task is invalidated exactly once via the prefix.
    const unsubTaskCompleted = ws.on("task:completed", (p) => {
      const payload = p as TaskCompletedPayload;
      const wsId = getCurrentWsId();

      // Issue-bound branch: invalidate per-issue detail cache so the next
      // render refetches status, assignee, and the issue body. The task
      // list under the issue is invalidated by the `task:` prefix path
      // above; we only need to refresh the issue itself here.
      if (!payload.chat_session_id && payload.issue_id && wsId) {
        qc.invalidateQueries({
          queryKey: issueKeys.detail(wsId, payload.issue_id),
        });
        return;
      }
    ...
    const unsubTaskFailed = ws.on("task:failed", (p) => {
      ...
      // P0 fix (memory multica-0.3.2): mirror the task:completed issue-branch
      // above — issue-bound failures were being dropped on the floor with
      // `if (!payload.chat_session_id) return;` leaving the issue page
      // stuck on "queued / running" forever. Same fix as task:completed.
      if (!payload.chat_session_id && payload.issue_id && wsId) {
        qc.invalidateQueries({
          queryKey: issueKeys.detail(wsId, payload.issue_id),
        });
