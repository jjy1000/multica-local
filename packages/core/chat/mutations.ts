...
/**
 * Migration 139 + 0.3.4 PR-6: pin a chat session so it sticks to the top
 * of the user's session list. Optimistically sets pinned_at to now() in the
 * cached list. The server uses chat:session_updated to broadcast the change
 * (no dedicated chat:session_pinned event kind needed — the existing
 * session_updated broadcast carries the updated_at that drives the re-sort).
 */
export function usePinChatSession() {
  ...
  return useMutation({
    mutationFn: (sessionId: string) => {
      logger.info("pinChatSession.start", { sessionId });
      return api.pinChatSession(sessionId);
    },
    onMutate: async (sessionId) => {
      await qc.cancelQueries({ queryKey: chatKeys.sessions(wsId) });
      const prevSessions = qc.getQueryData<ChatSession[]>(chatKeys.sessions(wsId));
      const pinnedAt = new Date().toISOString();
      const apply = (old?: ChatSession[]) =>
        old?.map((s) => (s.id === sessionId ? { ...s, pinned_at: pinnedAt } : s));
      qc.setQueryData<ChatSession[]>(chatKeys.sessions(wsId), apply);
      return { prevSessions };
    },
    onError: (err, sessionId, ctx) => {
      logger.error("pinChatSession.error.rollback", { sessionId, err });
      if (ctx?.prevSessions) qc.setQueryData(chatKeys.sessions(wsId), ctx.prevSessions);
    },
    onSettled: () => {
      qc.invalidateQueries({ queryKey: chatKeys.sessions(wsId) });
...
/**
 * Inverse of usePinChatSession. Optimistically clears pinned_at to null.
 * Same optimistic / rollback / invalidate pattern.
 */
export function useUnpinChatSession() {
  ...
  return useMutation({
    mutationFn: (sessionId: string) => {
      logger.info("unpinChatSession.start", { sessionId });
      return api.unpinChatSession(sessionId);
    },
    onMutate: async (sessionId) => {
      await qc.cancelQueries({ queryKey: chatKeys.sessions(wsId) });
      const prevSessions = qc.getQueryData<ChatSession[]>(chatKeys.sessions(wsId));
      const apply = (old?: ChatSession[]) =>
        old?.map((s) => (s.id === sessionId ? { ...s, pinned_at: null } : s));
      qc.setQueryData<ChatSession[]>(chatKeys.sessions(wsId), apply);
      return { prevSessions };
    },
    onError: (err, sessionId, ctx) => {
      logger.error("unpinChatSession.error.rollback", { sessionId, err });
      if (ctx?.prevSessions) qc.setQueryData(chatKeys.sessions(wsId), ctx.prevSessions);
    },
    onSettled: () => {
      qc.invalidateQueries({ queryKey: chatKeys.sessions(wsId) });
